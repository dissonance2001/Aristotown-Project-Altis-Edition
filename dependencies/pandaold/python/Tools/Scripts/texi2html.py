...
class HTMLNode:
    ...
    def write(self, *lines):
        list(map(self.lines.append, lines))
    ...
    def finalize(self):
        ...
        self.prologue = (
            ...
            '  <LINK REL=Next HREF="'
                + makefile(self.__next__) + '" TITLE="' + self.__next__ + '">\n'
    ...
    def output_links(self):
        if self.cont != self.__next__:
            self.link('  Cont', self.cont)
        self.link('  Next', self.__next__, rel='Next')
...
class TexinfoParser:
        ...
        if self.nodefp:
            ...
            if not self.cont and \
               (not self.node.type or \
                (self.node.__next__ and self.node.prev and self.node.up)):
                self.node.finalize()
                self.node.flush()
            else:
    ...
    def bgn_ifset(self, args):
        if args not in list(self.values.keys()) \
    ...
    def bgn_ifclear(self, args):
        if args in list(self.values.keys()) \
        ...
        if (self.node):
            ...
            while self.nodestack:
                if self.nodestack[-1].type > type:
                    ...
                    if not self.nodestack[-1].__next__:
    ...
    def resetindex(self):
        ...
        #
        self.whichindex = {}
        for name in list(self.indextitle.keys()):
...
class HTMLHelp:
    ...
    def dumpfiles(self, outfile=sys.stdout):
        filelist = list(self.filenames.values())
        filelist.sort()
