...
class LabeledScaleTest(AbstractTkTest, unittest.TestCase):
    ...
    def test_widget_destroy(self):
        ...
        x = tkinter.ttk.LabeledScale(self.root)
        ...
        name = myvar._name
        x = tkinter.ttk.LabeledScale(self.root, variable=myvar)
        x.destroy()
        if self.wantobjects:
        ...
        tkinter.ttk.LabeledScale(self.root, variable=myvar)
    ...
    def test_initialization(self):
        ...
        x = tkinter.ttk.LabeledScale(master)
        ...
        for pair in passed_expected:
            x = tkinter.ttk.LabeledScale(self.root, from_=pair[0])
            self.assertEqual(x.value, pair[1])
            x.destroy()
        x = tkinter.ttk.LabeledScale(self.root, from_='2.5')
        self.assertRaises(ValueError, x._variable.get)
        x.destroy()
        x = tkinter.ttk.LabeledScale(self.root, from_=None)
        self.assertRaises(ValueError, x._variable.get)
        x.destroy()
        ...
        x = tkinter.ttk.LabeledScale(self.root, variable=myvar, from_=0.5)
        ...
        x = tkinter.ttk.LabeledScale(self.root, compound='top')
        check_positions(x.scale, 'bottom', x.label, 'n')
        x.destroy()
        x = tkinter.ttk.LabeledScale(self.root, compound='bottom')
        check_positions(x.scale, 'top', x.label, 's')
        x.destroy()
        # invert default positions
        x = tkinter.ttk.LabeledScale(self.root, compound='unknown')
        check_positions(x.scale, 'top', x.label, 's')
        x.destroy()
        x = tkinter.ttk.LabeledScale(self.root) # take default positions
        check_positions(x.scale, 'bottom', x.label, 'n')
        x.destroy()

        # extra, and invalid, kwargs
        self.assertRaises(tkinter.TclError, tkinter.ttk.LabeledScale, master, a='b')


    def test_horizontal_range(self):
        lscale = tkinter.ttk.LabeledScale(self.root, from_=0, to=10)
        lscale.pack()
...
class OptionMenuTest(AbstractTkTest, unittest.TestCase):
    ...
    def test_widget_destroy(self):
        ...
        optmenu = tkinter.ttk.OptionMenu(self.root, var)
        name = var._name
    ...
    def test_initialization(self):
        self.assertRaises(tkinter.TclError,
            tkinter.ttk.OptionMenu, self.root, self.textvar, invalid='thing')

        optmenu = tkinter.ttk.OptionMenu(self.root, self.textvar, 'b', 'a', 'b')
    ...
    def test_menu(self):
        items = ('a', 'b', 'c')
        default = 'a'
        optmenu = tkinter.ttk.OptionMenu(self.root, self.textvar, default, *items)
        found_default = False
        ...
        def cb_test(item):
            ...
            success.append(True)
        optmenu = tkinter.ttk.OptionMenu(self.root, self.textvar, 'a', command=cb_test,
            *items)
    ...
    def test_unique_radiobuttons(self):
        ...
        optmenu2 = tkinter.ttk.OptionMenu(self.root, textvar2, default, *items)
        optmenu.pack()
        optmenu.wait_visibility()
        optmenu2.pack()
