...
def temp_dir(path=None, quiet=False):
    ...
    if path is None:
        ...
        if (have_unicode and isinstance(path, str) and
...
class SuppressCrashReport:
    ...
    def __exit__(self, *ignore_exc):
        ...
        if sys.platform.startswith('win'):
            ...
            if self.old_modes:
                import _testcapi
                for report_type, (old_mode, old_file) in list(self.old_modes.items()):
...
class SaveSignals:
    ...
    def restore(self):
        for signum, handler in list(self.handlers.items()):
