...
class Demo:
	...
	cw = six.moves.tkinter.Label(w.interior(),
		text = 'A group with the\ndefault Label tag')
	cw.pack(padx = 2, pady = 2, expand='yes', fill='both')

	w = Pmw.Group(parent, tag_pyclass = None)
	w.pack(fill = 'both', expand = 1, padx = 6, pady = 6)
	cw = six.moves.tkinter.Label(w.interior(), text = 'A group\nwithout a tag')
	cw.pack(padx = 2, pady = 2, expand='yes', fill='both')

	radiogroups = []
	self.var = six.moves.tkinter.IntVar()
	self.var.set(0)
	radioframe = six.moves.tkinter.Frame(parent)
	w = Pmw.Group(radioframe,
		tag_pyclass = six.moves.tkinter.Radiobutton,
		tag_text='radiobutton 1',
		tag_value = 0,
		tag_variable = self.var)
	w.pack(fill = 'both', expand = 1, side='left')
	cw = six.moves.tkinter.Frame(w.interior(),width=200,height=20)
	cw.pack(padx = 2, pady = 2, expand='yes', fill='both')
	...
	w = Pmw.Group(parent,
		tag_pyclass = six.moves.tkinter.Checkbutton,
		tag_text='checkbutton',
		tag_foreground='blue')
	w.pack(fill = 'both', expand = 1, padx = 6, pady = 6)
	cw = six.moves.tkinter.Frame(w.interior(),width=150,height=20)
	cw.pack(padx = 2, pady = 2, expand='yes', fill='both')

	w = Pmw.Group(parent,
		tag_pyclass = six.moves.tkinter.Button,
