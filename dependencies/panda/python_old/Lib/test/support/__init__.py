...
def u(s):
    return six.text_type(s, 'unicode-escape')
...
if os.name == 'java':
    ...
    if have_unicode:
        ...
        if isinstance('', six.text_type):
            ...
            # 2 latin characters.
            TESTFN_UNICODE = six.text_type("@test-\xe0\xf2", "latin-1")
...
def temp_dir(path=None, quiet=False):
    ...
    if path is None:
        ...
        if (have_unicode and isinstance(path, six.text_type) and
...
def open_urlresource(url, check=None):
    import six.moves.urllib.parse, six.moves.urllib.request, six.moves.urllib.error, six.moves.urllib.parse

    filename = six.moves.urllib.parse.urlparse(url)[2].split('/')[-1] # '/': it's URL!
    ...
    f = six.moves.urllib.request.urlopen(url, timeout=15)
    try:
...
class TransientResource(object):
    ...
    def __exit__(self, type_=None, value=None, traceback=None):
        ...
        if type_ is not None and issubclass(self.exc, type_):
            for attr, attr_value in six.iteritems(self.attrs):
...
def threading_setup():
    if thread:
        return six.moves._thread._count(),
    else:
        return 1,
...
def wait_threads_exit(timeout=60.0):
    ...
    join() method.
    """
    old_count = six.moves._thread._count()
    try:
        yield
    finally:
        ...
        while True:
            count = six.moves._thread._count()
            if count <= old_count:
                break
