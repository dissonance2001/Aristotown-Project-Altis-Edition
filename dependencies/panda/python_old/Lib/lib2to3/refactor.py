    ...
    for fixer in fixer_list:
        if fixer.pattern:
            ...
            if fixer._accept_type is not None:
                ...
                every.append(fixer)
    for node_type in chain(six.itervalues(pygram.python_grammar.symbol2number),
...
class RefactoringTool(object):
    ...
    def refactor_file(self, filename, write=False, doctests_only=False):
        ...
        if doctests_only:
            ...
            if self.write_unchanged_files or (tree and tree.was_changed):
                ...
                self.processed_file(six.text_type(tree)[:-1], filename,
    ...
    def refactor_stdin(self, doctests_only=False):
        ...
        if doctests_only:
            ...
            if self.write_unchanged_files or (tree and tree.was_changed):
                self.processed_file(six.text_type(tree), "<stdin>", input)
            else:
    ...
    def refactor_doctest(self, block, lineno, indent, filename):
        ...
        if self.refactor_tree(tree, filename):
            new = six.text_type(tree).splitlines(True)
