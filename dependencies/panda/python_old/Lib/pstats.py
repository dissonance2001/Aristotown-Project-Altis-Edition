...
class Stats:
    ...
    def add(self, *arg_list):
        ...
        for func, stat in six.iteritems(other.stats):
    ...
    def get_sort_arg_defs(self):
        ...
        if not self.sort_arg_dict:
            ...
            bad_list = {}
            for word, tup in six.iteritems(self.sort_arg_dict_default):
                fragment = word
                while fragment:
                    if not fragment:
                        break
    ...
    def sort_stats(self, *field):
        if not field:
            self.fcn_list = 0
            return self
        if len(field) == 1 and isinstance(field[0], six.integer_types):
    ...
    def eval_print_amount(self, sel, list, msg):
        ...
        if isinstance(sel, six.string_types):
            ...
            if isinstance(sel, float) and 0.0 <= sel < 1.0:
                ...
                new_list = list[:count]
            elif isinstance(sel, six.integer_types) and 0 <= sel < count:
                count = sel
                new_list = list[:count]
    ...
    def print_call_heading(self, name_size, column_title):
        ...
        subheader = False
        for cc, nc, tt, ct, callers in six.itervalues(self.stats):
            if callers:
                value = next(six.itervalues(callers))
...
def add_callers(target, source):
    ...
    new_callers = {}
    for func, caller in six.iteritems(target):
        new_callers[func] = caller
    for func, caller in six.iteritems(source):
...
def count_calls(callers):
    ...
    nc = 0
    for calls in six.itervalues(callers):
        nc += calls
    return nc
