...
def show_socket_error(err, address):
    ...
    if err.args[0] == 61: # connection refused
        ...
        six.moves.tkinter_messagebox.showerror("IDLE Subprocess Error", msg, parent=root)
    else:
        six.moves.tkinter_messagebox.showerror("IDLE Subprocess Error",
...
def fix_scaling(root):
    ...
    if scaling > 1.4:
        for name in six.moves.tkinter_font.names(root):
            font = six.moves.tkinter_font.Font(root=root, name=name, exists=True)
            size = int(font['size'])
            if size < 0:
