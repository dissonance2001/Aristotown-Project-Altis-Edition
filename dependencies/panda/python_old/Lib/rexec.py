...
class RExec(ihooks._Verbose):
    ...
    def set_trusted_path(self):
        ...
        self.trusted_path = list(filter(os.path.isabs, sys.path))
    ...
    def make_sys(self):
        ...
        m.argv = ['RESTRICTED']
        m.path = list(self.ok_path)
        m.exc_info = self.r_exc_info
        m = self.modules['sys']
        l = list(self.modules.keys()) + list(self.ok_builtin_modules)
        l.sort()
    ...
    def r_execfile(self, file):
        ...
        """
        m = self.add_module('__main__')
        exec(compile(open(file, "rb").read(), file, 'exec'), m.__dict__)
