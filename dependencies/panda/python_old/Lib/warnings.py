...
def formatwarning(message, category, filename, lineno, line=None):
    ...
    if line:
        ...
        if isinstance(s, unicodetype) and isinstance(line, str):
            line = six.text_type(line, 'latin1')
        s += "  %s\n" % line
    if isinstance(s, unicodetype) and isinstance(filename, str):
        enc = sys.getfilesystemencoding()
        if enc:
            try:
                filename = six.text_type(filename, enc)
...
def filterwarnings(action, message="", category=Warning, module="", lineno=0,
    ...
    assert isinstance(message, six.string_types), "message must be a string"
    assert isinstance(category, (type, type)), \
           "category must be a class"
    assert issubclass(category, Warning), "category must be a Warning subclass"
    assert isinstance(module, six.string_types), "module must be a string"
    assert isinstance(lineno, six.integer_types) and lineno >= 0, \
