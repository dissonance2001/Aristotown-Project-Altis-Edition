...
class Maildir(Mailbox):
    ...
    def next(self):
        ...
        if not hasattr(self, '_onetime_keys'):
            self._onetime_keys = six.iterkeys(self)
        while True:
            try:
...
class MH(Mailbox):
    ...
    def get_message(self, key):
        ...
        try:
            ...
            f.close()
        for name, key_list in six.iteritems(self.get_sequences()):
            if key in key_list:
    ...
    def __len__(self):
        """Return a count of messages in the mailbox."""
        return len(list(six.iterkeys(self)))
    ...
    def set_sequences(self, sequences):
        ...
        try:
            ...
            for name, keys in six.iteritems(sequences):
                if len(keys) == 0:
                    continue
                f.write('%s:' % name)
                prev = None
    ...
    def _dump_sequences(self, message, key):
        ...
        for name, key_list in six.iteritems(all_sequences):
...
class MHMailbox:
    ...
    def __init__(self, dirname, factory=rfc822.Message):
        ...
        list = list(filter(pat.match, list))
        list = list(map(int, list))
        list.sort()
        # This only works in Python 1.6 or later;
        # before that str() added 'L':
        self.boxes = list(map(str, list))
        self.boxes.reverse()
        self.factory = factory
