...
import errno, logging, socket, os, six.moves.cPickle, struct, time, re
...
class SMTPHandler(logging.Handler):
    ...
    def __init__(self, mailhost, fromaddr, toaddrs, subject,
        ...
        if isinstance(toaddrs, six.string_types):
            toaddrs = [toaddrs]
...
class HTTPHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            url = self.url
            data = six.moves.urllib.parse.urlencode(self.mapLogRecord(record))
            if self.method == "GET":
                if (url.find('?') >= 0):
                    sep = '&'
                else:
