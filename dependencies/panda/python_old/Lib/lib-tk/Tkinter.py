...
class Variable:
    ...
    def trace_vinfo(self):
        ...
        return list(map(self._tk.splitlist, self._tk.splitlist(
        ...
        for k, v in cnf.items():
            if v is not None:
                ...
                if hasattr(v, '__call__'):
                    ...
                    nv = []
                    for item in v:
                        if not isinstance(item, (six.string_types, int, int)):
                            break
...
class Tk(Misc, Wm):
    ...
    def readprofile(self, baseName, className):
        ...
        if os.path.isfile(class_py):
            exec(compile(open(class_py, "rb").read(), class_py, 'exec'), dir)
        if os.path.isfile(base_tcl):
            self.tk.call('source', base_tcl)
        if os.path.isfile(base_py):
            exec(compile(open(base_py, "rb").read(), base_py, 'exec'), dir)
