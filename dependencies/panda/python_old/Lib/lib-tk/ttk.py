...
from six.moves.tkinter import _flatten, _join, _stringify, _splitdict

# Verify if Tk is new enough to not need the Tile package
_REQUIRE_TILE = True if six.moves.tkinter.TkVersion < 8.5 else False
...
def _format_optdict(optdict, script=False, ignore=None):
    ...
    opts = []
    for opt, value in six.iteritems(optdict):
...
def _format_mapdict(mapdict, script=False):
    ...
    opts = []
    for opt, value in six.iteritems(mapdict):
        opts.extend(("-%s" % opt,
...
def _script_from_settings(settings):
    ...
    for name, opts in six.iteritems(settings):
...
def _tclobj_to_py(val):
    ...
    if val and hasattr(val, '__len__') and not isinstance(val, six.string_types):
...
def setup_master(master=None):
    ...
    if master is None:
        if six.moves.tkinter._support_default_root:
            master = six.moves.tkinter._default_root or six.moves.tkinter.Tk()
        else:
            raise RuntimeError(
...
class Widget(six.moves.tkinter.Widget):
    ...
    def __init__(self, master, widgetname, kw=None):
        ...
        if not getattr(master, '_tile_loaded', False):
            # Load tile now, if needed
            _load_tile(master)
        six.moves.tkinter.Widget.__init__(self, master, widgetname, kw=kw)
...
class Entry(Widget, six.moves.tkinter.Entry):
...
class Panedwindow(Widget, six.moves.tkinter.PanedWindow):
    ...
    forget = six.moves.tkinter.PanedWindow.forget # overrides Pack.forget
...
class Scale(Widget, six.moves.tkinter.Scale):
...
class Scrollbar(Widget, six.moves.tkinter.Scrollbar):
...
class Treeview(Widget, six.moves.tkinter.XView, six.moves.tkinter.YView):
...
class LabeledScale(Frame, object):
    ...
    def __init__(self, master=None, variable=None, from_=0, to=10, **kw):
        ...
        self._variable = variable or six.moves.tkinter.IntVar(master)
...
class OptionMenu(Menubutton):
    ...
    def __init__(self, master, variable, default=None, *values, **kwargs):
        ...
        *values and additional keywords.
        ...
        self['menu'] = six.moves.tkinter.Menu(self, tearoff=False)

        self._variable = variable
        self._callback = kwargs.pop('command', None)
        if kwargs:
            raise six.moves.tkinter.TclError('unknown option -%s' % (
    ...
    def set_menu(self, default=None, *values):
        ...
        for val in values:
            menu.add_radiobutton(label=val,
                command=six.moves.tkinter._setit(self._variable, val, self._callback),
