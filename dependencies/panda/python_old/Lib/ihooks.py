...
class BasicModuleImporter(_Verbose):
    ...
    def install(self):
        self.save_import_module = six.moves.builtins.__import__
        self.save_reload = six.moves.builtins.reload
        if not hasattr(__builtin__, 'unload'):
            six.moves.builtins.unload = None
        self.save_unload = six.moves.builtins.unload
        six.moves.builtins.__import__ = self.import_module
        six.moves.builtins.reload = self.reload
        six.moves.builtins.unload = self.unload

    def uninstall(self):
        six.moves.builtins.__import__ = self.save_import_module
        six.moves.builtins.reload = self.save_reload
        six.moves.builtins.unload = self.save_unload
        if not six.moves.builtins.unload:
            del six.moves.builtins.unload
