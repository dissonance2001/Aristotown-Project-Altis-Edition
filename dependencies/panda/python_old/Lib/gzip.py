...
class GzipFile(io.BufferedIOBase):
    ...
    def __init__(self, filename=None, mode=None,
        ...
        if mode and 'b' not in mode:
            mode += 'b'
        if fileobj is None:
            fileobj = self.myfileobj = six.moves.builtins.open(filename, mode or 'rb')
        if filename is None:
            ...
            if not isinstance(filename, six.string_types) or filename == '<fdopen>':
                filename = ''
        if mode is None:
        ...
        if decompress:
            if arg == "-":
                ...
                if arg[-3:] != ".gz":
                    ...
                    continue
                f = open(arg, "rb")
                g = six.moves.builtins.open(arg[:-3], "wb")
        else:
            if arg == "-":
                f = sys.stdin
                g = GzipFile(filename="", mode="wb", fileobj=sys.stdout)
            else:
                f = six.moves.builtins.open(arg, "rb")
                g = open(arg + ".gz", "wb")
        while True:
            chunk = f.read(1024)
            if not chunk:
