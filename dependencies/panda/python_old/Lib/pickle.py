...
class Pickler:
    ...
    if StringType is UnicodeType:
        ...
        def save_string(self, obj, pack=struct.pack):
            six.text_type = obj.isunicode()

            if self.bin:
                if six.text_type:
                    obj = obj.encode("utf-8")
                l = len(obj)
                if l < 256 and not six.text_type:
    ...
    def save_dict(self, obj):
        ...
        self.memoize(obj)
        self._batch_setitems(six.iteritems(obj))
...
class Unpickler:
    ...
    def load_binunicode(self):
        len = mloads('i' + self.read(4))
        self.append(six.text_type(self.read(len),'utf-8'))
    ...
    def _instantiate(self, klass, k):
        ...
        if not instantiated:
            try:
                ...
                six.reraise(TypeError, "in constructor for %s: %s" % (
