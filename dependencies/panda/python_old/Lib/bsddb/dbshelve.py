...
if sys.version_info[0] >= 3 :
    import six.moves.cPickle  # Will be converted to "pickle" by "2to3"
else :
...
HIGHEST_PROTOCOL = six.moves.cPickle.HIGHEST_PROTOCOL
def _dumps(object, protocol):
    return six.moves.cPickle.dumps(object, protocol=protocol)
...
class DBShelf(MutableMapping):
    ...
    def items(self, txn=None):
        ...
        for k, v in items:
            newitems.append( (k, six.moves.cPickle.loads(v)) )
        return newitems
    ...
    def values(self, txn=None):
        ...
        return list(map(six.moves.cPickle.loads, values))
    ...
    def associate(self, secondaryDB, callback, flags=0):
        def _shelf_callback(priKey, priData, realCallback=callback):
            ...
            if sys.version_info[0] < 3 or isinstance(priData, bytes) :
                data = six.moves.cPickle.loads(priData)
            else :
                data = six.moves.cPickle.loads(bytes(priData, "iso8859-1"))  # 8 bits
    ...
    def get(self, *args, **kw):
        ...
        try:
            return six.moves.cPickle.loads(data)
        except (EOFError, TypeError, six.moves.cPickle.UnpicklingError):
    ...
    def _extract(self, rec):
        if rec is None:
            ...
            if sys.version_info[0] < 3 or isinstance(data, bytes) :
                return key, six.moves.cPickle.loads(data)
            else :
                return key, six.moves.cPickle.loads(bytes(data, "iso8859-1"))  # 8 bits
