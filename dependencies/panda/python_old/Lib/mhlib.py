...
class Folder:
    ...
    def putsequences(self, sequences):
        ...
        fullname = self.getsequencesfilename()
        f = None
        for key, seq in six.iteritems(sequences):
            s = IntSet('', ' ')
            s.fromlist(seq)
        ...
        if i >= 0:
            ...
            try:
                ...
                if not head in seqs:
                    ...
                    six.reraise(Error, msg, sys.exc_info()[2])
                msgs = seqs[head]
                if not msgs:
    ...
    def refilemessages(self, list, tofolder, keepsequences=0):
        ...
        if refiled:
            if keepsequences:
                tofolder._copysequences(self, list(refiled.items()))
            self.removefromallsequences(list(refiled.keys()))
        if errors:
            if len(errors) == 1:
                raise os.error(errors[0])
            else:
