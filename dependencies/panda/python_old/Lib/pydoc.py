...
import sys, imp, os, re, types, inspect, six.moves.builtins, pkgutil, warnings
...
<tr><td bgcolor="%s">%s</td><td>%s</td>''' % (bgcol, marginalia, gap)
    ...
    def docclass(self, object, name=None, mod=None, funcs={}, classes={},
        ...
        attrs = [data for data in classify_class_attrs(object) if visiblename(data[0], obj=object)]
        mdict = {}
        ...
        while attrs:
            ...
            if thisclass is six.moves.builtins.object:
                attrs = inherited
                continue
...
class Helper:
    ...
    for topic, symbols_ in six.iteritems(_symbols_inverse):
...
def serve(port, callback=None, completer=None):
    import six.moves.BaseHTTPServer, mimetools, select
    ...
    class DocHandler(six.moves.BaseHTTPServer.BaseHTTPRequestHandler):
...
pydoc</strong> by Ka-Ping Yee &lt;ping@lfw.org&gt;</font>'''
    ...
    class DocServer(six.moves.BaseHTTPServer.HTTPServer):
    ...
    DocServer.base = six.moves.BaseHTTPServer.HTTPServer
...
def gui():
    ...
    class GUI:
        def __init__(self, window, port=7464):
            ...
            self.server_frm = six.moves.tkinter.Frame(window)
            self.title_lbl = six.moves.tkinter.Label(self.server_frm,
                text='Starting server...\n ')
            self.open_btn = six.moves.tkinter.Button(self.server_frm,
                text='open browser', command=self.open, state='disabled')
            self.quit_btn = six.moves.tkinter.Button(self.server_frm,
                text='quit serving', command=self.quit, state='disabled')

            self.search_frm = six.moves.tkinter.Frame(window)
            self.search_lbl = six.moves.tkinter.Label(self.search_frm, text='Search for')
            self.search_ent = six.moves.tkinter.Entry(self.search_frm)
            self.search_ent.bind('<Return>', self.search)
            self.stop_btn = six.moves.tkinter.Button(self.search_frm,
            ...
            self.result_lst = six.moves.tkinter.Listbox(window, font=font, height=6)
            self.result_lst.bind('<Button-1>', self.select)
            self.result_lst.bind('<Double-Button-1>', self.goto)
            self.result_scr = six.moves.tkinter.Scrollbar(window,
                orient='vertical', command=self.result_lst.yview)
            self.result_lst.config(yscrollcommand=self.result_scr.set)

            self.result_frm = six.moves.tkinter.Frame(window)
            self.goto_btn = six.moves.tkinter.Button(self.result_frm,
                text='go to selected', command=self.goto)
            self.hide_btn = six.moves.tkinter.Button(self.result_frm,
