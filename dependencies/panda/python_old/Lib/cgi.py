...
def parse(fp=None, environ=os.environ, keep_blank_values=0, strict_parsing=0):
    ...
    if environ['REQUEST_METHOD'] == 'POST':
        ...
        if sys.argv[1:]:
            ...
            qs = ""
        environ['QUERY_STRING'] = qs    # XXX Shouldn't, really
    return six.moves.urllib.parse.parse_qs(qs, keep_blank_values, strict_parsing)
...
def parse_qsl(qs, keep_blank_values=0, strict_parsing=0, max_num_fields=None):
    ...
    warn("cgi.parse_qsl is deprecated, use urlparse.parse_qsl instead",
         PendingDeprecationWarning, 2)
    return six.moves.urllib.parse.parse_qsl(qs, keep_blank_values, strict_parsing,
                              max_num_fields)
...
class FieldStorage:
    ...
    def getvalue(self, key, default=None):
        ...
        if key in self:
            ...
            if type(value) is type([]):
                return list(map(attrgetter('value'), value))
            else:
                return value.value
        else:
            return default
    ...
    def read_urlencoded(self):
        ...
        if self.qs_on_post:
            qs += '&' + self.qs_on_post
        query = six.moves.urllib.parse.parse_qsl(qs, self.keep_blank_values,
    ...
    def read_multi(self, environ, keep_blank_values, strict_parsing):
        ...
        self.list = []
        if self.qs_on_post:
            query = six.moves.urllib.parse.parse_qsl(self.qs_on_post,
