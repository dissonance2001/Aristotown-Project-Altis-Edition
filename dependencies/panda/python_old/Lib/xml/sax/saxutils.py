...
import os, six.moves.urllib.parse, six.moves.urllib.request, six.moves.urllib.parse, six.moves.urllib.error, types
import io
import sys
from . import handler
...
class XMLGenerator(handler.ContentHandler):
    ...
    def characters(self, content):
        if not isinstance(content, six.text_type):
            content = six.text_type(content, self._encoding)
...
def prepare_input_source(source, base = ""):
    ...
    if source.getByteStream() is None:
        try:
            ...
            if isinstance(sysid, six.text_type):
                if not isinstance(basehead, six.text_type):
                    try:
                        basehead = basehead.decode(encoding)
                    except UnicodeDecodeError:
                        sysid = sysid.encode(encoding)
            else:
                if isinstance(basehead, six.text_type):
                    try:
        ...
        if isfile:
            ...
            source.setSystemId(six.moves.urllib.parse.urljoin(base, source.getSystemId()))
            f = six.moves.urllib.request.urlopen(source.getSystemId())
