...
class Checker:
    ...
    def addroot(self, root, add_to_do = 1):
        if root not in self.roots:
            ...
            scheme, netloc, path, params, query, fragment = \
                    six.moves.urllib.parse.urlparse(root)
            i = path.rfind("/") + 1
            if 0 < i < len(path):
                path = path[:i]
                troot = six.moves.urllib.parse.urlunparse((scheme, netloc, path,
    ...
    def addrobot(self, root):
        root = six.moves.urllib.parse.urljoin(root, "/")
        if root in self.robots: return
        url = six.moves.urllib.parse.urljoin(root, "/robots.txt")
        self.robots[root] = rp = six.moves.urllib_robotparser.RobotFileParser()
    ...
    def getpage(self, url_pair):
        ...
        scheme, path = six.moves.urllib.parse.splittype(url)
...
class Page:
    ...
    def getlinkinfos(self):
        ...
        base = six.moves.urllib.parse.urljoin(self.url, self.parser.getbase() or "")
        infos = []
        for rawlink in rawlinks:
            t = six.moves.urllib.parse.urlparse(rawlink)
            # DON'T DISCARD THE FRAGMENT! Instead, include
            # it in the tuples which are returned. See Checker.dopage().
            fragment = t[-1]
            t = t[:-1] + ('',)
            rawlink = six.moves.urllib.parse.urlunparse(t)
            link = six.moves.urllib.parse.urljoin(base, rawlink)
...
class MyURLopener(six.moves.urllib.request.FancyURLopener):
    ...
    http_error_default = six.moves.urllib.request.URLopener.http_error_default

    def __init__(*args):
        self = args[0]
        six.moves.urllib.request.FancyURLopener.__init__(*args)
        self.addheaders = [
    ...
    def open_file(self, url):
        path = six.moves.urllib.request.url2pathname(six.moves.urllib.parse.unquote(url))
        if os.path.isdir(path):
            ...
            try:
                ...
                six.reraise(IOError, msg, exc_tb)
            names.sort()
            s = MyStringIO("file:"+url, {'content-type': 'text/html'})
            s.write('<BASE HREF="file:%s">\n' %
                    six.moves.urllib.parse.quote(os.path.join(path, "")))
            for name in names:
                q = six.moves.urllib.parse.quote(name)
                s.write('<A HREF="%s">%s</A>\n' % (q, q))
            s.seek(0)
            return s
        return six.moves.urllib.request.FancyURLopener.open_file(self, url)
