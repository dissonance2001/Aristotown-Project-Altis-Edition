...
class InventoryItem(AstronStruct):
    ...
    def fromMongo(cls, json) -> 'InventoryItem':
        ...
        # Migrate the temporary IDs previously used by the three Makeship
        # profile nameplates into their final sorted IDs.
        if itemType == ItemType.Profile_Nameplate:
            itemSubtype = {
                900: 616,
                901: 617,
                902: 618,
            }.get(itemSubtype, itemSubtype)

        subtypeCls = ItemType.getSubtypeClass(itemType)
        itemClass = cls.getItemClass(itemType)
        return itemClass(
            itemType=itemType,
            itemSubtype=subtypeCls(itemSubtype),
            quantity=json['q'],
