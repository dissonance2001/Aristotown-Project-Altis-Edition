...
class ItemDefinition:
    ...
    def getClickable(self) -> tuple[bool, str]:
        """Can this item be selected to be placed into the world? Also provides a string reason if not."""
        return True, ""
    ...
    def getEstateModel(self, *extraArgs, item: Optional[InventoryItem] = None) -> NodePathWithState:
        return self.getItemModel(*extraArgs, item)
    ...
    def getItemModel(self, *extraArgs, item: Optional[InventoryItem] = None) -> NodePathWithState:
        """
        ...
        if self.CACHES_3D_MODEL and item:
            ...
            if not self.cached3dModels.get(itemId):
                self.cached3dModels[itemId] = self.makeItemModel(*extraArgs, item=item)
            ...
            model = self.makeItemModel(*extraArgs, item=item)
            if isinstance(model, Actor):
