...
class Inventory(AstronStruct):
    ...
    def canRemoveItem(self, item: InventoryItem, manualDelete: bool, mustHaveEnough: bool = True) -> bool:
        ...
        if manualDelete:
            if not inventoryBehavior.canDeleteItems():
                return False
            if not itemBehavior.canDelete():
                return False
    ...
    def removeItemQuantity(self, itemSubtype: IntEnum, quantity: int = 1):
        # TODO: This doesn't work bug main
        items = self.findItems(itemSubtype)
        if len(items):
            self.removeItem(items[0].clone(newQuantity=quantity))

    def removeAllItemsOfSubtype(self, itemSubtype: IntEnum):
        stacks = self.findItems(itemSubtype)
        for stack in stacks:
            self.removeItem(stack)

    def removeAllItemsOfType(self, itemType: ItemType):
        """
        Removes *ALL* items of a specified Type.
        Be very careful using this! It will remove *all* of the items of this category.
        """
        stacks = self.findItemsOfType(itemType)
        for stack in stacks:
            self.removeItem(stack)
    ...
    def findItemOfID(self, itemID: ItemID):
        """
        Finds an item matching the unique item ID.
        """
        item = None

        # This try except looks really ugly, but this is the one case where this cache function is not used
        # purely for internal queries. This is for a more front-end facing query, and thus does not need to
        # be beholden to the KeyError that may pop up from this function.

        try:
            item = self.cache.getItemFromID(itemID)
        except KeyError:
            print(f'Could not find item of id {itemID}')
