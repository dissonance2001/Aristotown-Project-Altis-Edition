...
class ProfileBackgroundDefinition(ItemDefinition):
    ...
    def makeItemModel(self, *extraArgs, item: Optional[InventoryItem] = None) -> NodePath:
        ...
        if found.isEmpty():
            # NOTE: this specific background's sub-node isn't present in
            # Altis's copy of the shared model asset (phase_3.5/models/gui/
            # profile/background) -- this is an asset-content gap, not a code
            # bug. Fall back to an empty placeholder instead of crashing.
            return background
        background = found.copyTo(background)
        return background
...
ZoneToProfileBackground: Dict[int, BackgroundItemType] = {
    ...
    ToontownGlobals.YeOlde:             BackgroundItemType.PG_Sky_YOTT,  # Clash calls this OldeToontown
...
ProfileBackgroundRegistry: Dict[IntEnum, ProfileBackgroundDefinition] = {
    ...
    BackgroundItemType.Event_Witch: ProfileBackgroundDefinition(
