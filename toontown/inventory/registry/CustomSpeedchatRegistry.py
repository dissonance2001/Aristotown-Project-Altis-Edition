...
# NOTE: Altis's actual custom-speedchat-string dict lives in otp.otpbase.OTPLocalizer
# (re-exported from OTPLocalizerEnglish), not toontown.toonbase.TTLocalizer -- Clash's
# own TTLocalizer.CustomSCStrings would be a disconnected dict nothing else reads from.
for itemEnum, itemDefinition in CustomSpeedchatRegistry.items():
    OTPLocalizer.CustomSCStrings[itemEnum.value] = itemDefinition.getText()
