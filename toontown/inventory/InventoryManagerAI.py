...
class InventoryManagerAI(DistributedObjectAI):
    ...
    def debug_applyLocalDelta(self, inventoryDelta: list):
        ...
        """
        avId = self.air.getAvatarIdFromSender()
        # NOTE: Clash gated this to dev realms only via RealmGlobals.getCurrentRealm().
        # Altis has no realm/environment concept, so this protocol is disabled by default here
        # to avoid exposing a live inventory-editing backdoor. Flip ENABLE_DEBUG_DELTA to True
        # only on a local/dev server if you need this for testing.
        ENABLE_DEBUG_DELTA = False
        if not ENABLE_DEBUG_DELTA:
    ...
    def debug_spawnTTCchest(self):
        ...
        inv.addItem(HatItemType.Hat_Cop_Classic)
        inv.addItem(HatItemType.Hat_Gag_TV)
