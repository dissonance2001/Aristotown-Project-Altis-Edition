...
class ItemFrame(EasyManagedButton):
    ...
    def setItem(self, item: Optional[InventoryItem] = None):
        ...
        # NOTE: Clash swallowed exceptions here on dev realms via RealmGlobals.
        # Altis has no realm concept, so this always uses the production path.
    ...
    def _setItemRender(self, item: Optional[InventoryItem]):
        if item is None:
            ...
            if not itemDef.getClickable()[0]:
                self['frameColor'] = hexToPCol("696969")  # i promise this isn't intentional
