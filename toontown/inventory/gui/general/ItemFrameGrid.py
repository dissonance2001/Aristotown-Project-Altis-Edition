...
class ItemFrameGrid(DirectFrame):
    ...
    def refresh(self, force: bool = False):
        ...
        if self.__hoveredFrame is not None:
            self.__hoverItemFrame(self.__hoveredFrame, True, None)
    ...
    def __hoverItemFrame(self, frame: ItemFrame, hovered: bool) -> None:
        if hovered:
            base.hoverMgr.hoverObject(frame, item=frame.item, itemType=HoverFrameTypes.InventoryItem, frameDir=self['hoverDir'])
    ...
    def currentPageIndex(self) -> int:
        ...
        if maxPages == 0:
            return 0
        if self['allowPageOvershoot']:
            currentPageIndex = math.floor(self.currentPage)
        else:
