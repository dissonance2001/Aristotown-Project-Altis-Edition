...
class EventsPage(ShtikerPage.ShtikerPage):
    ...
    def downloadArticlesTask(self, task):
        ...
        self.articleText = {}
        try:
            urlfile = six.moves.urllib.request.urlopen(self.getNewsUrl())
        except IOError:
        ...
        for index in range(len(urls) / 2):
            ...
            try:
                ...
                imageFile = six.moves.urllib.request.urlopen(imageUrl)
                data = imageFile.read()
            ...
            try:
                ...
                textFile = six.moves.urllib.request.urlopen(textUrl)
                data = textFile.read()
    ...
    def getFileUrl(self, fileUrl):
        ...
        urlStrings = ''
        try:
            urlfile = six.moves.urllib.request.urlopen(fileUrl)
            urlStrings = urlfile.read()
            urlfile.close()
        except IOError:
