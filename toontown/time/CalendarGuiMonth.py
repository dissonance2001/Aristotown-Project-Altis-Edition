...
from toontown.time.CalendarGuiDay import CalendarGuiDay
...
class CalendarGuiMonth(DirectFrame):
    ...
    def createGuiObjects(self):
        self.monthLabel = DirectLabel(
            parent=self.monthLocator, relief=None,
            text=TTLocalizer.Months[self.startDate.month].upper(), text_scale=0.075,
            text_font=ToontownGlobals.getMinnieFont(), text_fg=(40 / 255.0,
                                                                140 / 255.0,
                                                                246 / 255.0,
                                                                1.0))
        self.yearLabel = DirectLabel(
            parent=self.yearLocator, relief=None, text=str(self.startDate.year),
            text_scale=0.03, text_font=ToontownGlobals.getMinnieFont(), text_fg=(140 / 255.0,
                                                                                 140 / 255.0,
                                                                                 246 / 255.0,
                                                                                 1.0))
        self.weekdayLabels = []
        for posIndex in range(7):
            ...
            self.weekdayLabels.append(DirectLabel(
                parent=self.weekDayLocators[posIndex], relief=None,
                text=TTLocalizer.DayNamesAbbrev[adjustedNameIndex],
                text_font=ToontownGlobals.getInterfaceFont(), text_fg=(255 / 255.0,
                                                                       146 / 255.0,
                                                                       113 / 255.0,
                                                                       1.0),
                text_scale=0.05))
        ...
        self.monthRightArrow = DirectButton(
            parent=self.monthRightLocator, relief=None, image=(arrowUp,
                                                               arrowDown,
                                                               arrowHover,
                                                               arrowUp),
    ...
    def changeMonth(self, monthChange):
        if monthChange != 0:
            ...
            self.curDate = datetime(
                newYear, newMonth, 1, self.curDate.time().hour, self.curDate.time().minute,
                self.curDate.time().second, self.curDate.time().microsecond, self.curDate.tzinfo)
        self.monthLabel['text'] = (TTLocalizer.Months[self.curDate.month].upper(),)
