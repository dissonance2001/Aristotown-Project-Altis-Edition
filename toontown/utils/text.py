...
def plural(value: float) -> str:
    return "" if value == 1 else "s"


def makeCommaSeparatedItems(items: List[str], useAnd: bool = True, oxfordComma: bool = False) -> str:
    """
...
def wordwrapWithVerticalCentering(label, wordwrapValue, text=None):
    ...
    label.setZ(label.getZ() + (newHeight - originalHeight) / 2)
...
def adjustZAfterTextScaleChange(label, originalScale, text=None):
    ...
    label.resetFrameSize()
    label.setZ(label.getZ() + (originalHeight - newHeight) / 2)


def splitTextByWordwrap(*, text: str, wordwrapValue: float) -> list:
    """
...
def reduceNumberIntoString(num: int, short: bool = False,
                           ignoreRedundant: bool = False,
                           decimalPlaces: int = 2) -> str:
    """
...
def formatNumberWithCommas(num: float) -> str:
    """Formats a number with commas."""
    return "{:,}".format(num)


def formatTimeRemainingIntoString(seconds, timesToShow = 4, useAnd: bool = False) -> str:
    ...
    if days and timesToShow:
        retString += f'%s day{"s" if days != 1 else ""}, ' % days
        timesToShow -= 1
    if hours and timesToShow:
        retString += f'%s hour{"s" if hours != 1 else ""}, ' % hours
        timesToShow -= 1
    if minutes and timesToShow:
        retString += f'%s minute{"s" if minutes != 1 else ""}, ' % minutes
        timesToShow -= 1
    if timesToShow:
        retString += f'%s second{"s" if seconds != 1 else ""}, ' % seconds
        timesToShow -= 1
    if not useAnd:
...
def formatTimestampToAbsoluteTime(seconds, withTime=False):
    ...
    year = dt.year
    if not withTime:
        return f'{month} {day}{ordinalSuffix}, {year}'
    else:
        hour = dt.hour
        ...
        if not is24hour:
            return f'{month} {day}{ordinalSuffix}, {year}, {hour}:{minute}'
        else:
            if hour < 12:
                timeSuffix = 'AM'
            else:
                timeSuffix = 'PM'
            hour = ((hour - 1) % 12) + 1
            return f'{month} {day}{ordinalSuffix}, {year}, {hour}:{minute} {timeSuffix}'
