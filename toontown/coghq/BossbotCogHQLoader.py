...
class BossbotCogHQLoader(CogHQLoader.CogHQLoader):
    ...
    def __fountainDamageTick(self, task):
        ...
        self.oilSound = base.loader.loadSfx('phase_9/audio/sfx/CHQ_FACT_paint_splash.ogg')
        base.playSfx(self.oilSound, volume=0.5)
        task.delayTime = 5.0
        return task.again
    ...
    def loadPlaceGeom(self, zoneId):
        ...
        if zoneId == ToontownGlobals.BossbotHQ:
            ...
            gzLinkTunnel.find('**/LinkTunnel1').setName('linktunnel_gz_17000_DNARoot')
            self.makeSigns()
            ...
            tex = loader.loadTexture('phase_12/maps/ttr_bossbotHQ_Statue.png')
            bigcheeseFountainStatue.setTexture(tex, 1)
            bigcheeseEOTStatues.setTexture(tex, 1)
    ...
    def makeSigns(self):
        ...
        def makeSign(topStr, signStr, textId, scale = TTLocalizer.BCHQLsignText):
            ...
            signText = DirectGui.OnscreenText(text=TextEncoder.upper(TTLocalizer.GlobalStreetNames[textId][-1]),
                                              font=ToontownGlobals.getSuitFont(), scale=scale, fg=(0, 0, 0, 1),
                                              parent=sign)
    ...
    def __makeGeyserIval(self, task):
        ...
        geyserSequence = Sequence(Func(self.activeGeysers.append, geyser),
                                  geyser.scaleInterval(1.5, scale, blendType='easeOut'),
                                  geyserIval(geyser, scale, length), geyser.scaleInterval(1.5, 0.0, blendType='easeIn'),
                                  Func(self.activeGeysers.remove, geyser))
