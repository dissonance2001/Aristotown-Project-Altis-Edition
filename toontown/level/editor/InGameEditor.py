...
class InGameEditor(AppShell):
    ...
    def __init__(self, level, doneEvent, requestSaveEvent,
                 ...
                 wireframeEvent, oobeEvent, toggleCollEvent,
                 runEvent, texEvent,
                 **kw):
        ...
        self.toggleCollEvent = toggleCollEvent
        ...
        # Some keybinds
        self.accept('delete-up', self.decideHowToRemove)
        self.accept('alt-up', self.level.duplicateSelectedEntity)
        self.accept(self.getEventMsgName('Delete'), self.decideHowToRemove)
    ...
    def createMenuBar(self):
        ...
        self.collButton = Button(toggleFrame, text ='Collisions',
                                 command = self.doCs)
        self.collButton.pack(side = RIGHT, expand = 0)
    ...
    def decideHowToRemove(self, levelElement=None):
        try:
            selectedEntId = self.level.selectedEntity.entId
            if self.level.getEntity(selectedEntId).getNumChildren() > 0:
                self.removeSelectedEntityTree()
            else:
                self.removeSelectedEntity()
        except Exception as e:
            print(f'tried to delete an entity, but failed! Exception: {e}')
    ...
    def removeSelectedEntity(self):
        ...
        if self.level.removeSelectedEntity() != -1:
            self.explorer.update(fUseCachedChildren = 1)
            if parentEntId:
    ...
    def updateAttribEditPane(self, entId, levelSpec, entTypeReg):
        ...
        for attribName in attribNames:
            ...
            elif datatype == 'questId':
                self.addQuestIdWidget(levelSpec, entSpec, entId, attribName, params)
    ...
    def addBoolWidget(self, levelSpec, entSpec, entId, attribName, params):
        ...
        def booleanCommand(booleanVar = flag):
            """
            command which uses this variable (flag)
            """
    ...
    def addQuestIdWidget(self, levelSpec, entSpec, entId, attribName, params):
        def veCommand(questIdTuple):
            self.level.setAttribEdit(entId, attribName, (int(questIdTuple[0]), int(questIdTuple[1]), int(questIdTuple[2])))
        ...
        floaterLabels = ['Source', 'Chain', 'Step']
        floaterType = 'floater'
        ...
        # A method to call final command at the end of slider interaction
        widg._floaters.component('valuatorGroup')['postCallback'] = (
            lambda source, chain, step: veCommand((source, chain, step)))
        # Record widget and undo/redo function
        self.attribWidgets.append(widg)
        self.curEntWidgets[attribName] = lambda x: widg.set(x, 0)
    ...
    def onDestroy(self, event):
        ...
        self.ignore('alt-up')
        self.ignore(self.getEventMsgName('Delete'))
    ...
    def doCs(self):
        messenger.send(self.toggleCollEvent)
