"""
WARNING:
    This module has a lot of eval functions!!
    Thankfully, this is intended for internal use only.
"""
...
from toontown.level.editor import EditMgr, EditorGlobals
# This is required in order to eval the incoming spec repr string
from toontown.level.SpecImports import *
from toontown.level.editor.InGameEditorElements import *
import string
...
class InGameEditorEditMgr(EditMgr.EditMgr, InGameEditorEntityBase):
    """
    InGameEditorEditMgr(EditMgr.EditMgr, InGameEditorEntityBase)
    """
...
class AttribModifier(Entity.Entity, InGameEditorEntityBase):
    """
    AttribModifier(Entity.Entity, InGameEditorEntityBase)
    """
...
def getInGameEditorEntityCreatorClass(level):
    ...
    class InGameEditorEntityCreator(EntCreatorClass):
        """ This is the factory entity creator hijacked to create
            editor-specific entity classes for all of its types.  """
        ...
        def __init__(self, editor):
            ...
            for entType in entTypes:
                self.entType2Ctor[entType] = InGameEditorEntity
...
class DistributedInGameEditor(DistributedObject.DistributedObject, Level.Level, InGameEditorElement):
    ...
    def setSpecSenderDoId(self, doId):
        ...
        self.cr.relatedObjectMgr.abortRequest(self.blobRequest)
        self.blobRequest = self.cr.relatedObjectMgr.requestObjects([doId], allCallback=self.__gotSpecSender)

    def __gotSpecSender(self, specSender):
        specSender = specSender[0]

        def setSpecBlob(specBlob, blobSender = specSender, self = self):
            blobSender.sendAck()
            # This import is needed for the eval of the spec blob
            from toontown.level.LevelSpec import LevelSpec
            curSpec = eval(specBlob)
            self.gotCurrentSpec(curSpec)

        if specSender.isComplete():
            setSpecBlob(specSender.getBlob())
        else:
            evtName = self.uniqueName('specDone')
            specSender.setDoneEvent(evtName)
    ...
    def gotCurrentSpec(self, curSpec):
        ...
        # Last 1 overrides defeault colorScale
        ...
        base.startTk()
        from toontown.level.editor import InGameEditor
        ...
        toggleCollEvent = self.uniqueName('toggleCollEvent')
        ...
        self.editor = InGameEditor.InGameEditor(
            ...
            oobeEvent=oobeEvent,
            toggleCollEvent=toggleCollEvent,
            runEvent=runEvent,
            texEvent=texEvent,
            )
        ...
        self.accept(toggleCollEvent, self.doToggleColl)
        ...
        self.accept(self.editor.getEventMsgName('Flash'),
                    self.handleEntitySelect)
        # As a quick access key for identification.
        self.accept('f', self.handleEntitySelect)
    ...
    def editorIsLocalToon(self):
        """ Return True if we own this editor. """
    ...
    def handleEntitySelect(self, entity=None):
        if hasattr(self, 'selectedEntity') and entity is None:
            entity = self.selectedEntity
        self.selectedEntity = entity

        # At this rate, just return. There will be nothing happening here.
        if entity is None:
            return
        ...
        if entity is self:
            ...
            def safeSetColorScale(entityNode, r, g, b, a):
                if entityNode and not entityNode.isEmpty():
                    entityNode.setColorScale(r, g, b, a)

            entityNP = self.getEntInstanceNP(entity.entId)
            if entityNP is not None:
                dur = 1.25
                oColor = entityNP.getColorScale()
                flashIval = Sequence(Func(safeSetColorScale, entityNP, 1, 0, 0, 1),
                                     Wait(dur / 3),
                                     Func(safeSetColorScale, entityNP, 0, 1, 0, 1),
                                     Wait(dur / 3),
                                     Func(safeSetColorScale, entityNP, 0, 0, 1, 1),
                                     Wait(dur / 3),
                                     Func(safeSetColorScale, entityNP, oColor[0], oColor[1], oColor[2], oColor[3]))
                ...
                wireIval = Sequence(
                    Func(safeSetColorScale, entCp, 1, 0, 0, 1),
                    Wait(dur/3),
                    Func(safeSetColorScale, entCp, 0, 1, 0, 1),
                    Wait(dur/3),
                    Func(safeSetColorScale, entCp, 0, 0, 1, 1),
                    Wait(dur/3),
                    Func(entCp.removeNode))

                self.identifyIval = Parallel(flashIval,
                                             boundIval,
                                             wireIval)
    ...
    def doWireframe(self):
        pass#messenger.send('chatCommand', ['~wire'])

    def doOobe(self):
        pass#messenger.send('chatCommand', ['~oobe'])

    def doToggleColl(self):
        pass#messenger.send('chatCommand', ['~toggleCollisions'])

    def doCs(self):
        pass#messenger.send('chatCommand', ['~cs'])

    def doRun(self):
        pass#messenger.send('chatCommand', ['~run'])

    def doTex(self):
        pass#messenger.send('chatCommand', ['~tex'])
    ...
    def duplicateSelectedEntity(self, entity=None, newEntParentEntId=None):
        selectedEntity = self.selectedEntity if entity is None else entity
        try:
            selectedEntId = selectedEntity.entId
            parentEntId = selectedEntity._parentEntity.entId if newEntParentEntId is None else newEntParentEntId
        except AttributeError:
            self.editor.showWarning('Please select a valid entity to be removed first.', 'error')
            return
        ...
        if selectedEntity.getNumChildren() > 0:
            self.notify.info('copying tree of entities.')
        ...
        print(f'the copy attribs are: {copyAttribs}')
        copyAttribs['comment'] = ''
        ...
        def setNewEntityId(entId, self=self, removeAction=removeAction,
            ...
            for attribName, value in copyAttribs.items():
                # Manually override the copy attrib parent ent id to put our requested one in
                if attribName == 'parentEntId' and newEntParentEntId:
                    value = newEntParentEntId
            ...
            if duplicateChildren:
                for child in oldSelectedEntity.children:
                    self.duplicateSelectedEntity(entity=child, newEntParentEntId=entId)
