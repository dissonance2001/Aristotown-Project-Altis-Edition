...
class NodePathEntity(Entity.Entity, NodePath, NodePathAttribs):
    ...
    def setParentEntId(self, parentEntId):
        NodePathAttribs.setParentEntId(self, parentEntId)
        # Listen for when our parent entity is done generating
        # We may need to re-attach ourselves later if they are distributed
        self.level.setEntityCreateCallback(parentEntId, self.__parentEntityGenerated)

    def __parentEntityGenerated(self):
        parentEntity = self.level.getEntity(self.parentEntId)
        if isinstance(parentEntity, DistributedNodePathEntity):
            # Listen for when our parent distributed entity is re-generated,
            # as we may need to re-attach ourselves to it.
            self.accept(parentEntity.getDNPEGeneratedEventName(), self.__parentDistributedEntityRegenerated)

    def __parentDistributedEntityRegenerated(self):
        self.level.requestReparent(self, self.parentEntId)
...
class DistributedNodePathEntity(DistributedEntity.DistributedEntity,
    ...
    """
    PLEASE NOTE:
    If you are going to be parented other entities under a DistributedNodePathEntity,
    please ensure they are NodePathEntities. Otherwise, they will not work properly
    and everything may explode.
    """
    ...
    def announceGenerate(self):
        ...
        self.initNodePathAttribs(doReparent=1)
        messenger.send(self.getDNPEGeneratedEventName())

    def getDNPEGeneratedEventName(self):
        return f'DistributedNodePathEntity-Generated-{self.levelDoId}-{self.entId}'

    def delete(self):
        # Make sure we aren't killing our children that don't need to be
