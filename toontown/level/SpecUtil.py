...
from . import LevelSpec, LevelUtil, LevelConstants, EntityTypes
from .editor import EditorGlobals
from direct.showbase.PythonUtil import list2dict
import types

"""
TO CREATE A NEW SPEC:
from toontown.level import SpecUtil
from toontown.coghq import FactoryEntityTypes
SpecUtil.makeNewSpec('G:/Shared drives/Tech Team/Level Editor Data/SpecUtil Specs/leg-factory-spec', 'phase_9/models/cogHQ/SelbotLegFactory', FactoryEntityTypes)
OR, more simply:
use the command ~makeLevelSpec [modelPath]
This will automatically place the new spec file in the google shared drive.
"""
...
def makeNewSpec(filename, modelPath, entTypeModule=EntityTypes):
    ...
    result = spec.saveToDisk(fname, makeBackup=0)
    print('Done.')
    return result
...
def privUpdateSpec(spec, modelPath, entTypeModule, newZonesGloballyVisible=0):
    ...
    """
    assert EditorGlobals.wantLevelEditor()
    assert isinstance(entTypeModule, types.ModuleType)
    from . import EntityTypeRegistry
    etr = EntityTypeRegistry.EntityTypeRegistry(entTypeModule)
    spec.setEntityTypeReg(etr)

    if modelPath is None:
        modelPath = spec.getEntitySpec(LevelConstants.LevelMgrEntId)['modelFilename']
    else:
        spec.doSetAttrib(LevelConstants.LevelMgrEntId, 'modelFilename', modelPath)

    # load the model
    # disable texture loading for speed
    TexturePool.setFakeTextureImage('phase_3/maps/invisible.png')
