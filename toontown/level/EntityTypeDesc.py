...
from toontown.level import AttribDesc
...
class EntityTypeDesc:
    ...
    def isPermanent(self):
        """means that entity of this exact type cannot be inserted or removed in the editor"""
            ...
            for desc in base._attribDescs:
                ...
                for d in baseADs:
                    if desc.getName() == d.getName():
                        EntityTypeDesc.notify.debug('%s inherits attrib %s from multiple bases' % (c.__name__, desc.getName()))
                        break
                else:
                    baseADs.append(desc)
