...
from toontown.level import EntityTypeDesc
...
class EntityTypeRegistry:
    def __init__(self, entityTypeModule):
        ...
        # readlines() converts \r\n to \n
        with open(getPyExtVersion(EntityTypes.__file__)) as f:
        ...
        # avoid CR/LF issues by reading the file line by line
        # readlines() converts \r\n to \n
        with open(getPyExtVersion(self.entTypeModule.__file__)) as f:
            fileLines = f.readlines()
            hv.hashString(''.join(fileLines))
            s += str(hv.asHex())
            self.hashStr = s
