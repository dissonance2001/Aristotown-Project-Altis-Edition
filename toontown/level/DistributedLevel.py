...
from toontown.level import LevelUtil
...
class DistributedLevel(DistributedObject.DistributedObject, Level.Level):
    ...
    def initializeTitleText(self):
        self.titleText = OnscreenText.OnscreenText('', fg=self.titleColor, shadow=(0, 0, 0, 1),
                                                   font=ToontownGlobals.getSuitFont(), pos=(0, -0.5), scale=0.16,
                                                   drawOrder=0, mayChange=1)
        self.smallTitleText = OnscreenText.OnscreenText('', fg=self.titleColor, font=ToontownGlobals.getSuitFont(),
                                                        pos=(0.65, 0.9), scale=0.08, drawOrder=0, mayChange=1,
                                                        bg=(0.5, 0.5, 0.5, 0.5), align=TextNode.ARight)
    ...
    def setZoneIds(self, zoneIds):
        ...
        if hasattr(self, 'startTime') and len(self.zoneIds):
            # load if we have a valid zoneIds list and startTime
    ...
    def setStartTimestamp(self, timestamp):
        ...
        if hasattr(self, 'zoneIds') and len(self.zoneIds):
            # load if we have a valid zoneIds list and startTime
            self.privGotAllRequired()
    ...
    def privGotAllRequired(self):
        if hasattr(self, 'zoneIds') and hasattr(self, 'startTime') and len(self.zoneIds):
    ...
    def getZoneNode(self, zoneEntId):
        if zoneEntId not in self.zoneNum2node:
            self.notify.warning(f'Warning!! {zoneEntId} Not in dict! Most likely an invalid zone id included in visibility list of a zone.')
    ...
    def showInfoText(self, text = 'hello world'):
        ...
        if description and description != '':
            ...
            titleSeq = Sequence(
                ...
                Wait(3.1),
                LerpColorInterval(self.titleText, 0.5, Vec4(self.titleColor[0], self.titleColor[1], self.titleColor[2], self.titleColor[3]), startColor=Vec4(self.titleColor[0], self.titleColor[1], self.titleColor[2], 0.0))
            )
    ...
    def setOuch(self, penalty, anim = None):
        if anim == 'Squish':
            if base.cr.playGame.getPlace():
                base.cr.playGame.getPlace().request('Squished')
        elif anim == 'Fall':
            if base.cr.playGame.getPlace():
                base.cr.playGame.getPlace().request('FallDown')
        av = base.localAvatar
        av.stunToon()
