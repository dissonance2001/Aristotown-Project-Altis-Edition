...
class Entity(DirectObject):
    ...
    def destroy(self):
        ...
        """
        self.notify.debug('Entity.destroy() %s' % self.entId)
        ...
        if self.level:
            if self.level.isInitialized():
                ...
                self.notify.warning('Entity %s destroyed after level??' % self.entId)
        self.ignoreAll()
        del self.level
        del self.entId

    def getUniqueName(self, name, entId = None):
        """returns a name that is unique for a particular entity; defaults to this entity"""
        if entId is None:
            entId = self.entId
