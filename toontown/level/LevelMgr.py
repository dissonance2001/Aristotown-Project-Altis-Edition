...
from toontown.level import LevelMgrBase
...
class LevelMgr(LevelMgrBase.LevelMgrBase):
    ...
    def handleZoneCreated(self, entId):
        ...
        self.accept(self.level.getLevelPostCreateEvent(), self.privAssignZoneIds)
    ...
    def privAssignZoneIds(self):
        ...
        if not hasattr(self, 'level') or self.level is None or not hasattr(self.level, 'zoneIds') or len(self.level.zoneIds) < 1:
            # if something is wrong with self.level(.zoneIds), warn + return
            self.notify.warning("Failed to get level info (we probably didn't clean up in time); ignoring.")
            return
        ...
        for i in range(len(self.zoneNums)):
            ...
            try:
                ...
                zoneId = base.localAvatar.getZoneId() # Only fall back to this in facility loading errors, print to log.
                self.notify.warning("Failed to get zone data properly! We failed at index {}; zoneNums count={}, zoneIds count={}.".format(i, len(self.zoneNums), len(self.level.zoneIds)))
