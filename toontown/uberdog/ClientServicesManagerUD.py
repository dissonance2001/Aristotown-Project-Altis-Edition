...
class LocalAccountDB(AccountDB):
    ...
    def lookup(self, cookie, ip, callback):
        ...
        if str(cookie) not in self.dbm:
            ...
            # Note: this dbm only ever stores a userId->accountId mapping
            # (see storeAccountID below) -- there's no persisted access
            # level to read back here, so we default it same as a new account.
            response = {
                'success': True,
                'userId': cookie,
...
class LoginAccountFSM(OperationFSM):
    ...
    def enterQueryAccountDB(self):
        print('[DEBUG] enterQueryAccountDB: calling accountDB.lookup()')
        self.csm.accountDB.lookup(self.token, self.ip, self.__handleLookup)

    def __handleLookup(self, result):
        print('[DEBUG] __handleLookup: result=%r' % (result,))
    ...
    def enterRetrieveAccount(self):
        print('[DEBUG] enterRetrieveAccount: dbId=%r accountId=%r' % (self.csm.air.dbId, self.accountId))
    ...
    def enterCreateAccount(self):
        print('[DEBUG] enterCreateAccount: dbId=%r userId=%r' % (self.csm.air.dbId, self.userId))
        self.account = {
            'ACCOUNT_AV_SET': [0] * 6,
            'ESTATE_ID': 0,
            'ACCOUNT_AV_SET_DEL': [],
        ...
        self.csm.air.dbInterface.createObject(
            ...
            self.account,
            self.__handleCreate)
        print('[DEBUG] enterCreateAccount: createObject() call returned, waiting for callback')

    def __handleCreate(self, accountId):
        print('[DEBUG] __handleCreate: accountId=%r' % (accountId,))
    ...
    def enterStoreAccountID(self):
        print('[DEBUG] enterStoreAccountID')
    ...
    def __handleStored(self, success=True):
        print('[DEBUG] __handleStored: success=%r' % (success,))
        if not success:
...
class ClientServicesManagerUD(DistributedObjectGlobalUD):
    ...
    def login(self, cookie, authKey):
        print('[DEBUG] login() called, sender=%r' % (self.air.getMsgSender(),))
        ...
        # We used to ask Astron for the client's real network address here
        # (via CLIENTAGENT_GET_NETWORK_ADDRESS) before continuing login.
        # That request goes unanswered against this Astron build, hanging
        # every login indefinitely -- and LocalAccountDB.lookup() never
        # actually reads the ip it's given anyway. For local/LAN play,
        # skip the round trip and complete login immediately.
        self.completeLogin(sender, '0.0.0.0')

    def completeLogin(self, context, ip):
        print('[DEBUG] completeLogin() called, context=%r ip=%r' % (context, ip))
