...
class DistributedSuit(DistributedSuitBase.DistributedSuitBase, DelayDeletable):
    ...
    def setUnstableDNAString(self, dnaString):
        from toontown.suit import SuitDNA

        oldParent = self.getParent()
        oldPos = self.getPos(oldParent)
        oldHpr = self.getHpr(oldParent)
        ...
        self.reparentTo(oldParent)
        self.setPos(oldParent, oldPos)
        self.setHpr(oldParent, oldHpr)
    ...
    def setUnstableDNAString(self, dnaString):
        ...
        if dnaString in ('foreman', 'supervis', 'clerk', 'clubpres', 'ovt'):
            dna.newSuit(dnaString)
        else:
            dna.makeFromNetString(dnaString)
    ...
    def playDialogue(self, type, length, delay=0.0):
        ...
        if type == 'statementA' or type == 'statementB':
            ...
            notify.error('unrecognized dialogue type: %s' % type)
            return
