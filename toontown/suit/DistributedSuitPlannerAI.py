...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    CogdoRatio = 0
    SuitHoodInfo = [
        [ToontownGlobals.ToontownCentralOld, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80), (30, 20, 0, 0, 20, 20, 10),
         (1, 2, 3, 4, 5), [], 0],
        ...
        [ToontownGlobals.PunchlinePlace, 8, 20, 0, 5, 15, 3, (1, 5, 10, 40, 60, 80), (5, 5, 30, 30, 10, 10, 10),
         (1, 2, 3, 4, 5), [], 0],
        [ToontownGlobals.WackyWay, 8, 20, 0, 5, 15, 3, (1, 5, 10, 40, 60, 80), (30, 20, 5, 5, 20, 10, 10),
         (1, 2, 3, 4, 5, 6), [], 0],
        [ToontownGlobals.BuccaneerBoulevard, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80), (70, 10, 0, 0, 10, 10, 0),
         (2, 3, 4, 5, 6), [], 0],
        [ToontownGlobals.SeaweedStreet, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80), (0, 10, 50, 30, 0, 10, 0),
         (2, 3, 4, 5, 6), [], 0],
        [ToontownGlobals.LighthouseLane, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80), (10, 40, 10, 10, 30, 0, 0),
         (2, 3, 4, 5, 6), [], 0],
        [ToontownGlobals.AnchorAvenue, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80),(40, 0, 0, 30, 10, 10, 10),
         (3, 4, 5, 6, 7), [], 0],
        ...
        [ToontownGlobals.SopranoStreet, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80),(5, 5, 70, 5, 5, 5, 5),
         (5, 6, 7, 8, 9, 10), [], 0],
        [ToontownGlobals.PetuniaPlace, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80), (5, 5, 40, 10, 10, 30, 0),
         (3, 4, 5, 6, 7), [], 0],
        [ToontownGlobals.DaisyDrive, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80),(30, 50, 0, 0, 10, 10, 0),
         (3, 4, 5, 6, 7), [], 0],
        [ToontownGlobals.TulipTerrace, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80),(5, 5, 5, 70, 5, 10, 0),
         (3, 4, 5, 6, 7, 8), [], 0],
        [ToontownGlobals.SunflowerStreet, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80), (20, 10, 20, 10, 10, 5, 25),
         (3, 4, 5, 6, 7), [], 0],
        [ToontownGlobals.AlmondAvenue, 8, 20, 0, 99, 100, 4, (1, 5, 10, 40, 60, 80),(10, 10, 10, 20, 30, 10, 10),
         (6, 7, 8, 9, 10, 11), [], 0],
    ...
    def __init__(self, air, zoneId):
        ...
        self.air = air
        self.spawnMGRCogs('streetmgr_spawner')
        taskMgr.doMethodLater(180, self.spawnMGRCogs, 'streetmgr_spawner')
        self.zoneId = zoneId
    ...
    def spawnMGRCogs(self, Task):
        for manager in ToontownGlobals.streetMgrs2Zones:
            if self.zoneId in ToontownGlobals.streetMgrs2Zones[manager]:
                self.createNewSuit([], self.streetPointList, suitName=manager, suitLevel=ToontownGlobals.streetMgrs2Levels[manager])
    ...
    def createNewSuit(self, blockNumbers, streetPoints, toonBlockTakeover=None,
        ...
        executive=None, forceLevel=False):
        self.skeleChance = 0
        ...
        if (suitLevel is None) and (buildingHeight is not None):
            suitLevel = SuitBattleGlobals.getRandomSuitLevel(suitName
                )
        if suitName is not None and suitLevel is not None:
            ...
            if not forceLevel:
                suitLevel = SuitBattleGlobals.clampNaturalSuitLevel(
                    suitName,
                    suitLevel
                )
        else:
            suitLevel, suitType, suitTrack = (
                self.pickLevelTypeAndTrack(
                    suitLevel,
                    suitType,
                    suitTrack
                )
            )
        newSuit.setupSuitDNA(
            suitLevel,
            suitType,
            suitTrack,
            suitName=suitName,
        ...
        while numToAssign > 0:
            ...
            while repeat and (buildingTrack is not None) and (buildingHeight is not None):
                ...
                if numTarget >= SuitBuildingGlobals.buildingMinMax[self.zoneId][1] or numTarget >= numTotalBuildings:
                    ...
                    totalWeightPerTrack[5] -= weight * tracks[5]
                    totalWeightPerTrack[6] -= weight * tracks[6]
    ...
    def pickLevelTypeAndTrack(self, level=None, type=None, track=None):
        ...
        if type is None:
            possibleTypes = list(range(1, self.MAX_SUIT_TYPES_HQ + 1))
    ...
    def getSuitsForTier(dept, tier):
        ...
        for suitName in normalSuitNamesByDept.get(dept, ()):
            data = SuitBattleGlobals.SuitAttributes.get(suitName)
...
def spawnCog(name, level=None, revives=0, skelecog=0, waiter=0):
    ...
    if name in ['ceo', 'cj', 'clo', 'cfo', 'vp', 'cio', 'hocn', 'chairman2', 'chairman', 'ottoman']:
    ...
    if sp is None:
        return "Could not spawn %s in the current zone; no suit planner exists." % suitFullName
    ...
    if revives > 2:
        ...
        'autocad', 'clerk', 'wsi', 'ovt', 'supervis',
        'styx', 'nix', 'hydra', 'kerberos', 'charon',
        'watchm', 'foreman', 'chairp', 'bdirector',
        'ant', 'sya'
    ) and revives > 1:
        return "Unable to spawn %s with more than 1 revive." % suitFullName

    elif name in (
        'derrhand', 'dold', 'wsi', 'dopa',
        'erclaim', 'erfit', 'redd'
    ):
        revives = 1
    ...
    return "Spawned %s at Level %s in current zone." % (
        suitFullName,
        level
    )
