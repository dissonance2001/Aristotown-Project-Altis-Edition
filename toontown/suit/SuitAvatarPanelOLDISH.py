...
class SuitAvatarPanel(AvatarPanel.AvatarPanel):
    ...
    def __updateHp(self, currHp, maxHp, delta):
        ...
        self.labelInterval = Parallel(
            LerpColorScaleInterval(self.hpLabel, duration=.2, startColorScale=(1, 0, 0, 1), colorScale=(1, 1, 1, 1),
                                   blendType='easeInOut'),
            LerpFunctionInterval(__updateLabel, duration=.2, fromData=currHp + delta, toData=currHp,
                                 blendType='easeInOut')
        )
        self.labelInterval.start()

    def _cleanupSequence(self):
