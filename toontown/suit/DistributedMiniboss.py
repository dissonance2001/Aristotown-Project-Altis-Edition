...
class DistributedMiniboss(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedMiniboss')
    ...
    def __init__(self, cr):
        ...
        self.e2 = None
        self.e3 = None
        self.battleNode = self.attachNewNode('battleA')
    ...
    def setBattleIds(self, battleNumber, battleId, useless):
        self.battleNumber = battleNumber
        self.battleId = battleId
        self.cr.relatedObjectMgr.abortRequest(self.battleRequest)
        self.battleRequest = self.cr.relatedObjectMgr.requestObjects([self.battleId], allCallback=self.__gotBattles)

    def __gotBattles(self, battles):
        self.battleRequest = None
        if self.battle and self.battle != battles[0]:
    ...
    def releaseToons(self, finalBattle = 0):
        for toonId in self.involvedToons:
            toon = self.cr.doId2do.get(toonId)
            if toon:
                if self.battle and toon in self.battle.toons:
                    pass
                else:
