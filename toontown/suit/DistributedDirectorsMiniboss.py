...
class DistributedDirectorsMiniboss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedDirectorsMiniboss')
    debugPositions = False

    def __init__(self, cr):
        self.notify.debug('----- __init___')
        DistributedBossCog.DistributedBossCog.__init__(self, cr)
        FSM.FSM.__init__(self, 'DistributedDirectorsMiniboss')
        self.lawyers = []
        ...
        # self.bossbotBoss = BossCog.BossCog()
        # dna = SuitDNA.SuitDNA()
        # dna.newBossCog('c')
        # self.bossbotBoss.bossbotBoss = True
        # self.bossbotBoss.setDNA(dna)
        # self.bossbotBoss.initializeDropShadow()
        # self.bossbotBoss.setH(0)
        # self.bossbotBoss.loop('Ff_neutral')
        # self.bossbotBoss.setPosHpr(0, 5, 22, 0, 0, 0)
        # self.bossbotBoss.setName('C. E. O.\nBossbot')
        # self.bossbotBoss.doId = 0
        self.loop('Ff_neutral')
        from toontown.suit.DistributedSuitBase import DistributedSuitBase
        self.chairman = DistributedSuitBase(cr)
        suitDNA = SuitDNA.SuitDNA()
        suitDNA.newSuit('chairman')
        self.chairman.setDNA(suitDNA)
        self.chairman.setPickable(0)
        self.chairman.setDisplayName('Chairman\nBoardbot')
        self.chairman.doId = 0
        self.chairman.loop('neutral')
        self.chairman.reparentTo(render)
        self.chairman.setPosHpr(0, 146.7, 11.2, 180, 0, 0)
        self.chairman.hide()

        self.dold = DistributedSuitBase(cr)
        suitDNA = SuitDNA.SuitDNA()
        suitDNA.newSuit('dold')
        self.dold.setDNA(suitDNA)
        self.dold.setPickable(0)
        self.dold.setDisplayName('Director Of Land Development\nBoardbot\nLevel 25.mgr v2.0')
        self.dold.doId = 0
        self.dold.loop('neutral')
        self.dold.reparentTo(render)
        self.dold.setPosHpr(5, 106.75, 0, 90, 2.9, 0)

        self.dopa = DistributedSuitBase(cr)
        suitDNA = SuitDNA.SuitDNA()
        suitDNA.newSuit('dopa')
        self.dopa.setDNA(suitDNA)
        self.dopa.setPickable(0)
        self.dopa.setDisplayName('Director Of Public Affairs\nSellbot\nLevel 25.mgr v2.0')
        self.dopa.doId = 0
        self.dopa.loop('neutral')
        self.dopa.reparentTo(render)
        self.dopa.setPosHpr(5, 110.75, 0, 90, 2.9, 0)

        self.derrhand = DistributedSuitBase(cr)
        suitDNA = SuitDNA.SuitDNA()
        suitDNA.newSuit('derrhand')
        self.derrhand.setDNA(suitDNA)
        self.derrhand.setPickable(0)
        self.derrhand.setDisplayName('Derrick Hand\nBossbot\nLevel 25.mgr v2.0')
        self.derrhand.doId = 0
        self.derrhand.loop('neutral')
        self.derrhand.reparentTo(render)
        self.derrhand.setPosHpr(5, 102.75, 0, 90, 2.9, 0)
        return
    ...
    def loadEnvironment(self):
        ...
        self.sideDoor = self.geom.find('**/side_door')
        self.show()
       ...
       # self.openDoors = ElevatorUtils.getOpenInterval(self, self.leftDoor, self.rightDoor, self.openSfx, self.finalOpenSfx, self.elevatorType)
        ...
        self.elevatorModel.setScale(1.25)
        self.elevatorModel.setY(-3)
        ...
        self.elevatorMusic = base.loader.loadMusic('phase_12/audio/bgm/directors_elevator.ogg')
        self.promotionMusic = base.loader.loadMusic('phase_12/audio/bgm/directors_cutscene.ogg')
        ...
        self.battleOneMusic = base.loader.loadMusic('phase_12/audio/bgm/directors_encntr.ogg')
        ...
        for row in range(3):
            for column in range(3):
                ...
                x = centerX + ((column - 1) * spacingX)
                y = centerY + ((row - 1) * spacingY)

                chandelier.setPos(x, y, centerZ)
    ...
    def loadPodium(self):
        ...
        self.podium.reparentTo(self.geom)
    ...
    def loadWitnessStand(self):
        ...
        self.realWitnessStand.reparentTo(self.geom)
    ...
    def loadScaleNew(self):
        ...
        self.scaleNodePath.wrtReparentTo(self.geom2)
    ...
    def enterIntroduction(self):
        ...
        self.reparentTo(render)
        self.bossbotBoss = self
        self.setPos(-145, 155, 0)
        self.setHpr(90, 0, 0)
        self.loop('Ff_neutral')
        # self.stopAnimate()
    ...
    def makeIntroductionMovie(self, delayDeletes):
        derrhand = self.derrhand
        dold = self.dold
        dopa = self.dopa
        propellerInSfx = loader.loadSfx('phase_5/audio/sfx/ENC_propeller_in.ogg')

        def setCamFov(fov):
            base.camLens.setMinFov(fov/(4./3.))

        ceoPivotPos = Vec3(-45, 155, 0)

        track = Sequence(Func(self.bossbotBoss.addActive))
        track.append(LerpPosHprInterval(base.camera, 3, Vec3(-20, 100, 3.5), Vec3(45, 4, 0), blendType='easeInOut'))
        rollTrack, hpr = self.bossbotBoss.rollBossToPoint(
            Vec3(-145, 155, 0), self.bossbotBoss.getHpr(), ceoPivotPos, self.bossbotBoss.getHpr(), False)
        track.append(Parallel(
            Sequence(
                Wait(1.5),
                LerpHprInterval(self.sideDoor, 1.5, Vec3(-110, 0, 0))
            ),
            rollTrack,
            Sequence(
                Wait(5.5),
                LerpPosHprInterval(base.camera, 3, Vec3(-20, 60, 8), Vec3(0, 5, 0), blendType='easeInOut')
            ),
            Sequence(
                Wait(3),
                Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogOne, CFSpeech)
            )
        ))  # Move CEO out of door.
        rollTrack, hpr = self.bossbotBoss.rollBossToPoint(ceoPivotPos, hpr, ceoPivotPos, Vec3(25, 0, 0), False)
        track.append(Parallel(
            LerpHprInterval(self.sideDoor, 1.5, Vec3(0, 0, 0)),
            rollTrack,
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogTwo, CFSpeech)
        ))  # Move CEO to addressing point
        track.append(Sequence(
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogThree, CFSpeech),
            Wait(5)
        ))
        track.append(Sequence(
            Func(self.bossbotBoss.loop, 'Ff_speech'),
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogFour, CFSpeech),
            Wait(5)
        ))
        track.append(Sequence(
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogFive, CFSpeech),
            Wait(5)
        ))
        track.append(Parallel(
            LerpPosHprInterval(base.camera, 1, Vec3(-30, 120, 6.5), Vec3(15, 30, 0), blendType='easeOut'),
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogSix, CFSpeech)
        ))
        track.append(Wait(5))
        track.append(Sequence(
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogSeven, CFSpeech),
            Wait(5)
        ))
        track.append(Sequence(
            LerpPosHprInterval(base.camera, 2, Vec3(-9.16, 88.8, 7.3), Vec3(-43.73, 1.9, 0), blendType='easeOut'),
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogEight, CFSpeech),
            Wait(5),
            Func(self.bossbotBoss.clearChat)
        ))
        track.append(Sequence(LerpPosHprInterval(base.camera, 2.5, Vec3(-14.16, 102.73, 7.3), Vec3(-90, 2.9, 0),  blendType='easeInOut')))
        track.append(Parallel(
            Parallel(
                ActorInterval(derrhand, 'speak'),
                Func(derrhand.setChatAbsolute, TTLocalizer.DerrickHandResponse, CFSpeech)
            ),
            LerpPosHprInterval(base.camera, 7, Vec3(-16.66, 102.73, 7.3), Vec3(-90, 2.9, 0))
        ))
        track.append(Parallel(
            Func(derrhand.loop, 'neutral'),
            Func(derrhand.clearChat)
        ))
        track.append(Sequence(
            LerpPosHprInterval(base.camera, 2.5, Vec3(-9.16, 92.8, 7.3), Vec3(-43.73, 1.9, 0), blendType='easeOut'),
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogNine, CFSpeech),
            Wait(5),
            Func(self.bossbotBoss.clearChat)
        ))
        track.append(Sequence(LerpPosHprInterval(base.camera, 2.5, Vec3(-14.16, 106.73, 7.3), Vec3(-90, 2.9, 0), blendType='easeInOut')))
        track.append(Parallel(
            Parallel(
                ActorInterval(dold, 'finger-wag'),
                Func(dold.setChatAbsolute, TTLocalizer.LandDevelopmentResponse, CFSpeech)
            ),
            LerpPosHprInterval(base.camera, 4.5, Vec3(-16.16, 106.73, 7.3), Vec3(-90, 2.9, 0))
        ))
        track.append(Parallel(
            Func(dold.loop, 'neutral'),
            Func(dold.clearChat)
        ))
        track.append(Sequence(
            LerpPosHprInterval(base.camera, 2.5, Vec3(-9.16, 96.8, 7.3), Vec3(-43.73, 1.9, 0), blendType='easeOut'),
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogTen, CFSpeech),
            Wait(5),
            Func(self.bossbotBoss.clearChat)
        ))
        track.append(Sequence(LerpPosHprInterval(base.camera, 2.5, Vec3(-14.16, 110.73, 7.3), Vec3(-90, 2.9, 0), blendType='easeInOut')))
        track.append(Parallel(
            Parallel(
                ActorInterval(dopa, 'speak'),
                Func(dopa.setChatAbsolute, TTLocalizer.PublicAffairsResponse, CFSpeech)
            ),
            LerpPosHprInterval(base.camera, 7, Vec3(-16.66, 110.73, 7.3), Vec3(-90, 2.9, 0))
        ))
        track.append(Parallel(
            Func(dopa.loop, 'neutral'),
            Func(dopa.clearChat)
        ))
        track.append(Sequence(
            Parallel(
                LerpPosHprInterval(base.camera, 2, Vec3(-30, 120, 6.5), Vec3(15, 30, 0), blendType='easeOut'),
                Func(self.bossbotBoss.loop, 'Ff_neutral'),
                Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogEleven, CFSpeech)
            ),
            Wait(5)
        ))
        track.append(Parallel(
            Func(base.playSfx, propellerInSfx),
            Sequence(
                Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogTwelve, CFSpeech),
                Wait(5),
                Func(self.bossbotBoss.clearChat)
            )
        ))
        track.append(Func(self.chairman.addActive))
        track.append(Func(self.chairman.show)) # HERE HE IS
        track.append(Parallel(
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogTwelvea, CFSpeech),
            LerpPosHprInterval(base.camera, 2, Vec3(0, 130, 8), Vec3(0, 20, 0)),
            ActorInterval(self.chairman, 'landing'),
            Func(self.chairman.loop, 'neutral')
        ))
        track.append(Func(self.bossbotBoss.clearChat))
        track.append(Sequence(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogOnea, CFSpeech),
            Wait(3)
        ))
        track.append(Func(self.chairman.loop, 'speak'))
        track.append(Sequence(
            Parallel(
                Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogOne, CFSpeech),
                LerpPosHprInterval(base.camera, 3.5, Vec3(0, 125, 8), Vec3(0, 20, 10))
            )
        ))
        track.append(Sequence(
            Parallel(
                Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogTwo, CFSpeech),
                LerpPosHprInterval(base.camera, 3, Vec3(0, 130, 8), Vec3(0, 30, -10))
            )
        ))
        track.append(Sequence(
            Parallel(
                Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogFour, CFSpeech),
                LerpPosHprInterval(base.camera, 2.5, Vec3(0, 130, 13), Vec3(0, 5, 0))
            )
        ))
        track.append(Func(self.chairman.loop, 'effort'))
        track.append(Sequence(
            Parallel(
                Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogFive, CFSpeech),
                LerpPosHprInterval(base.camera, 0.5, Vec3(0, 135, 14), Vec3(0, 0, 0))
            ),
            LerpPosHprInterval(base.camera, 5, Vec3(0, 134, 14.4), Vec3(0, 0, 0))
        ))
        track.append(Func(self.chairman.loop, 'neutral'))
        track.append(Sequence(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogSix, CFSpeech),
            Wait(3),
            Func(self.chairman.clearChat)
        ))
        track.append(Sequence(
            LerpPosHprInterval(base.camera, 0.5, Vec3(0, 140, 14), Vec3(0, 0, 0)),
            Func(self.chairman.makeMouthDrop),
            Wait(2)
        ))
        track.append(Func(base.camera.setPos, 0, 30, 3))
        track.append(Func(base.camera.setHpr, -180, 5, 0))
        track.append(Parallel(
            LerpPosHprInterval(base.camera, 3, Vec3(0, 33, 4.5), Vec3(-180, 2.5, 0)),
            self.loseCogSuitsNoCamera(self.involvedToons)
        ))
        track.append(Sequence(
            Func(self.chairman.setPlayRate, 3.0, 'effort'),
            Func(self.chairman.pingpong, 'effort', fromFrame=102, toFrame=122),
            Parallel(
                Sequence(
                    Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogSeven, CFSpeech),
                    Wait(1.5),
                    Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogEight, CFSpeech)
                ),
                LerpColorScaleInterval(self.chairman, 2, Vec4(1.0, 0.0, 0.0, 1.0)),
                LerpPosHprInterval(base.camera, 0.5, Vec3(0, 135, 14.5), Vec3(0, 0, 0))),
            Wait(3)
        ))
        track.append(Sequence(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogNine, CFSpeech),
            Wait(4.5)
        ))
        track.append(Sequence(
            LerpPosHprInterval(base.camera, 1, Vec3(-30, 120, 6.5), Vec3(15, 30, 0), blendType='easeOut'),
            Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogThirteen, CFSpeech),
            Wait(2.5),
            Func(self.bossbotBoss.clearChat)
        ))
        track.append(Parallel(
            Func(self.chairman.loop, 'walk'),
            LerpHprInterval(self.chairman, 1, (90, 0, 0)),
            LerpColorScaleInterval(self.chairman, 1, Vec4(1.0, 1.0, 1.0, 1.0)),
            LerpPosHprInterval(base.camera, 1, Vec3(0, 130, 14), Vec3(0, 0, 0)),
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogNinea, CFSpeech)
        ))
        track.append(Sequence(
            Func(self.chairman.loop, 'neutral'),
            Wait(2)
        ...
        track.append(Sequence(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogNineb, CFSpeech),
            LerpPosHprInterval(base.camera, 3.5, Vec3(0, 130.58, 14.32), Vec3(0, 0, 0))
        ))
        track.append(Sequence(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogNinec, CFSpeech),
            LerpPosHprInterval(base.camera, 2.5, Vec3(0, 131, 14.5), Vec3(0, 0, 0))
        ))
        rollTrack, hpr = self.bossbotBoss.rollBossToPoint(ceoPivotPos, self.bossbotBoss.getHpr(), ceoPivotPos, Vec3(90, 0, 0), True)
        track.append(Parallel(
            Func(self.bossbotBoss.clearChat),
            LerpPosHprInterval(base.camera, 3, Vec3(-20, 60, 5), Vec3(0, 10, 0), blendType='easeInOut'),
            Sequence(
                Wait(3),
                Func(self.bossbotBoss.setChatAbsolute, TTLocalizer.DirectorsCEODialogThirteena, CFSpeech)
            ),
            Sequence(
                Wait(1.5),
                LerpHprInterval(self.sideDoor, 1.5, Vec3(-110, 0, 0))
        ...
        rollTrack, hpr = self.bossbotBoss.rollBossToPoint(ceoPivotPos, hpr, Vec3(-105, 155, 0), Vec3(90, 0, 0), True)
        track.append(Parallel(
            LerpHprInterval(self.sideDoor, 1.5, Vec3(0, 0, 0)),
            rollTrack
        ))  # Get out of there CEO!
        track.append(Sequence(
            Func(self.bossbotBoss.clearChat),
            Func(self.bossbotBoss.hide),
            Func(self.bossbotBoss.removeActive)
        ))
        track.append(Parallel(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogTen, CFSpeech),
            Func(self.chairman.loop, 'walk'),
            LerpHprInterval(self.chairman, 1, (180, 0, 0)),
            LerpPosHprInterval(base.camera, 1, Vec3(0, 125, 12.5), Vec3(0, 0, 0))
        ))
        track.append(Sequence(
            Func(self.chairman.loop, 'neutral'),
            LerpPosHprInterval(base.camera, 3.5,  Vec3(0, 125.33, 13), Vec3(0, 0, 0))
        ))
        track.append(Sequence(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogEleven, CFSpeech),
            LerpPosHprInterval(base.camera, 3.5, Vec3(0, 125.66, 13.5), Vec3(0, 0, 0))
        ))
        track.append(Sequence(
            Func(self.chairman.setChatAbsolute, TTLocalizer.DirectorsChairmanDialogTwelve, CFSpeech),
            Wait(1.5),
            self.chairman.beginSupaFlyMove(self.chairman.getPos(), False, 'chairmanFlyOut', False)
        ))
        track.append(Parallel(Func(self.chairman.removeActive), Func(dold.hide), Func(dopa.hide), Func(derrhand.hide)))
