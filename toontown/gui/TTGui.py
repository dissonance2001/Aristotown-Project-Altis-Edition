...
class ScalingButton(DirectButton):
    def __init__(self, parent = aspect2d, **kw):
        ...
        self.initialiseoptions(ScalingButton)
        self.bindScale()
        self.hoverInterval = None

    def bindScale(self):
        self.bind(DGG.WITHIN, self.hoverButton, [1.1])
        self.bind(DGG.WITHOUT, self.hoverButton, [1.0])
        self.baseScale = self.getScale()

    def destroy(self):
        if self.hoverInterval is not None:
            self.hoverInterval.pause()
            self.hoverInterval = None
        DirectButton.destroy(self)

    def setScale(self, *args, **kwargs):
        super().setScale(*args, **kwargs)
        self.baseScale = self.getScale()

    def hoverButton(self, scale, event):
        self.hoverInterval = Sequence(
            self.scaleInterval(.1, self.baseScale * scale * 1.1, blendType = 'easeInOut'),
            self.scaleInterval(.1, self.baseScale * scale, blendType = 'easeInOut')
        ).start()


class ScrollWheelFrame(DirectScrolledFrame):
    """
    Gives us a basic DirectScrolledFrame with mouse scrolling support.
    """
    ...
    def __init__(self, parent=aspect2d, **kw):
        optiondefs = (
            ...
            ('scrollCorrection', None, None),
        )

        self.defineoptions(kw, optiondefs)
        DirectScrolledFrame.__init__(self, parent, **kw)
        self.initialiseoptions(ScrollWheelFrame)
    ...
    def getCanvasWidth(self):
        """Returns the width of the canvas, not counting the scrollbar width."""
        left, right, *_ = self['canvasSize']
        return right - left - self['scrollBarWidth']

    def scroll(self, direction):
        if self['scrollCondition'] is True or (callable(self['scrollCondition']) and self['scrollCondition']() is True):
            canvasHeight = abs(self['canvasSize'][2] - self['canvasSize'][3])
            if canvasHeight == 0:
                # We can't have division by zero errors!!!!
                return

            scrollPercent = self['scrollDistance'] / canvasHeight
            self['verticalScroll_value'] = self['verticalScroll_value'] + scrollPercent * direction
        correction = self['scrollCorrection']
        if correction is not None:
            # When a scrollCorrection is defined, it will align the scroll value to
            # line up, assuming that the scrollbar is divided into segments of the correction value.
            self['verticalScroll_value'] = round(self['verticalScroll_value'] * correction) / correction

    def bindToScroll(self, gui):
        gui.bind(DGG.WHEEL_UP, lambda _: self.scroll(-1))
        gui.bind(DGG.WHEEL_DOWN, lambda _: self.scroll(1))

    def destroyChildren(self, exceptions=None):
        # familicide
        # taken from DirectScrolledFrame.destroy()
        if exceptions is None:
            exceptions = []
        for child in self.canvas.getChildren():
            childGui = self.guiDict.get(child.getName())
            if childGui in exceptions:
                continue
            if childGui:
                childGui.destroy()
            else:
                ...
                if simpleChildGui in exceptions:
                    continue
                if simpleChildGui:
