...
from panda3d.core import ConfigVariableDouble, ConfigVariableBool

from toontown.chat.ui.speedchat.SCConstants import SCMenuFinalizePriority
from toontown.chat.ui.speedchat.SCObject import SCObject
...
class SCMenu(SCObject, NodePath):
    ...
    def __init__(self, holder=None, binLevel=1):
    ...
    def appendFromStructure(self, structure):
        ...
        def addChildren(menu, childList):
            for child in childList:
                ...
                if isinstance(child, int):
                ...
                elif isinstance(child, SCEmoteTerminal):
                    menu.append(child)
                elif isinstance(child, list):
                    if isinstance(child[0], str):
                        holderTitle = child[0]
                        subMenu = SCMenu(binLevel=menu.binLevel + 1)
                        subMenuChildren = child[1:]
                    else:
                        menuType, holderTitle = child[0], child[1]
                        subMenu = menuType(binLevel=menu.binLevel + 1)
                    ...
                    raise Exception('error parsing speedchat structure. invalid child: %s' % child)
    ...
    def setBinLevel(self, binLevel):
        # As more menus continue to pop up, their bin will increase
        # to ensure that they are on top and the user is
        # properly interacting with the highest menu
        self.binLevel = binLevel
        self.setBin('sorted-gui-popup', GuiBinGlobals.SpeedChatBin + binLevel)
