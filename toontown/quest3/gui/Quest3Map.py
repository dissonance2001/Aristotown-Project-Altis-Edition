...
class QuestMap(DirectFrame):
    ...
    # zones that have valid street maps we can load
    StreetMapZones = [
        ToontownCentral,
        DonaldsDock,
        YeOlde,
    ...
    def getSuitIcon(self, dept):
        ...
        corpIcon = icons.find(f'**/{deptIconDict.get(dept)}')
        if not corpIcon:
            self.notify.warning(f"Couldn't find icon to use for dept {dept}!")
        icons.removeNode()
        return corpIcon
    ...
    def transformAvPos(self, pos):
        if self.cornerPosInfo is None:
            self.notify.warning("Couldn't find cornerPosInfo for map, defaulting to (0, 0)")
            return (0, 0)
    ...
    def start(self):
        ...
        if self.av:
            ...
            if hoodId not in self.StreetMapZones:
                self.stop()
                return

            shortname = ToontownGlobals.zoneAbbreviationMap.get(hoodId)
            mapsGeom = loader.loadModel('gui/common/models/cc_m_txc_gui_map_' + shortname)
            self.mapOpenButton.show()
            self.mapCloseButton.hide()

            # address: <zoneAbbreviation>_<zoneId>
            streetAddress = f"{shortname}_{zoneId}"

            if base.wantHalloween:
                # Try loading a halloween variant if it exists
            ...
            # If we are still empty, let's just bounce.
            # Most of these hits will be caused from a missing playground map.
            if not self.mapImage:
                self.notify.debug(f"Couldn't find map info for {streetAddress}!")
                self.stop()
                return
