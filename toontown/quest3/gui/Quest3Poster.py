...
from toontown.clashsuit.suit import BossCog, SuitGlobals
from toontown.clashsuit.suit.Suit import ModelDict, Suit, CustomSkelecogHeads
from toontown.clashsuit.suit.SuitDNA import SuitDNA, getSuitBodyType, getSuitDept
...
class QuestPoster(EasyManagedItem):
    ...
    def load(self):
        ...
        buttonCircleGui = loader.loadModel('phase_4/models/parties/schtickerbookHostingGUI')

        circle = genBgIconsTexcard.find('**/circle_1')
        circle.setColorScale(BG_CIRCLE_1_COLOR_DEFAULT)

        glass = commonIconsTexcard.find('**/magnify_1')

        jb = commonIconsTexcard.find("**/jellybean_1")
        ...
        mapIcon = buttonCircleGui.find('**/startPartyButton_inactive')
        ...
        buttonCircleGui.removeNode()
        mapIcon.removeNode()
        monthAsset.removeNode()
        commonIconsTexcard.removeNode()
    ...
    def _modifyRewardsHover(self, questReference):
        ...
        if questSource == QuestSource.KudosQuest:
            ...
            self.label_rewardsHover['image_scale'] = 1.0
            kudosGui.removeNode()
        else:
            genBgIconsTexcard = loader.loadModel(f'{GUI_ICON_MODEL_PATH}{GUI_TEXCARD_PREFIX}gen_bg_icons')
            circle = genBgIconsTexcard.find('**/circle_1')
            self.label_rewardsHover['image'] = circle
            self.label_rewardsHover['image_pos'] = (-1.05, 0, 1.03)
            self.label_rewardsHover['image_scale'] = 0.45
    ...
    def visual_setNpcFrame(self, positionIndex: int, npcId: int):
        ...
        # Make the geom
        if npcId == QuestGlobals.SNOWMAN_ID:
            ...
            geomScale = 0.019
            geomPos = (0, 10, -0.07)
        elif npcId in QuestGlobals.SUIT_NPC_IDS:
            geom = self.createSuitHead(QuestGlobals.SUIT_NPC_IDS[npcId])
        else:
    ...
    def visual_setSnowballGeom(self, positionIndex: int) -> None:
        ...
        snowballModel = loader.loadModel("phase_3.5/models/gui/cc_m_txc_gui_icon_throwables")
        geom = snowballModel.find('**/snowball_1')
        self.visual_setFrameGeom(positionIndex, geom=geom, scale=0.075)
    ...
    @staticmethod
    def createToonHeadWithAlternates(npcId: int) -> NodePath:
        """Returns an NPC Toon Head, with the alternates of the snowman and suit npcs"""
        geomPos = (0, 0, 0)
        geomHpr = (0, 0, 0)
        geomScale = 0.15
        ...
        if npcId == QuestGlobals.SNOWMAN_ID:
            ...
            geom = QuestPoster.createSuitHead(QuestGlobals.SUIT_NPC_IDS[npcId])
        else:
            geom = QuestPoster.createToonHead(npcId)

        newNode = NodePath('npc-holder')
        geom.reparentTo(newNode)
        geom.setPos(geomPos)
        geom.setHpr(geomHpr)
        geom.setScale(geomScale*1.7)
        return newNode
