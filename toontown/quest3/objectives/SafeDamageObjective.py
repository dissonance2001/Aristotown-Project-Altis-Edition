...
from toontown.quest3.QuestLocalizer import AuxillaryText_Against, AuxillaryText_Complete, HL_Damage, PROG_Damage, QuestProgress_Complete, SC_Damage, SC_DamageWeapon
...
class SafeDamageObjective(DamageBossObjective):
    ...
    def getDifficultyRange(questerType: QuesterType, questSource: QuestSource, **extraArgs):
        if questSource == QuestSource.KudosQuest:
            ...
            if questTier in (QuestTier.NEWBIE, QuestTier.TTC, QuestTier.BB, QuestTier.YOTT, QuestTier.DG):
                return
    ...
    def generateFromDifficulty(cls, rng, difficulty: float, questSource: QuestSource, **extraArgs):
        ...
        damageAmount = int(averageBossHp * 0.55) * (difficulty ** 1.3) // 40
    ...
    def modifyPoster(self, questReference: QuestReference, poster):
        ...
        if not (complete and self.npcReturnable):
            safe = loader.loadModel('phase_10/models/cogHQ/CBSafe')
            poster.visual_setFrameGeom(poster.LEFT, safe, 0.015, pos=(0, 10, -0.06))
            poster.visual_setFrameColor(poster.LEFT, 'orange')
            poster.visual_setFrameText(poster.LEFT, f'Deal {self.damageAmount} Safe Damage')
    ...
    def getSpeedchatMessages(self, quester, questReference: QuestReference) -> tuple:
        ...
        return SC_DamageWeapon % (self.damageAmount, cogName, "a safe"),


SafeDamageObjective() # thanks m ai n
