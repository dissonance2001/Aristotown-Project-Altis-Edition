...
from toontown.quest3.QuestLocalizer import (Anywhere, BefriendCog,
                                            HL_CogFriend, QuestProgress_Complete, SC_CogFriend,
                                            OBJ_CogFriend, PROG_Befriended)


class CogFriendObjective(QuestObjective):
    ...
    def calculateProgress(self, context: CogFriendContext, questReference: QuestReference, quester: Quester) -> int:
        return type(context) is CogFriendContext
    ...
    def modifyPoster(self, questReference: QuestReference, poster):
        ...
        poster.visual_setSuitHead(searchPoster, 'f')
        poster.visual_setFrameText(searchPoster, BefriendCog)
    ...
    def getProgressFormat(self, questReference):
        return questReference.getQuestProgress(self.objectiveIndex), 1, PROG_Befriended

    def __repr__(self):
        return f'CogFriendObjective({self._getKwargStr()[:-2]})'
