...
class DamageBossObjective(CogBossObjective):
    ...
    def generateFromDifficulty(cls, rng, difficulty: float, questSource: QuestSource, **extraArgs):
        ...
        averageBossHps = {
            "s": BossCogGlobals.SellbotBossMaxDamage * 0.65,
            "m": BossCogGlobals.CashbotBossMaxDamage[1],
            "l": BossCogGlobals.LawbotBossMaxDamage[1],
            "c": BossCogGlobals.BossbotBossMaxDamage[0],
            # todo: boardbot
        }
        zoneId = extraArgs.get("zoneId")
        if rng.random() < 0.5:
