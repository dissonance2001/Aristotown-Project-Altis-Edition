...
from toontown.quest3.QuestLocalizer import (GoSwim, HL_Swim, InThePlayground, QuestProgress_Complete,
                                            SC_Swim)


class SwimObjective(QuestObjective):
    ...
    def calculateProgress(self, context: SwimContext, questReference: QuestReference, quester: Quester) -> int:
        return type(context) is SwimContext
    ...
    def modifyPoster(self, questReference: QuestReference, poster):
        ...
        poster.visual_setFrameQuestionGeom(searchPoster)
        poster.visual_setFrameText(searchPoster, GoSwim)
    ...
    def getObjectiveGoal(self) -> str:
        return f"{GoSwim} {InThePlayground.lower()}"
    ...
    def getProgressFormat(self, questReference):
        return questReference.getQuestProgress(self.objectiveIndex), 1, GoSwim

    def __repr__(self):
        return f'SwimObjective({self._getKwargStr()[:-2]})'
