...
class DeliverGagObjective(VisitObjective):
    ...
    def getProgressFormat(self, questReference):
        return questReference.getQuestProgress(self.objectiveIndex), self.gagCount, PROG_Deliver

    def getObjectiveGoal(self) -> str:
        return OBJ_Deliver % f"{'a' if self.gagCount == 1 else str(self.gagCount)} Level {self.gagLevel+1} Gag{text.plural(self.gagCount)} to {self.getToNpcName()}"
