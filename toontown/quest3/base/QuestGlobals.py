...
QuestHistoryForPlaygroundCompletion = {
    ...
    ToontownGlobals.YeOlde:       QuestHistory(questSource=QuestSource.MainQuest, chainId=31),
...
# Iterate through the MainQuestLine, mark off all main quest buildings as protected.
for chain in MainQuestLine.questLine.values():
    ...
    for multiObjective in chain.steps.values():
        ...
        for questObjective in multiObjective.getQuestObjectives():
            ...
            # ALL zones are protected in the main questline
            if zoneId not in protectedBuildings:
                protectedBuildings.append(zoneId)


potentialBuildings = {}

# Iterate through the SideQuestLine, mark off all TTC/BB buildings and other buildings with 3+ hits as protected.
for chain in SideQuestLine.questLine.values():
    # Iterate the chain's multiobjectives.
    for multiObjective in chain.steps.values():
        # Iterate the objectives of the multiobjective.
        for questObjective in multiObjective.getQuestObjectives():
            ...
            if 1000 <= zoneId <= 2999 and zoneId not in protectedBuildings:
                protectedBuildings.append(zoneId)
                continue
            ...
            if zoneId in potentialBuildings:
                ...
                if potentialBuildings[zoneId] == 3 and zoneId not in protectedBuildings:
                    protectedBuildings.append(zoneId)
            else:
                potentialBuildings[zoneId] = 1
