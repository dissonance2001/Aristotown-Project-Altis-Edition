...
class QuestObjective:
    ...
    def getSpeedchatMessages(self, quester, questReference: QuestReference):
        ...
        :rtype: tuple
        """
        raise NotImplementedError(f"No speedchat messages text for {self.__class__.__name__}")
    ...
    def getHeadline(self, questReference: QuestReference, objectiveSelected: int = 0) -> str:
        ...
        raise NotImplementedError(f"No headline text for {self.__class__.__name__}")
    
    def getObjectiveGoal(self) -> str:
        """Returns the goal of the current objective, used for the RewardPanel."""
        raise NotImplementedError(f"No objective goal text for {self.__class__.__name__}")
    ...
    def getProgressString(self, questReference: QuestReference, progress: int) -> str:
        ...
        raise NotImplementedError(f"No progress stringtext for {self.__class__.__name__}")

    def getProgressFormat(self, questReference):
        """Returns a string to mark progress for tasks."""
        raise NotImplementedError(f"No progress format text for {self.__class__.__name__}")
