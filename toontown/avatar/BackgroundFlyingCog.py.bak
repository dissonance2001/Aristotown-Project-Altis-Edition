...
class BackgroundFlyingCog(Suit.Suit):
    ...
    def attachPropeller(self):
        # Defined here since this is only defined in DistributedSuitBase. Since these are background Cogs, these don't need to be distributed.
        if self.prop == None:
    ...
    def setDNA(self, dna):
        # Overriden from Suit so we can set DNA multiple times
        if self.style:
            self.flush()
        self.style = dna
        self.generateSuit()
    ...
    def flyTask(self, task):
        try:
            ...
            return Task.done # we got deleted
