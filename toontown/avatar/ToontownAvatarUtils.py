...
def createUniqueToon(name, dna, hat, glasses, backpack, shoes, x = 0, y = 0, z = 0, h = 0, p = 0, r = 0, parent = render, anim = 'neutral', LOD = 1000, isDisguised = False, suitType = 'f', isWaiter=False, coll = True,
                     isRental=False, colorType=NametagGroup.CCNonPlayer, cogLevels = [0, 0, 0, 0, 0], cheesyEffect=ToontownGlobals.CENormal, nametagStyle=100):
    newToon = Toon.Toon()
...
def createCog(cogType, x = 0, y = 0, z = 0, h = 0, p = 0, r = 0, isSkelecog = False, isWaiter = False, anim = 'neutral', parent = render, name = None, dept = None, level = None, coll = True):
    """
    By default, createCog generates a Suit. Use createdDistributedCog for a client-sided DistributedSuitBase.
    Suits are less expensive and also have the benefit of not creating a "distributed" object on clientside only.
    """
    newCog = Suit.Suit()
    ...
    if name != None:
        newCog.setName(name)
    if dept is False:
        nameInfo = TTLocalizer.SuitBaseNameWithoutDept % {'name': newCog._name,
         'level': level if level != None else newCog.getActualLevel()}
    elif cogType == 'sj':
        nameInfo = 'Shadow Justice'
    elif cogType == 'ph':
        nameInfo = TTLocalizer.SuitBaseNameWithLevel % {'name': newCog._name,
         'dept': 'Crazybot',
         'level': level if level != None else newCog.getActualLevel()}
    else:
        nameInfo = TTLocalizer.SuitBaseNameWithLevel % {'name': newCog._name,
         'dept': dept if dept != None else newCog.getStyleDept(),
         'level': level if level != None else newCog.getActualLevel()} # for a v2.0 pass `level = '69 ' + TTLocalizer.SkeleRevivePostFix` into the function
...
def createDistributedCog(cogType, x = 0, y = 0, z = 0, h = 0, p = 0, r = 0, isSkelecog = False, isWaiter = False, anim = 'neutral', parent = render, name = None, dept = None, level = None, coll = True,
                         isVirtual=False, colorType=NametagGroup.CCSuit):
    """
    Creating a DO for client only is hacky! ONLY use it if you can't use a Suit (eg. you need to make the cog fly in/out)
    """
