...
class ClashBattleBaseAI(DistributedObjectAI, SafeFSM, BattleBase, BattleListenerObject):
    defaultTransitions = {
    ...
    def __removeSuit(self, suit: ClashSuitBaseAI):
    ...
    # Add suit
    def addSuit(self, suit: ClashSuitBaseAI):
    ...
    def __joinSuit(self, suit: ClashSuitBaseAI):
    ...
    def joinSuitNoPending(self, suit: ClashSuitBaseAI):
    ...
    def __makeAvPending(self, avId, makePending: bool = True):
        ...
        if not makePending and av and isinstance(av, ClashSuitBaseAI):
            for suit in self.suits:
    ...
    def rewardDone(self):
        ...
        if self.ignoreResponses == 1:
            ...
            self.notify.warning('State %s is not in ClashBattleBaseAI!' % stateName)
            return
        elif self.toons.count(toonId) == 0:
    ...
    def getAdditionalMovieTime(self) -> float:
        ...
        for attack in self.battleCalc.getAttackOrder():
            ...
            for target in attack.getDeadTargets():
                ...
                if suit and isinstance(suit, ClashSuitBaseAI) and suit.dna.name in SuitBattleGlobals.DEATH_EXTEND_MOVIE_TIME:
    ...
    def __movieDone(self):
        ...
        for activeToon in self.activeToons:
            toon: ClashDistributedToonBaseAI = self.getToon(activeToon)
            if toon is not None:
                toon.hpOwnedByBattle = 0
    ...
    def addActiveSuit(self, suit: ClashSuitBaseAI) -> None:
