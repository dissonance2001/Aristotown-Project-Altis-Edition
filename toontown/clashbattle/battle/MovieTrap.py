...
def placeTrap(trapProp, suit, trapLevel):
    if not trapProp or trapProp.isEmpty():
        return
    trapProp.wrtReparentTo(suit)
    trapProp.show()
    trapName = BattleGlobals.AvProps[AttackEnum.TOON_TRAP][trapLevel]
    if trapName == 'rake':
        ...
        trapProp.setHpr(180, 0, 0)
        trapProp.setPos(0, MovieUtil.SUIT_TRAP_DISTANCE, 0)
        ...
        trapProp.setHpr(180, 90, 0)
        trapProp.setPos(0, MovieUtil.SUIT_TRAP_TNT_DISTANCE - 1.3, 0.4)
    elif trapName in ('quicksand', 'trapdoor', 'xspot'):
        trapProp.setPos(0, MovieUtil.SUIT_TRAP_DISTANCE, 0.025)
    elif trapName == 'spring':
        trapProp.setPos(0, MovieUtil.SUIT_TRAP_DISTANCE, -0.241)
    else:
...
def doTraps(traps):
    ...
    for trap in traps:
        ...
        if len(targets) == 1:
            ...
            if trap['level'] == UBER_GAG_LEVEL_INDEX:
                notify.debug(f"doTraps(): group trap targets={(x['suit'] for x in trap['target'])}, "
                             f"active battle suits={trap['battle'].activeSuits}")
                if len(traps) > 1:
    ...
    for trapList in suitTrapLists:
        ...
        for trap in trapList:
            level = trap['level']
            if level == 0:
                trapPropList.append([globalPropPool.getProp('banana')])
            elif level == 1:
                rake = globalPropPool.getProp('rake')
                rake.pose('rake', 0)
                trapPropList.append([rake])
            elif level == 2:
                trapPropList.append([globalPropPool.getProp('spring')])
            elif level == 3:
                trapPropList.append([globalPropPool.getProp('marbles')])
            elif level == 4:
                ...
                trapPropList.append([globalPropPool.getProp('tnt')])
            elif level == 8:
                trapPropList.append([globalPropPool.getProp('traintrack')])
            else:
    ...
    camTrack = traps[0]['battle'].camera.chooseTrapShot(camDuration)
...
def __doTrapLevel(trap, trapProps, explode=0):
    ...
    if level == 0:
        ...
        return __trapSpring(trap, trapProps, explode)
    elif level == 3:
...
def __createThrownTrapMultiTrack(trap, propList, propName, propPos=None, propHpr=None, anim=0, explode=0):
    ...
    if explode == 1:
        ...
        throwTrack.append(Func(placeTrap, trapProp, suit, level))
        if trapName == 'tnt':
            ...
            throwTrack.append(Func(sparks.start, particleNode))
...
def __createPlacedTrapMultiTrack(trap, prop, propName, propPos=None, propHpr=None, explode=0,
    ...
    trapPoint = Point3(0, MovieUtil.SUIT_TRAP_DISTANCE, -0.241 if propName == 'spring' else 0.025)
    trapDelay = 2.5
    ...
    for target in targets:
        ...
        trapTrack.append(Func(trapProp.setScale, 0.01, 0.01, 0.01))
        startScale = Vec3(1.7, 1.7, 1.7)
        ...
        trapTrack.append(Sequence(Wait(0.3), Parallel(xBounce, yBounce), Wait(0.3)))
        if explode == 1:
            dustNode = hidden.attachNewNode('DustNode')
    ...
    toonTrack.append(Parallel(ActorInterval(button, 'trap-button'), ActorInterval(toon, 'pushbutton')))
    ...
    if propName == 'quicksand':
        ...
        springSoundTrack = Sequence(Wait(2.3), Parallel(
            SoundInterval(buttonSound, duration=0.67, node=toon),
            Sequence(Wait(0.4), SoundInterval(propSound, duration=0.5, node=toon))))
    else:
    ...
    soundTrack = Sequence(Wait(2.3), SoundInterval(buttonSound, duration=0.67, node=toon), Wait(0.3),
                          SoundInterval(propSound, duration=0.5, node=toon)) if propName != 'spring' else springSoundTrack
...
def __trapSpring(trap, trapProps, explode):
    toon = trap["avatar"]
    targets = trap['target']
    for target in targets:
        suit = target['suit']
        notify.debug('toon: %s lays spring in front of suit: %d' % (toon.getName(), suit.doId))
...
def __trapWreckingBall(trap, trapProps, explode):
    ...
    for target in targets:
        suit = target['suit']
        notify.debug('toon: %s lays wrecking ball in front of suit: %d' % (toon.getName(), suit.doId))

    xspot = trapProps[0]
    return __createPlacedTrapMultiTrack(trap, xspot, 'xspot', explode=explode)
...
def createThrowingTrack(obj, target, duration=1.0, parent=render, gravity=-32.144):
    values = {}
    values['origin'] = None
    values['velocity'] = None

    def calcOriginAndVelocity(obj=obj, target=target, values=values, duration=duration, parent=parent, gravity=gravity):
    ...
    def throwPos(t, obj, duration, target, values=values, gravity=-32.144):
    ...
    return Sequence(Func(calcOriginAndVelocity), LerpFunctionInterval(throwPos, fromData=0.0, toData=1.0,
                                                                      duration=duration, extraArgs=[obj, duration,
                                                                                                    target]))
...
def __createPlacedGroupTrapTrack(trap, prop, propName, centerSuit, propPos=None, propHpr=None, explode=0):
    ...
    if True:
        ...
        # Increase the size of the train track based on the number of suits over 4.
        trapPropScale = MovieUtil.getRailroadXScale(battle)
        tracksTexScale = MovieUtil.getRailroadTextureXScale(battle)
        # Bit wonky, but otherwise the battle will not get the correct scale for adjusting the movie.
        battle.railroadTrapXScale = trapPropScale
        trapProp.find('**/tracksA').setTexScale(TextureStage.getDefault(), 1.0, tracksTexScale)
        trapTrack.append(LerpScaleInterval(trapProp, 1.2, Point3(trapPropScale, 1.0, 1.0)))
        if explode == 1:
            dustNode = hidden.attachNewNode('DustNode')
            ...
            for target in trap['target']:
                ...
                if otherSuit.battleTrapProp:
                    ...
                    otherTrapTrack.append(createCartoonExplosionTrack(otherDustNode, 'dust',
            ...
            for target in targets:
                ...
                if kbbonus == 0:
                    ...
                    suitTrack.append(MovieUtil.unlureSuit(unluredSuit, battle))
...
def enableTrapFreeMove(suit, battle):
    if suit and battle and suit.battleTrapProp:
        suit.battleTrapProp.wrtReparentTo(battle)


def disableTrapFreeMove(suit, battle):
    if suit and battle and suit.battleTrapProp:
        suit.battleTrapProp.wrtReparentTo(suit)
