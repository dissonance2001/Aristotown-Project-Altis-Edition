...
# This is a list of the effects that were used in the tutorial
# They are in phase_3.5, not phase_5
TutorialParticleEffects = ('gearExplosionBig.ptf',
                           'gearExplosionSmall.ptf',
                           'gearExplosion.ptf')

ParticleNames = ('audit-div',
                 'audit-five',
                 'audit-four',
                 'audit-minus',
                 'audit-mult',
...
def cleanupSystem(effect: ParticleEffect, duration: float = 3.0, instant: bool = False):
    """
    ...
    if not instant:
        def killEffect(_):
            if effect:
                effect.cleanup()
        taskMgr.doMethodLater(duration, killEffect, f'{effect.getName()}-cleanup')
    else:
        effect.cleanup()
