...
class ToontownLoader(nLoader.Loader):
    ...
    def loadModel(self, *args, **kw):
        ...
        if ret and 'inventory_icons' in modelName:
            ContentPackCompatibility.applyClashGagIconCompatibility(ret)

        if ret and 'status_effects' in modelName:
            ContentPackCompatibility.applyStatusEffectsCompatibility(ret)
    ...
    def loadFont(self, *args, **kw):
        if args:
            newArgs = list(args)
            newArgs[0] = ContentPackCompatibility.resolveFontPath(
                args[0]
            )
            args = tuple(newArgs)
