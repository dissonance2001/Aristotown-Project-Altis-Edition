...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def d_setGumballBoosters(self, boosters):
        self.sendUpdate('setGumballBoosters', [six.moves.cPickle.dumps(boosters, 1)])
    ...
    def d_setGumballBounties(self, bounties):
        self.sendUpdate('setGumballBounties', [six.moves.cPickle.dumps(sorted(bounties.items()), 1)])
...
def dump_doId2do():
    ...
    objSizes = {}
    for object in six.itervalues(simbase.air.doId2do):
    ...
    sorted_objSizes = sorted(six.iteritems(objSizes), key=operator.itemgetter(1))
