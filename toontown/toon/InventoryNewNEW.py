...
class InventoryNewNEW(InventoryBase.InventoryBase, DirectFrame):
    ...
    def load(self):
        ...
        for x in range(len(self.trackRows)):
            self.hideTrack(x)
        ...
        for x in range(len(self.trackTabs)):
    ...
    def doTab(self, index):
        ...
        for track in range(len(self.trackRows)):
    ...
    def showDetail(self, track, level, event = None):
        ...
        if track == DROP_TRACK and organicBonus:
            accString += '(+10%)'
        if (track == TRAP_TRACK and organicBonus) or track == ZAP_TRACK:
            ...
            self.detailDataLabel.configure(text=TTLocalizer.InventoryDetailData % {'accuracy': accString,
                                                                                   ...
                                                                                   ToontownBattleGlobals.AvSoakRounds[
                                                                                       level],
                                                                                   -ToontownBattleGlobals.AvSoakDefReduction),
        ...
        if self.itemIsCredit(track, level):
            ...
            if self.toon and hasattr(self.toon, 'getGumballBoosters'):
                from toontown.gumball import GumballGlobals
                if hasattr(self.toon, 'getEffectiveBoosters'):
                    boosters = self.toon.getEffectiveBoosters()
                else:
                    boosters = self.toon.getGumballBoosters()
                mult += GumballGlobals.getGagExperienceMultiplier(boosters, track)
