...
class FilterManager(DirectObject):
    ...
    def renderSceneInto(self, depthtex=None, colortex=None, auxtex=None, auxbits=0, textures=None, id=0):
        ...
        * All clears are disabled on the original display region.
        ...
        colortex.setFormat(Texture.F_rgba)
        # colortex.setComponentType()
        texgroup = (depthtex, colortex, auxtex, None)

        # Choose the size of the offscreen buffer.

        (winx, winy) = self.getScaledSize(1, 1, 1)
        buffer = self.createBuffer("filter-base%d" % id, winx, winy, texgroup)

        if buffer is None:
            return None

        cm = CardMaker("filter-base-quad%d" % id)
        ...
        quadcamnode = Camera("filter-quad-cam%d" % id)
    ...
    def renderQuadInto(self, mul=1, div=1, align=1, depthtex=None, colortex=None, auxtex0=None, auxtex1=None, id=0):
        ...
        buffer = self.createBuffer("filter-stage%d" % id, winx, winy, texgroup, depthbits)

        if buffer is None:
            return None

        cm = CardMaker("filter-stage-quad%d" % id)
        cm.setFrameFullscreenQuad()
        quad = NodePath(cm.generate())
        quad.setDepthTest(0)
        quad.setDepthWrite(0)
