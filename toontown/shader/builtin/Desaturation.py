...
# Desaturation with hue bandpass (e.g. keep only red things)
#
...
class Desaturation(Filter):
    """Desaturation (monochrome) filter with tinting and adjustable strength.
    ...
    def __init__(self, **kwargs):
        super(Desaturation, self).__init__(**kwargs)

    def onReset(self):
        super(Desaturation, self).onReset()  # reset inherited properties
    ...
    def onSynthesizeCompositor(self):
        ...
        if self.mode == "basic":
            ...
            comment = "// Desaturation filter (compile-time mode: basic)\n"
            ...
            comment = "// Desaturation filter (compile-time mode: bandpass)\n"
        else:
    ...
    def mode(self):
        ...
        String, one of:
          ...
          "bandpass" = Desaturation with hue bandpass (e.g. keep only red things).
