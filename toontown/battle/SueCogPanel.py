...
class SueCogPanel(StateData.StateData):
    ...
    def enter(self, numAvatars, localNum = None, luredIndices = None, trappedIndices = None, track = None, sueCosts = None):
        if not self.loaded:
            self.load()
        self.frame.show()
        invalidTargets = []
    ...
    def __placeButtons(self, numAvatars, invalidTargets, localNum, sueCosts):
        ...
        for i in range(7):
            if numAvatars > i and i not in invalidTargets and i != localNum:
                ...
                if sueCosts[i] <= localAvatar.getCeaseAndDesists():
        ...
        if cansue:
            self.textFrame['text'] = TTLocalizer.SueCogTitle % localAvatar.getCeaseAndDesists()
        else:
            self.textFrame['text'] = TTLocalizer.SueCogLowTitle % localAvatar.getCeaseAndDesists()
        confused = False
