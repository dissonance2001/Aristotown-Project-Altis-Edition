...
class BattleCalculatorAI:
        ...
        for currTarget in xrange(len(targetList)):
            ...
            if atkTrack == SUE:
                        ...
                        if not self.__combatantDead(targetId, toon=toonTarget):
                           ...
                           # for s in self.battle.activeSuits:
                            ...
                            if self.suitHasCondition(targetId, 'soakImmune') and self.suitHasCondition(targetId, 'soaked'):
                                attackDamage *= 0.4
                            if theSuit.dna.name == 'clubpres' and theSuit.getActualLevel() == 25:
                                self.setSuitCondition(theSuit.doId, 'shivering', self.getSuitConditionModifier(theSuit.doId, 'shivering') + 1, 1, 'setBoth')
                            if theSuit.dna.name == 'clubpres' and theSuit.getActualLevel() == 23:
                                self.setSuitCondition(theSuit.doId, 'rpm', self.getSuitConditionModifier(theSuit.doId, 'rpm') + 1, 1, 'setBoth')
                            if theSuit.dna.name == 'sgoat' and not self.suitHasCondition(targetId, 'enraged'):
                ...
                if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[TOON_TRACK_COL] == PETSOS:
                    ...
                    attackDamage = 0
                    if suit:
                        if not suit.getManager() and not suit.getGovernaught() and not suit.currHP > (suit.maxHP * 1.5) and not self.suitHasCondition(targetId, 'insured') and not self.suitHasCondition(targetId, 'contracted'):
                            costToSue = math.ceil(suit.getActualLevel() / 4)
                            abilityToSue = toon.getCeaseAndDesists()
                            toon.removeCeaseAndDesists(costToSue)
                            if costToSue > abilityToSue:
                    ...
                    if attackDamage > 0:
                        ...
                        if organicBonus:
                            if self.suitHasCondition(targetId, 'soaked'):
                                self.setSuitCondition(target.doId, 'zapped', self.getSuitConditionModifier(suit.doId, 'zapped') + math.ceil(attackDamage / 4), 2, 'setBoth')
                ...
                if currTarget.getImmuneStatus() == 1:
                    ...
                    if track == SQUIRT and kbbonus == 0 and hpbonus == 0:
                        ...
                        if self.suitHasCondition(currTarget.doId, 'enraged') and self.suitHasCondition(currTarget.doId, 'desperation'):
                            attackDamage *= 1
                        if self.suitHasCondition(currTarget.doId, 'soakImmune') and self.suitHasCondition(currTarget.doId, 'soaked'):
                            attackDamage *= 0.4
                        if self.toonHasCondition(toonId, 'groupDamageDown'):
                            attackDamage *= 0.5
                        if self.toonHasCondition(toonId, 'noDamage'):
                        ...
                        if suitIndex - 1 >= 0:
                            ...
                            if organicBonus:
                                if not self.suitHasCondition(target2.doId, 'immune'):
                                    target2.setHP(target2.getHP() - (damageDone * .75))
                                ...
                                if not self.suitHasCondition(target2.doId, 'immune'):
                                    target2.setHP(target2.getHP() - (damageDone / 3))
                                    if target2.dna.name == 'supervis' and target2.getActualLevel() == 20:
                                        self.levels += atkLevel
                                    if target2.dna.name == 'clubpres' and target2.getActualLevel() == 25:
                                        self.setSuitCondition(target2.doId, 'shivering', self.getSuitConditionModifier(target2.doId, 'shivering') + 1, 1, 'setBoth')
                        ...
                        if suitIndex + 1 < len(activeSuits):
                            ...
                            if organicBonus:
                                if not self.suitHasCondition(target3.doId, 'immune'):
                                    target3.setHP(target3.getHP() - (damageDone * .75))
                                ...
                                if not self.suitHasCondition(target3.doId, 'immune'):
                                    target3.setHP(target3.getHP() - (damageDone / 3))
                                    if target3.dna.name == 'supervis' and target3.getActualLevel() == 20:
                                        self.levels += atkLevel
                                    if target3.dna.name == 'clubpres' and target3.getActualLevel() == 25:
                                        self.setSuitCondition(target3.doId, 'shivering', self.getSuitConditionModifier(target3.doId, 'shivering') + 1, 1, 'setBoth')
                ...
                if len(currTgt[currAtkType]) > 1 or not hp and len(currTgt[currAtkType]) > 0:
                    ...
                    if hp:
                        ...
                        if self.suitHasCondition(suit, 'kbImmune'):
                            ...
                            self.setSuitCondition(suit, 'overseerCombo', 1, 1, 'none')
                            ...
                            self.setSuitCondition(suit, 'attorneyKB', 1, 1, 'none')
                        ...
                        if self.suitHasCondition(suit, 'overseer'):
                            ...
                            self.comboDamage += math.ceil(totalDmgs * (lureKBValue / 2))
                            self.setSuitCondition(suit, 'overseerKB', 1, 1, 'none')
                        else:
    ...
    def __calcSuitAtkHp(self, attack):
        ...
        for currTarget in xrange(len(targetList)):
            ...
            if result > 0 and self.__suitCanAttack(theSuit.doId):
                ...
                if self.suitHasCondition(theSuit.doId, 'syphon') and self.__suitAtkHit(attack[SUIT_ID_COL], attack[SUIT_ATK_COL]):
                   # self.syphonHP *= 0
    ...
    def __calcSuitAtkHpALT(self, attack):
        ...
        for currTarget in xrange(len(targetList)):
            ...
            if atkType['name'] == 'SynergyFees':
                ...
                if self.toonHasCondition(toon.doId, 'markedforsanction2'):
                ...
                if self.toonHasCondition(toon.doId, 'bound'):
                    ...
                    if self.toonHasCondition(toon.doId, 'bound') and self.getToonConditionTurns(toon.doId,
                        ...
                        self.setToonCondition(toon.doId, 'markedforsnap', 1, 5, 'setBoth')
                        for s in self.battle.suits:
                            ...
                            if s.dna.name == 'lgator':
                                ...
                                currentBossHealth = s.currHP
                                if currentBossHealth >= 1:
                                    self.setSuitCondition(suit.doId, 'snappedcalculator2', 1, 10, 'setBoth')
                else:
                    result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                ...
                if self.toonHasCondition(toon.doId, 'noDamage'):
                    ...
                    self.setToonCondition(toon.doId, 'markedforsanction2', 1, 5, 'setBoth')
                    for s in self.battle.suits:
                        if s.dna.name == 'stenog':
                            suit = s
                            currentBossHealth = s.currHP
                            if currentBossHealth >= 1:
                                self.setSuitCondition(suit.doId, 'sanctioncalculator3', 1, 10, 'setBoth')
                else:
                    result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                ...
                self.setSuitCondition(theSuit.doId, 'damageReduction', 1, 99, 'setBoth')
                self.setSuitCondition(theSuit.doId, 'alreadyAbsorbing', 1, 10, 'setBoth')
                for s in self.battle.suits:
                ...
                result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                self.setSuitCondition(theSuit.doId, 'dropImmune', 1, 99, 'setBoth')
                self.setSuitCondition(theSuit.doId, 'powerhouseRotation', 0, 0, 'setBoth')
                ...
                result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                self.setSuitCondition(theSuit.doId, 'zapImmune', 1, 99, 'setBoth')
                self.setSuitCondition(theSuit.doId, 'powerhouseRotation', 0, 0, 'setBoth')
                ...
                if not self.suitHasCondition(theSuit.doId, 'vulnerablevideographer'):
                    self.setSuitCondition(theSuit.doId, 'vulnerablevideographer', 11, 99, 'setBoth')
                else:
                    self.setSuitCondition(theSuit.doId, 'vulnerablevideographer', (self.getSuitConditionModifier(theSuit.doId, 'vulnerablevideographer') + 11),
                                          99, 'setBoth')
                ...
                if currentBossHealth >= 1:
                    for suit in self.battle.activeSuits:
                        if self.suitHasCondition(suit.doId, 'lured'):
                            self.setSuitCondition(suit.doId, 'lured', 0, 0, 'setBoth')
                            self.setSuitCondition(suit.doId, 'unlureSuit', 0, 0, 'setBoth')
                        for s in self.currentlyLuredSuits.keys():
                ...
                if self.toonHasCondition(toon.doId, 'markToon'):
                    result = 30
                    attack[SUIT_HP_COL][targetIndex] = result
                    self.setToonCondition(toon.doId, 'markToon', 1, 1, 'setBoth')
                ...
                self.setSuitCondition(theSuit.doId, 'levelshielding', 1, 3, 'setBoth')
            elif atkType['name'] == 'WiretapperBrokenConnection':
                result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                self.setSuitCondition(theSuit.doId, 'vulnerable', 1, 3, 'setBoth')
                self.setSuitCondition(theSuit.doId, 'immune', 0, 0, 'setBoth')
                self.setSuitCondition(theSuit.doId, 'levelshielding', 0, 0, 'setBoth')
                ...
                self.syphonHP += math.ceil(result * 2)
                ...
                if self.toonHasCondition(toon.doId, 'busted'):
                    if self.toonHasCondition(toon.doId, 'busted') and self.getToonConditionTurns(toon.doId,
                                                                                                'busted') <= 1:
                        self.setToonCondition(toon.doId, 'markedforsanction', 1, 5, 'setBoth')
                    result = 25
                else:
                ...
                result = 35
                attack[SUIT_HP_COL][targetIndex] = result
                self.setToonCondition(toon.doId, 'busted', 1, 3, 'setBoth')
                ...
                if self.toonHasCondition(toon.doId, 'markedforsanction'):
                    self.setToonCondition(toon.doId, 'markedforsanction', 0, 0, 'setBoth')
                    currentBossHealth = -1
                    for s in self.battle.suits:
                        if s.dna.name == 'safesupervis':
                            currentBossHealth = s.currHP
                ...
                result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                self.costsMultiplier += 7
                self.costsCalculatorMultiplier += 7
                ...
                for t in self.battle.activeToons:
                    self.setToonCondition(toon.doId, 'usedZap', 0, 0, 'setBoth')
                    self.setToonCondition(toon.doId, 'usedDrop', 0, 0, 'setBoth')
                    self.setToonCondition(toon.doId, 'usedHeal', 0, 0, 'setBoth')
                    self.setToonCondition(toon.doId, 'usedLure', 0, 0, 'setBoth')
                    self.setToonCondition(toon.doId, 'usedSound', 0, 0, 'setBoth')
                    self.setToonCondition(toon.doId, 'usedSquirt', 0, 0, 'setBoth')
                    self.setToonCondition(toon.doId, 'usedTrap', 0, 0, 'setBoth')
                    self.setToonCondition(toon.doId, 'usedThrow', 0, 0, 'setBoth')
                ...
                for s in self.battle.suits:
                    ...
                    if s.dna.name == 'racket':
                        ...
                        currentBossHealth = s.currHP
                        if currentBossHealth >= 1:
                            self.setSuitCondition(suit.doId, 'extortioncalculator', 1, 10, 'setBoth')
                ...
                if currentBossHealth >= 1:
                    if self.getToonConditionModifier(toonId, 'snapped') > 1.75:
                        ...
                        self.setToonCondition(toon.doId, 'snapped', 1.75, 3, 'setBoth')
                    self.setToonCondition(toon.doId, 'allGagBoost', 75, 3, 'setBoth')
                    self.setToonCondition(toon.doId, 'lureBoost', 75, 3, 'setBoth')
                else:
                    if self.getToonConditionModifier(toonId, 'snapped') > 1.5:
                ...
                if self.toonHasCondition(toon.doId, 'noFires'):
                    result = 25
                else:
                    result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                ...
                result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                self.setToonCondition(toon.doId, 'partnered', 1, 99, 'setBoth')
                self.setSuitCondition(theSuit.doId, 'partnered', 1, 99, 'setBoth')
                self.setSuitCondition(theSuit.doId, 'alreadyContracted', 1, 99, 'setBoth')
            elif atkType['name'] == 'ForemanBurning':
                result = 25
                attack[SUIT_HP_COL][targetIndex] = result
                self.setToonCondition(toon.doId, 'smoked', 1, 3, 'setBoth')
                ...
                attack[SUIT_HP_COL][targetIndex] = result
                for suit in self.battle.activeSuits:
                    if suit.getHP() < suit.maxHP and not suit.dna.name == 'whistleb':
                        suit.setDamageMultiplier(suit.getDamageMultiplier() * 1.05)
                        self.setSuitCondition(suit.doId, 'lureResist', 1, 99, 'setBoth')
            elif atkType['name'] == 'AttorneyOverseer':
                result = self.comboDamage + self.knockbackDamage
                attack[SUIT_HP_COL][targetIndex] = result
                theSuit.setHP(theSuit.currHP + (self.comboDamage + self.knockbackDamage))
                ...
                result = self.comboDamage
                attack[SUIT_HP_COL][targetIndex] = result
                theSuit.setHP(theSuit.currHP + self.comboDamage)
                self.setSuitCondition(theSuit.doId, 'attorneyKB', 0, 0, 'setBoth')
                self.comboDamage = 0
            elif atkType['name'] == 'PresidentHighStakes':
                randomizer = random.randint(-99, 99)
                ...
                result = 0
                attack[SUIT_HP_COL][targetIndex] = result
                self.setSuitCondition(theSuit.doId, 'selfTarget', 1, 1, 'setBoth')
                if len(self.battle.activeSuits) > 1:
                    targetSuit = self.battle.activeSuits[0]
                    if targetSuit.getHP() > 0 and not targetSuit.getHP() > targetSuit.maxHP and self.suitHasCondition(
                            targetSuit.doId, 'unlureSuit') and not self.suitHasCondition(
                            targetSuit.doId, 'selfTarget'):
                        self.setSuitCondition(theSuit.doId, 'target7', 1, 10, 'setBoth')
                    else:
                        if len(self.battle.activeSuits) > 1:
                            targetSuit = self.battle.activeSuits[1]
                            if targetSuit.getHP() > 0 and not targetSuit.getHP() > targetSuit.maxHP and self.suitHasCondition(
                ...
                if not self.suitHasCondition(theSuit.doId, 'alreadyMulligan'):
                    ...
                    if not self.suitHasCondition(theSuit.doId, 'mulligan'):
                        self.setSuitCondition(theSuit.doId, 'mulligan', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan2', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan2') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan3', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan3') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan4', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan4') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan5', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan5') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan6', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan6') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan7', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan7') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan8', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan8') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan9', 1, 99, 'setBoth')
                        ...
                        theSuit.doId,
                        'mulligan9') < 99:
                        self.setSuitCondition(theSuit.doId, 'mulligan10', 1, 99, 'setBoth')
                ...
                attack[SUIT_HP_COL][targetIndex] = result
                for t in self.battle.activeToons:
                    self.setToonCondition(t, 'disable4s', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disable5s', 1, 3, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disable6s', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disable7s', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disable8s', 0, 0, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disable5s', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disable6s', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disable7s', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disable8s', 0, 0, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disable6s', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disable7s', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disable8s', 1, 3, 'setBoth')
                ...
                attack[SUIT_HP_COL][targetIndex] = result
                for t in self.battle.activeToons:
                    self.setToonCondition(t, 'disableToonUp', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disableTrap', 1, 3, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disableLure', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disableThrow', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSquirt', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableZap', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSound', 0, 0, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disableTrap', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableLure', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableThrow', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disableSquirt', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableZap', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSound', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableDrop', 0, 0, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disableLure', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableThrow', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSquirt', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disableZap', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSound', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableDrop', 0, 0, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disableThrow', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSquirt', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableZap', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disableSound', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableDrop', 0, 0, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disableSquirt', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableZap', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSound', 1, 3, 'setBoth')
                    self.setToonCondition(t, 'disableDrop', 0, 0, 'setBoth')
                ...
                for t in self.battle.activeToons:
                    ...
                    self.setToonCondition(t, 'disableZap', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableSound', 0, 0, 'setBoth')
                    self.setToonCondition(t, 'disableDrop', 1, 3, 'setBoth')
            ...
            try:
                if atkType['name'] == 'CalculatingFees':
                    ...
                    result = self.damageHP
                    toon.setHp(toon.hp + math.ceil(result))
                    attack[SUIT_HP_COL][targetIndex] = result
                    if theSuit.currHP > 0:
                        theSuit.setHP(theSuit.currHP - self.damageHP)
                        self.damageHP *= 0
                elif atkType['name'] == 'AttorneyOverseer':
                    attack[SUIT_HP_COL][targetIndex] = math.ceil(result)
                    toon.setHp(toon.hp + math.ceil(result))
    ...
    def __getGenericSuitAttack(self, suitId):
        ...
        if theSuit.dna.name == 'liquid':
            ...
            currentBossHealth = -1
            for s in self.battle.suits:
                if s.dna.name == 'rkeeper' or s.dna.name == 'cbutcher' or s.dna.name == 'cdirector':
                    currentBossHealth = s.currHP
            if currentBossHealth == -1 and not self.suitHasCondition(theSuit.doId, 'desperation'):
                self.setSuitCondition(theSuit.doId, 'desperationcalculator', 1, 100, 'setBoth')
        if theSuit.dna.name == 'rkeeper':
            ...
            currentBossHealth = -1
            for s in self.battle.suits:
                if s.dna.name == 'liquid' or s.dna.name == 'cbutcher' or s.dna.name == 'cdirector':
                    currentBossHealth = s.currHP
            if currentBossHealth == -1 and not self.suitHasCondition(theSuit.doId, 'desperation'):
                self.setSuitCondition(theSuit.doId, 'desperationcalculator', 1, 100, 'setBoth')
        ...
        if theSuit.dna.name == 'cdirector':
            ...
            currentBossHealth = -1
            for s in self.battle.suits:
                if s.dna.name == 'liquid' or s.dna.name == 'rkeeper' or s.dna.name == 'cbutcher':
                    currentBossHealth = s.currHP
            if currentBossHealth == -1 and not self.suitHasCondition(theSuit.doId, 'desperation'):
                self.setSuitCondition(theSuit.doId, 'desperationcalculator', 1, 100, 'setBoth')
        if theSuit.dna.name == 'videog':
        ...
        for i in xrange(len(self.battle.activeSuits)): # Cheat Calculators
            ...
            if self.battle.activeSuits[i].dna.name == 'lgator':
                ...
                if self.battle.activeSuits[i].currHP <= 4100:
                    if (x + 1) % 4 == 0 and self.suitHasCondition(suitId, 'bashcalculator') and not self.deadSuits > 0:
                        self.setSuitCondition(suitId, 'bashcalculator', 0, 0, 'setBoth')
                        self.setSuitCondition(suitId, 'bellowcalculator', 1, 10, 'setBoth')
                    elif (x + 1) % 4 == 0 and not self.suitHasCondition(suitId, 'bashcalculator'):
                        self.setSuitCondition(suitId, 'bashcalculator', 1, 10, 'setBoth')
                if self.battle.activeSuits[i].currHP <= 3600:
                    if (x + 2) % 4 == 0 and self.suitHasCondition(suitId, 'bashcalculator') and not self.deadSuits > 0:
                        self.setSuitCondition(suitId, 'bashcalculator', 0, 0, 'setBoth')
                        self.setSuitCondition(suitId, 'bellowcalculator', 1, 10, 'setBoth')
                    elif (x + 2) % 4 == 0 and not self.suitHasCondition(suitId, 'bashcalculator'):
                        self.setSuitCondition(suitId, 'bashcalculator', 1, 10, 'setBoth')
                if self.battle.activeSuits[i].currHP <= 3100:
                    if (x + 3) % 4 == 0 and self.suitHasCondition(suitId, 'bashcalculator') and not self.deadSuits > 0:
                        self.setSuitCondition(suitId, 'bashcalculator', 0, 0, 'setBoth')
                        self.setSuitCondition(suitId, 'bellowcalculator', 1, 10, 'setBoth')
                ...
                if self.battle.activeSuits[i].currHP <= 1600:
                    if (x + 2) % 3 == 0 and self.suitHasCondition(suitId, 'bashcalculator') and not self.deadSuits > 0:
                        self.setSuitCondition(suitId, 'bashcalculator', 0, 0, 'setBoth')
                        self.setSuitCondition(suitId, 'bellowcalculator', 1, 10, 'setBoth')
                ...
                if self.battle.activeSuits[i].currHP <= 600:
                    if (x + 1) % 2 == 0 and self.suitHasCondition(suitId, 'bashcalculator') and not self.deadSuits > 0:
                        self.setSuitCondition(suitId, 'bashcalculator', 0, 0, 'setBoth')
                        self.setSuitCondition(suitId, 'bellowcalculator', 1, 10, 'setBoth')
            ...
            if self.battle.activeSuits[i].dna.name == 'radiog': #radiographer
                if x % 5 == 0 and self.suitHasCondition(suitId, 'dancesession'):
            ...
            if self.battle.activeSuits[i].dna.name == 'cdirector':  # recordkeeper
                if (x + 3) % 5 == 0:
                    self.setSuitCondition(suitId, 'costscalculator', 1, 10, 'setBoth')
    ...
    def __calculateSuitAttacksLawbotLitigation(self):
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.suitHasCondition(suitId, 'deadcase') and not self.suitHasCondition(suitId, 'healfinished') and self.battle.activeSuits[i].dna.name in SuitBattleGlobals.SpecialCogDict and not self.battle.activeSuits[i].dna.name == 'caseman':
                ...
                self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'stenog':
                if self.suitHasCondition(suitId, 'sanctioncalculator3') and not self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'lgator':  # arbitrator
                if self.suitHasCondition(suitId, 'throwbookcalculator') and self.suitHasCondition(suitId, 'desperation') and not len(self.battle.activeSuits) > 1 and self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'throwbookcalculator') and not self.suitHasCondition(suitId, 'desperation') and not len(self.battle.activeSuits) > 2 and self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'caseman':
                ...
                if self.suitHasCondition(suitId, 'bindingscalculator2') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                     'name': 'CaseManagerLegalBindings2', # 2 Target Legal Bindings
                     'animName': 'throw-object',
                     'hp': 0,
                     'acc': 100,
                     'freq': 0,
    ...
    def __calculateSuitAttacksBoardbotLitigation(self):
        for i in xrange(len(self.battle.activeSuits)):
            suitId = self.battle.activeSuits[i].doId
            if self.battle.activeSuits[i].dna.name == 'rkeeper':
                if self.suitHasCondition(suitId, 'recordkeeperHit') and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                ...
                if self.suitHasCondition(suitId, 'bannedGagUsed') and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                            ...
                                            'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'papertrailcalculator') and not self.__suitCanAttack(suitId) and self.battle.activeSuits[
                    i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'papertrailcalculator') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            suitId = self.battle.activeSuits[i].doId
            if self.battle.activeSuits[i].dna.name == 'rkeeper':
                if self.suitHasCondition(suitId, 'costscalculator') and not self.__suitCanAttack(suitId) and self.battle.activeSuits[
                    i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'costscalculator') and self.__suitCanAttack(suitId):
                ...
                if self.suitHasCondition(suitId, 'redlinedcalculator') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                            'name': 'RecordkeeperRedlinedClause',  # Audit
                                                            'animName': 'sanction',
                                                            'hp': 0,
                                                            'acc': 100,
                                                            'freq': 0,
    ...
    def __calculateSuitAttacksBossbotLitigation(self):
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'ambass':  # ambassador
                ...
                if self.suitHasCondition(suitId, 'headrollercalculator') and not len(self.battle.activeSuits) > 1 and self.suitHasCondition(suitId, 'phase3') and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'headrollercalculator') and len(self.battle.activeSuits) > 1 and self.suitHasCondition(suitId, 'phase3') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'phouse': #powerhouse
                ...
                if self.suitHasCondition(suitId, 'mulligansnipe') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                     'name': 'PowerhouseSnipeMulligan', # Burn Retaliation For Mulligan Attacks
                     'animName': 'magic3-alt',
                     'hp': 0,
                     'acc': 100,
                     'freq': 0,
            ...
            if self.battle.activeSuits[i].dna.name == 'bkeeper':  # bookkeeper
                ...
                if self.suitHasCondition(suitId, 'bookkeepingcalculator') and self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
                    ...
                    self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'wtapper':  # wiretapper
                if self.suitHasCondition(suitId, 'collectcallcalculator') and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                ...
                if self.suitHasCondition(suitId, 'voicemailcalculator') and not self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'voicemailcalculator') and not self.suitHasCondition(suitId, 'brokenconnection') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                ...
                if self.suitHasCondition(suitId, 'brokenconnectioncalculator') and self.__suitCanAttack(suitId):
                    ...
                    self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'ambass': #ambassador
                if self.battle.activeSuits[i].currHP <= 2495 and not self.suitHasCondition(suitId, 'phase3') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'phouse': #powerhouse
                ...
                if self.suitHasCondition(suitId, 'vulnerablesnipe') and self.__suitCanAttack(suitId):
                    ...
                    self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'phouse': #powerhouse
                if self.suitHasCondition(suitId, 'burncalculator') and not self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'burncalculator') and self.__suitCanAttack(suitId):
                    if self.battle.activeSuits[i].currHP <= 2000:
                ...
                if self.getSuitConditionModifier(suitId, 'powerhouseRotation') >= 95 and not self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
                    ...
                    self.battle.suitAttacks.append(attack)
                if self.getSuitConditionModifier(suitId, 'powerhouseRotation') >= 95 and self.__suitCanAttack(suitId):
                    if self.suitHasCondition(suitId, 'alreadyAbsorbing'):
                        attack = self.__getCheatAttack(suitId, random.choice([
                                                                              ...
                                                                              {'suitName': self.battle.activeSuits[i].dna.name,
                                                                               ...
                                                                               'group': SuitBattleGlobals.ATK_TGT_SINGLE}]))
                        self.battle.suitAttacks.append(attack)
                    elif self.suitHasCondition(suitId, 'alreadyDropImmune'):
                        attack = self.__getCheatAttack(suitId, random.choice([
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             'name': 'PowerhouseSoakImmune',
                             'animName': 'nothing',
                            ...
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             ...
                             'group': SuitBattleGlobals.ATK_TGT_SINGLE}]))
                        self.battle.suitAttacks.append(attack)
                    elif self.suitHasCondition(suitId, 'alreadyZapImmune'):
                        attack = self.__getCheatAttack(suitId, random.choice([
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             'name': 'PowerhouseSoakImmune',
                             'animName': 'nothing',
                            ...
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             ...
                             'group': SuitBattleGlobals.ATK_TGT_SINGLE}]))
                        self.battle.suitAttacks.append(attack)
                    elif self.suitHasCondition(suitId, 'alreadySyphon'):
                        attack = self.__getCheatAttack(suitId, random.choice([
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             'name': 'PowerhouseSoakImmune',
                             'animName': 'nothing',
                            ...
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             ...
                             'group': SuitBattleGlobals.ATK_TGT_SINGLE}]))
                        self.battle.suitAttacks.append(attack)
                    elif self.suitHasCondition(suitId, 'alreadyLureImmune'):
                        attack = self.__getCheatAttack(suitId, random.choice([
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             'name': 'PowerhouseSoakImmune',
                             'animName': 'nothing',
                            ...
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             ...
                             'group': SuitBattleGlobals.ATK_TGT_SINGLE}]))
                        self.battle.suitAttacks.append(attack)
                    elif self.suitHasCondition(suitId, 'alreadySoakImmune'):
                        attack = self.__getCheatAttack(suitId, random.choice([
                            {'suitName': self.battle.activeSuits[i].dna.name,
                             'name': 'PowerhouseLureImmune',
                             'animName': 'nothing',
                        ...
                        attack = self.__getCheatAttack(suitId, random.choice([{'suitName': self.battle.activeSuits[i].dna.name,
                                                                              ...
                                                                              {'suitName': self.battle.activeSuits[i].dna.name,
                                                                               ...
                                                                               'group': SuitBattleGlobals.ATK_TGT_SINGLE}]))
                        self.battle.suitAttacks.append(attack)
                if self.TurnsElapsed % 1 == 0 and self.suitHasCondition(suitId, 'soaked') and not self.__suitCanAttack(suitId) and \
                        self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.TurnsElapsed % 1 == 0 and self.suitHasCondition(suitId, 'soaked') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                            ...
                                            'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                if self.TurnsElapsed % 1 == 0 and self.suitHasCondition(suitId, 'zapped') and not self.__suitCanAttack(suitId) and \
                        self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.TurnsElapsed % 1 == 0 and self.suitHasCondition(suitId, 'zapped') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
            ...
            if self.battle.activeSuits[i].dna.name == 'wtapper':
                ...
                if self.TurnsElapsed % 1 == 0 and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                     ...
                     'group': SuitBattleGlobals.ATK_TGT_GROUP})
                    self.battle.suitAttacks.append(attack)
                if self.TurnsElapsed % 1 == 0 and self.__suitCanAttack(suitId) and (self.suitHasCondition(suitId, 'bantracks') or self.suitHasCondition(suitId, 'desperation')):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                          'name': 'BanLevel%s' % random.randint(4, 8),
                     'animName': 'cease',
                     'hp': 0,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            suitId = self.battle.activeSuits[i].doId
            if self.battle.activeSuits[i].dna.name == 'phouse': #powerhouse
                if self.suitHasCondition(suitId, 'burncalculator2') and not self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'burncalculator2') and self.__suitCanAttack(suitId):
                    if self.battle.activeSuits[i].currHP <= 2000:
                        attack = self.__getCheatAttack(suitId, {'suitName': 'phouse',
                                                'name': 'PowerhouseSnipeSoaked',
                                                'animName': 'magic3-alt',
    ...
    def __calculateSuitAttacksSellbotLitigation(self):
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'radiog':
                if self.suitHasCondition(suitId, 'marked') and not self.__suitCanAttack(suitId) and \
                        self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'marked') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                     ...
                     'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'zapped') and not self.__suitCanAttack(suitId) and \
                        self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'zapped') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                     'name': 'RadiographerHotTake', # Hot Take
                     'animName': 'throw-object',
                     'hp': 0,
                     ...
                     'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'soaked') and not self.__suitCanAttack(suitId) and \
                        self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'soaked') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                     ...
                     'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'dazed') and not self.__suitCanAttack(suitId) and \
                        self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'dazed') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                     'name': 'RadiographerHotTake', # Hot Take
                     'animName': 'throw-object',
                     'hp': 0,
                     ...
                     'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                if self.__suitCanAttack(suitId) and self.damageHP > 0:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'safesupervis':
                if self.suitHasCondition(suitId, 'soaked') and self.__suitCanAttack(suitId):
                    ...
                    if self.battle.activeSuits[i].currHP <= 1500:
                        attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                                    ...
                                                                    'group': SuitBattleGlobals.ATK_TGT_GROUP})
                        self.battle.suitAttacks.append(attack)
                    elif self.battle.activeSuits[i].currHP <= 2500:
                        attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                                    'name': 'RacketeerPeckingOrderRetaliationSoak',
                                                                    'animName': 'nothing',
                                                                    'hp': 0,
                                                                    ...
                                                                    'group': SuitBattleGlobals.ATK_TGT_TRIPLE})
                        self.battle.suitAttacks.append(attack)
                    elif self.battle.activeSuits[i].currHP <= 3500:
                        attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                                    'name': 'RacketeerPeckingOrderRetaliationSoak',
                                                                    'animName': 'nothing',
                                                                    'hp': 0,
    ...
    def __calculateSuitAttacksFaceTheFamily(self):
        ...
        for i in xrange(len(self.battle.activeSuits)):
            suitId = self.battle.activeSuits[i].doId
            if self.battle.activeSuits[i].dna.name == 'foreman' and self.battle.activeSuits[i].getActualLevel() == 21:  # Burning Foreman Smoked Damage
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
            ...
            if self.battle.activeSuits[i].dna.name == 'clubpres' and self.battle.activeSuits[i].getActualLevel() == 21:  # Mulligan President
                ...
                if self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                                ...
                                                                'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                #if self.suitHasCondition(suitId, 'mulligan') and self.battle.activeSuits[i].currHP > 0:
                   # attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                         #                   'name': 'PresidentMulligan',
                                        #                    'animName': 'golf-club-swing',
                                         #                   'hp': 0,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'foreman' and self.battle.activeSuits[i].getActualLevel() == 23:  # Contractor Foreman
                ...
                if self.battle.activeSuits[i].currHP <= 0:
                    for t in self.battle.activeToons:
                        self.setToonCondition(t, 'partnered', 0, 0, 'setBoth')
            ...
            if self.battle.activeSuits[i].dna.name == 'foreman' and self.battle.activeSuits[i].getActualLevel() == 20:  # Sleepy Foreman
                if not self.suitHasCondition(suitId, 'sounded') and self.suitHasCondition(suitId, 'unlureSuit') and self.getSuitConditionTurns(suitId, 'sleepy') == 1:
                    attack = self.__getLureRemoval(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.battle.activeSuits[i].currHP > 0 and self.getSuitConditionTurns(suitId, 'sleepy') == 1:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                            ...
                                                            'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'foreman' and self.battle.activeSuits[i].getActualLevel() == 22:
                if not self.suitHasCondition(suitId, 'sounded') and self.suitHasCondition(suitId, 'unlureSuit') and self.getSuitConditionTurns(suitId, 'explosive') == 1:
                    attack = self.__getLureRemoval(suitId)
                    self.battle.suitAttacks.append(attack)
                # Explosive Foreman
                if self.battle.activeSuits[i].currHP > 0 and self.getSuitConditionTurns(suitId, 'explosive') == 1:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'ovt':  # Mint Supervisor Life Insurance
                if not self.suitHasCondition(suitId, 'sounded') and self.suitHasCondition(suitId, 'unlureSuit') and (self.battle.activeSuits[i].currHP < self.battle.activeSuits[i].maxHP):
                    attack = self.__getLureRemoval(suitId)
                    self.battle.suitAttacks.append(attack)
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                ...
                self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'ovt':  # Mint Supervisor Life Insurance
                if (len(self.battle.activeSuits) == 1) and (self.battle.activeSuits[i].currHP > 0) and (self.battle.activeSuits[i].currHP < self.battle.activeSuits[i].maxHP):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
            ...
            if self.battle.activeSuits[i].dna.name == 'supervis' and self.battle.activeSuits[i].getActualLevel() == 20:  # Abacus Mint Supervisor
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                        ...
                                                        'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'supervis' and self.battle.activeSuits[i].getActualLevel() == 23:  # Abacus Mint Supervisor
                if len(self.battle.activeSuits) > 5:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
    ...
    def __calculateSuitAttacksHighRoller(self):
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'director':
                if self.suitHasCondition(suitId, 'directorcalculator') and len(self.battle.activeSuits) == 6 and not self.__suitCanAttack(suitId) and \
                        self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getAbilityQueued(suitId)
                    self.battle.suitAttacks.append(attack)
    ...
    def __calculateSuitAttacksWitnessStandIn(self):
        ...
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.battle.activeSuits[i].dna.name == 'wsi': #witness stand-in
                if self.suitHasCondition(suitId, 'soakedcalculator') or (x + 2) % 3 == 0 and len(self.battle.activeSuits) >= 6  and not self.deadSuits >= 4 and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
            ...
            if self.battle.activeSuits[i].dna.name == 'redd':  # redd heir wing
                if (x + 4) % 5 == 0 and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
    ...
    def __calculateEndOfRoundAttacks(self):
        for i in xrange(len(self.battle.activeSuits)):
            ...
            if self.suitHasCondition(suitId, 'bellowattack') and self.__suitCanAttack(suitId) and self.battle.activeSuits[i].currHP > 0 and self.battle.activeSuits[i].dna.name == 'ambass':
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                        ...
                                        'group': SuitBattleGlobals.ATK_TGT_GROUP})
                self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'whistleb':
                if self.battle.activeSuits[i].currHP > 0 and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                            'name': 'WhistleCompensation',  # Compensation
                                            'animName': 'cease',
                                            'hp': 0,
                                            'acc': 100,
                                            'freq': 0,
        ...
        for i in xrange(len(self.battle.activeSuits)):
            suitId = self.battle.activeSuits[i].doId
            if self.suitHasCondition(suitId, 'soaked') and not self.suitHasCondition(suitId, 'alreadySoakRemoval') and self.getSuitConditionTurns(suitId, 'soaked') == 1 and self.battle.activeSuits[i].currHP > 0:
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                 ...
                 'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                self.battle.suitAttacks.append(attack)
            if self.suitHasCondition(suitId, 'marked') and not self.suitHasCondition(suitId, 'alreadyMarkRemoval') and self.getSuitConditionTurns(suitId, 'marked') == 1 and not self.battle.activeSuits[i].dna.name == 'bcaster' and not self.battle.activeSuits[i].dna.name == 'hrollers' and self.battle.activeSuits[i].currHP > 0:
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
        ...
        for i in xrange(len(self.battle.activeSuits)): # Cheats before Cog Attacks
            ...
            if self.suitHasCondition(suitId, 'shielding') and self.absorbDamage > 0 and self.battle.activeSuits[i].currHP > 0:
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                        ...
                                        'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                self.battle.suitAttacks.append(attack)
            if self.suitHasCondition(suitId, 'overseerKB') or self.suitHasCondition(suitId, 'overseerCombo') and (self.comboDamage > 0 or self.knockbackDamage > 0) and not self.suitHasCondition(suitId, 'dead'):
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                        'name': 'AttorneyOverseer',  # Overseer Combo/Knockback Heal Movie
                                        'animName': 'nothing',
                                        'hp': 0,
                                        'acc': 100,
                                        'freq': 0,
            ...
            if self.suitHasCondition(suitId, 'deathcheck') and not self.suitHasCondition(suitId, 'dead') and not self.suitHasCondition(suitId, 'alreadyDeathCheck'):
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                        ...
                                        'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].getGovernaught() and self.battle.activeSuits[i].currHP <= 0:
                attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
            ...
            if self.battle.activeSuits[i].dna.name == 'clubpres':
                if self.battle.activeSuits[i].currHP > 0:
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                                ...
                                                                'group': SuitBattleGlobals.ATK_TGT_SINGLE})
                    self.battle.suitAttacks.append(attack)
                if not self.suitHasCondition(suitId, 'sounded') and self.suitHasCondition(suitId, 'unlureSuit') and (self.suitHasCondition(suitId, 'target6') or self.suitHasCondition(suitId, 'target2') or self.suitHasCondition(suitId, 'target7') or self.suitHasCondition(suitId, 'target3') or self.suitHasCondition(suitId, 'target4') or self.suitHasCondition(suitId, 'target5')):
                    attack = self.__getLureRemoval(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'target2'):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
            ...
            if self.battle.activeSuits[i].dna.name == 'safesupervis':
                if self.TurnsElapsed % 1 == 0 and self.__suitCanAttack(suitId):
                    ...
                    self.battle.suitAttacks.append(attack)
            if self.battle.activeSuits[i].dna.name == 'wtapper':
                if self.levelDamage > 0 and self.battle.activeSuits[i].currHP > 0 and self.suitHasCondition(suitId, 'immune'):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                            'name': 'AbsorbMovieLevel',  # Absorb Damage Movie Level
                                            'animName': 'nothing',
                                            'hp': 0,
            ...
            if self.battle.activeSuits[i].dna.name == 'bkeeper':  # bookkeeper
                ...
                if self.suitHasCondition(suitId, 'soakedcalculator') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId, random.choice([{'suitName': self.battle.activeSuits[i].dna.name,
                                                           'name': 'BookkeeperPaperCutSoaked',  # Snap Soaked
                                                           'animName': 'nothing',
                                                           'hp': 0,
                                                           'acc': 100,
                                                           'freq': 0,
                ...
                if self.suitHasCondition(suitId, 'markedcalculator') and self.__suitCanAttack(suitId):
                    attack = self.__getCheatAttack(suitId,  random.choice([{'suitName': self.battle.activeSuits[i].dna.name,
                                                           'name': 'BookkeeperPaperCutMarked',  # Snap Soaked
                                                           'animName': 'nothing',
                                                           'hp': 0,
                                                           'acc': 100,
                                                           'freq': 0,
        ...
        for i in xrange(len(self.battle.activeSuits)):
               ...
               # attack = self.__getGenericSuitAttack(suitId)
                ...
                if not self.battle.activeSuits[i].dna.name in SuitBattleGlobals.SpecialCogDict:
                    attack = self.__getGenericSuitAttack(suitId)
                    self.battle.suitAttacks.append(attack)
                if self.suitHasCondition(suitId, 'syphon') and not self.battle.activeSuits[i].dna.name in SuitBattleGlobals.SpecialCogDict and self.syphonHP > 0 and not self.suitHasCondition(suitId, 'dead'):
                    attack = self.__getCheatAttack(suitId, {'suitName': self.battle.activeSuits[i].dna.name,
                                                            'name': 'SyphonMovie',  # Syphon Movie
                                                            'animName': 'nothing',
                                                            'hp': 0,
    ...
    def calculateRound(self):
        ...
        for suit in self.battle.activeSuits:
            if suit.isGenerated():
                ...
                if suit.dna.name == 'safesupervis':
                    for s in self.battle.activeSuits:
                        self.setSuitCondition(s.doId, 'deadpromotion', 1, 99, 'setBoth')
        ...
        self.__calculateSuitAttacksBoardbotLitigation()
    ...
    def getLuredSuits(self):
        ...
        self.sacrificedCogs -= self.sacrificedCogs
        self.syphonHP -= self.syphonHP
        self.damageHP -= self.damageHP
        self.fraudulentDamage = 0
        self.comboDamage = 0
        self.knockbackDamage = 0
        self.objectionDamage = 0
    ...
    def __incLuredCurrRound(self, suitId):
       ...
       # if x % 99 == 0 and theSuit.dna.name == 'sgoat':
        ...
        if theSuit.dna.name == 'gatekeep':
            self.currentlyLuredSuits[suitId][0] = self.currentlyLuredSuits[suitId][1] - 1
        if self.getSuitConditionTurns(suitId, 'damageReduction') == 1:
            self.currentlyLuredSuits[suitId][0] = self.currentlyLuredSuits[suitId][1] - 1
