...
def getSuitAnimTrack(attack, delay = 0, splicedAnims = None, playRate = 1.0):
    ...
    if attack[
        ...
        track.append(Func(suit.setChatAbsolute, random.choice(['Hrm...', 'Hmph...', 'Hm, hm...', 'Hrnhmpf...']),
                          CFSpeech | CFTimeout))
    elif attack['suitName'] == 'lgator' and attack['name'] == 'LitigatorBayouBash':  # Special track for when Head Honchos use cigar smoke so the animations are no longer playing at the same time.
        track.append(Func(suit.setChatAbsoluteSpecial, taunt,
                          CFSpeech | CFTimeout))
    elif attack['suitName'] == 'sgoat' and attack['name'] == 'ScapegoatEnraged':  # Special track for when Head Honchos use cigar smoke so the animations are no longer playing at the same time.
        track.append(Func(suit.setChatAbsoluteSpecial, taunt,
                          CFSpeech | CFTimeout))
    elif attack['suitName'] == 'caseman' and attack['name'] == 'CaseManagerInsurancePlan':  # Special track for when Head Honchos use cigar smoke so the animations are no longer playing at the same time.
        track.append(Func(suit.setChatAbsoluteSpecial, taunt,
                          CFSpeech | CFTimeout))
    elif attack['suitName'] == 'caseman' and attack['name'] == 'CaseManagerLegalBindings2':  # Special track for when Head Honchos use cigar smoke so the animations are no longer playing at the same time.
        track.append(Func(suit.setChatAbsoluteSpecial, taunt,
                          CFSpeech | CFTimeout))
    else:
        track.append(Func(suit.setChatAbsolute, taunt, CFSpeech | CFTimeout))
    if splicedAnims:
        ...
        track.append(ActorInterval(suit, attack['animName'], playRate=playRate))
    if suit.dna.name == 'sgoat' and suit.isAngry and not attack['name'] == 'ScapegoatShieldsUp' and not attack['name'] == 'ScapegoatEnraged':
        ...
        if attack['name'] == 'ScapegoatEnraged':
            track.append(
            Func(suit.setNeutralAnimationAttack))
        else:
            track.append(
                Func(suit.setNeutralAnimationDrop))
    return track
...
def doShieldsUp(attack):
    ...
    return Parallel(suitTrack, soundTrack, makeDamageUp, suitColorTrack, makeShielding)
