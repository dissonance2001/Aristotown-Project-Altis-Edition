...
def doPromotion(attack, ind):
    ...
    selfDamageTrack = Sequence(Wait(5.0), Parallel(dustCloudHideIval, Sequence(ActorInterval(targetSuit, 'slip-forward', startTime=2.43), Func(targetSuit.setNeutralAnimation)),
                                                   ...
                                                   Func(targetSuit.updateHealthBar, 0)), Func(battle.unSueSuit, targetSuit))
...
def doNoStrikeClause(attack):
    ...
    for t in targets:
        ...
        toonTracks.append(Parallel(Func(toon.makeNoDodge), Func(toon.addNoDodgeRounds, 2)))
...
def doRadioInfrequency(attack):
    ...
    for t in targets:
        toon = t['toon']
        makeDamageDown = Parallel(Func(toon.makeGroupDamageDown), Func(toon.addGroupDamageDownRounds, 2))
...
def doHotTake(attack):
    ...
    notifyTrack.append(Parallel(Func(toon.makeBombed), Func(toon.addBombedRounds, 2)))
