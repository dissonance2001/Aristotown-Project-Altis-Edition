...
def doPaperCut(attack):
    ...
    for t in targets:
        ...
        partTrack = getPartTrack(particleEffect, 0.5, 3.5, (particleEffect, toon, 0), softStop=-2.0)
        toonTrack = getToonTracksCheat(attack, .5, ['cringe'], 3.4, ['struggle'])
        notifyTrack = Sequence(Wait(0.5), Func(toon.showHpTextNew, -int(dmg), text="MARKED!", colorCode=4))
        notifyTrack.append(Parallel(Func(toon.setToonStatusEffect, 'commissionerMarked', turns=3)))
        if dmg > 0:
...
def doBookkeepingRetaliation(attack):
    ...
    for t in targets:
        ...
        notifyTrack.append(Parallel(Func(toon.setToonStatusEffect, 'damageDown', modifier=99, turns=2, mode='keepHighest')))
...
def doOilRainHeal(attack):
    ...
    for targetData in targets:
        ...
        for activeSuit in battle.activeSuits:
            if activeSuit.dna.name == 'phouse':
                currentBossHealth = activeSuit.currHP
                break
        ...
        initialCloudHeight = targetSuit.height + 3
        ...
        cloudPropTrack.append(getPropAppearTrack(cloud, targetSuit, cloudPosPoints, 1e-06, Point3(3, 3, 3), scaleUpTime=0.25))
        ...
        cloudPropTrack.append(Parallel(Sequence(ParticleInterval(rainEffect, cloud, worldRelative=0, duration=4.1, cleanup=True, softStopT=-1)), Sequence(ActorInterval(cloud, 'stormcloud', startTime=3, duration=0.1), ActorInterval(cloud, 'stormcloud', startTime=1, duration=2.3))))
        ...
        puddleTrack = Sequence(Func(battle.movie.needRestoreRenderProp, puddle), Func(puddle.reparentTo, targetSuit), Func(puddle.wrtReparentTo, render), LerpScaleInterval(puddle, 0.9, Point3(1.7, 1.7, 1.7), startScale=MovieUtil.PNT3_NEARZERO), Wait(6.2), LerpFunctionInterval(puddle.setAlphaScale, fromData=1, toData=0, duration=0.8), Func(MovieUtil.removeProp, puddle), Func(battle.movie.clearRenderProp, puddle))

        # =====================================================
        # TARGET MOVEMENT / REANIMATION
        # =====================================================
        if targetSuit != suit and not (targetSuit.dna.name == 'foreman' and targetSuit.getActualLevel() == 28):
            moveTrack = Sequence(Wait(1.8), LerpPosInterval(targetSuit, 0.9, Point3(0, 0, -3.1), other=puddle), LerpPosInterval(targetSuit, 0.4, Point3(0, 0, -9.1), other=puddle))
            ...
            animTrack = Sequence(
                ...
                Func(targetSuit.soakSuit, 0, 1),
                Func(targetSuit.setPos, puddle, Point3(0, 0, 0)),
                Func(targetSuit.clearSuitStatusEffect, 'soaked'),
                Func(targetSuit.clearSuitStatusEffect, 'drenched'),
                ActorInterval(targetSuit, 'reanimated'),
                Func(targetSuit.setSuitStatusEffect, 'oilRain', modifier=1, turns=3),
        ...
        if heal > 0:
            healTrack = Sequence(
                Wait(3.1),
                Parallel(
                    Func(targetSuit.showHpTextNew, heal),
                    Func(targetSuit.setHealthForMe, heal),
                    Func(targetSuit.updateHealthBar, 0),
                    Func(targetSuit.checkHealingPhrases, 0)
                )
            )

            healTracks.append(healTrack)

        # Optional support if Refinement can also damage anything.
        if damage > 0:
            damageTrack = Sequence(
                Wait(3.1),
                Parallel(
                    Func(targetSuit.showHpTextNew, -damage),
                    Func(targetSuit.setHealthForMe, -damage),
                    Func(targetSuit.updateHealthBar, 0)
                )
            )

            healTracks.append(damageTrack)

    # =========================================================
    # AMBASSADOR MAIN TRACK
    # =========================================================
    suitTrack = Sequence(
        getSuitAnimTrack(attack),
        Wait(0.5),
        Func(suit.clearSuitStatusEffect, 'drenched'),
        Func(suit.setSuitStatusEffect, 'oilRain', modifier=1, turns=3),
        Func(suit.clearSuitStatusEffect, 'soaked'),
        Func(suit.soakSuit, 0, 1),
        ActorInterval(suit, 'summon-cog'),
    ...
    soundTrack3 = getSoundTrack('SA_zombie_cogs_rising.ogg', delay=suit.getDuration('snap') + 0.5, node=suit)

    return Parallel(suitTrack, moveTracks, soundTrack4, soundTrack3, soundTrack2, animTracks, healTracks, cloudPropTracks, soundTrack, puddleTracks)
...
def doOilRemoval(attack):
    ...
    for s in battle.activeSuits:
        if s.hasSuitStatusEffect('oilRain'):
            suitTrack.append(Parallel(Sequence(ActorInterval(s, 'soak', startTime=3.5), Func(s.setNeutralAnimationDrop)), Sequence(Wait(1.0), __soakSuit(s, 1)),  Func(s.clearSuitStatusEffect, 'oilRain')))
    return Parallel(suitTrack)
...
def doCollectCall(attack):
    ...
    notifyTrack = Sequence(Wait(4.75), Func(toon.setToonStatusEffect, 'collectCalled', turns=2), Func(toon.showHpTextNew, 0, text="CONNECTED!", colorCode=1))
...
def doAdvancement(attack):
    ...
    suitTrack2 = Sequence(
        ...
        Parallel(
            ...
            LerpAnimInterval(suit, duration=.75, startAnim='sacrifice-cog', endAnim='neutral', startWeight=0.0, endWeight=1.0, blendType='easeInOut')
        ),
    ...
    for targetData in targets:
        ...
        suitTrack = Sequence(
            Wait(1.5),
            Func(targetSuit.checkHealingPhrases, 1),
            Func(targetSuit.setSuitStatusEffect, 'ambassadorTarget', turns=2),
            ActorInterval(targetSuit, 'slip-forward'),
            Func(targetSuit.setNeutralAnimationDrop)
        )
    ...
    return Parallel(suitTracks, soundTrack2, suitTrack2)
...
def doPinkSlip(attack):
    ...
    for t in targets:
        ...
        propTrack.append(Parallel(Func(toon.setToonStatusEffect, 'hotShot', modifier=25, turns=2)))
...
def doVoicemailReal(attack):
    ...
    makeImmune = Parallel(Func(suit.setSuitStatusEffect, 'immune', modifier=1, turns=3))
...
def doMandatoryFiling(attack):
    ...
    for t in targets:
        ...
        y = toonPos.getY()
        propPos = Point3(toonPos.getX(), y, 30)
        soundTrack4 = getSoundTrack('AA_drop_safe.ogg', delay=1.25, node=suit)
        ...
        Parallel(
            ...
            Func(toon.showHpTextNew,  0, text="NO DEFENSE!", colorCode=1),
        ...
        propTracks.append(Parallel(propTrack2, soundTrackMiss, safeAnimTrack, soundTrack4))
...
def doHeadRollerGroup(attack):
    ...
    for targetData in targets:
        ...
        targetTrack = Sequence(
            Wait(1.0),
            ActorInterval(targetSuit, 'soak', duration=2.0),
            Parallel(
                Func(targetSuit.makeUnTarget),
                Sequence(
                    Func(targetSuit.setChatAbsolute, "Ouch.", CFSpeech | CFTimeout),
                    ActorInterval(targetSuit, 'pie-small-react'),
                    Func(targetSuit.setNeutralAnimationDrop)
                ),
                MovieUtil.spawnHeadExplosion(targetSuit, battle),
                Func(targetSuit.showHpTextNew, -dmg, text="TERMINATED!", colorCode=4),
        ...
        if died:
            targetTrack.append(MovieUtil.createSuitHeadlessDeathTrack(targetSuit, battle))
...
def doGhostMentality(attack):
    ...
    origPos2 = manager.getPos(battle)

    walkOutPos = Point3(origPos)
    walkOutPos.setY(walkOutPos.getY() - 12.5)

    # =========================================================
    # CALCULATE FACING TOWARD ACTUAL TARGET
    # =========================================================
    ...
    moveTrack = Sequence(
        Parallel(
            LerpHprInterval(manager, 1.5, targetHpr, startHpr=origHpr, other=battle),
            LerpPosInterval(manager, 1.5, walkOutPos, startPos=origPos, other=battle)
        ),
        Wait(manager.getDuration('deadwood')),
        Parallel(
            LerpHprInterval(manager, 1.5, origHpr, startHpr=targetHpr, other=battle),
            LerpPosInterval(manager, 1.5, origPos, startPos=walkOutPos, other=battle)
        ),
        Func(manager.setPos, battle, origPos),
        Func(manager.setHpr, battle, origHpr)
    )
    ...
    # TARGET EFFECT
    # =========================================================
    effectDelay = manager.getDuration('deadwood')
    ...
    return Parallel(managerTrack, moveTrack, targetTrack, soundTrack)
...
def doAmbassadorDamageUp(attack):
    ...
    syphonedManagerSuit = []
    for suit in battle.activeSuits:
        if not suit.dna.name in SuitBattleGlobals.SpecialCogDict and suit.hasSuitStatusEffect('ambassadorTarget'):
...
def doDamageUp1(attack):
    ...
    damageUpAmount = int(math.ceil(heal * 0.05))

    makeDamageUp = Parallel(Func(theSuit.setSuitStatusEffect, 'damageUp', modifier=damageUpAmount, mode='refreshModifier'))

    managerHealTrack = Sequence(
        Wait(2.0),
        Parallel(
            Func(theSuit.showHpTextNew, heal, text="+%s%% Damage!" % damageUpAmount, colorCode=1),
    ...
    return Parallel(suitTrack, soundTrack, managerHealTrack, makeDamageUp)
...
def doTeeOff(attack):
    ...
    for t in targets:
        ...
        flameEffect.setScale(.7)
        flameTrack = getPartTrack(flameEffect, 0.0, 4.0, (flameEffect, ball, 0), softStop=-1.0)
    ...
    return Parallel(suitTrack, notifyTracks, soundTrack1, toonTracks, clubPropTrack, ballPropTracks, soundTrack)
...
def doCollectCallDamage(attack):
    ...
    for toon in battle.activeSuits:
        if not toon.dna.name == 'wtapper':
            ...
            spinTracks1.append(getPartTrack(spinEffect1, 1.5, 5.9, (spinEffect1, toon, 0), softStop=-2.0))
            spinTracks2.append(getPartTrack(spinEffect2, 1.5, 5.9, (spinEffect2, toon, 0), softStop=-2.0))
            spinTracks3.append(getPartTrack(spinEffect3, 1.5, 5.9, (spinEffect3, toon, 0), softStop=-2.0))
...
def doBookkeeping(attack):
    suit = attack['suit']
    suitTrack = Sequence(getSuitAnimTrack(attack))
    makeDamageUp = Func(suit.setSuitStatusEffect, 'closedSession', modifier=1, turns=2)
    soundTrack = Sequence(SoundInterval(globalBattleSoundCache.getSound('suit_promotion_sfx.ogg'), node=suit))
...
def doGeneration2(attack):
    ...
    suitTrack = Sequence(Parallel(getSuitAnimTrack(attack), ActorInterval(suit, 'mob-mentality', endTime=1)), Parallel(Func(suit.setSuitStatusEffect, 'powerhouseGeneration', modifier=5, mode='refreshModifier')), 
    ...
    makeUnShielding7 = Func(suit.clearSuitStatusEffect, 'soakResist')
    makeUnShielding2 = Func(suit.clearSuitStatusEffect, 'soundImmune')
    makeUnShielding3 = Func(suit.clearSuitStatusEffect, 'throwImmune')
    makeUnShielding4 = Func(suit.clearSuitStatusEffect, 'zapImmune')
    makeUnShielding5 = Func(suit.clearSuitStatusEffect, 'dropImmune')
    makeUnShielding6 = Func(suit.clearSuitStatusEffect, 'syphon')
    makeUnShielding8 = Func(suit.clearSuitStatusEffect, 'lureImmune')
    return Parallel(suitTrack, cagePropTracks, headTrack, makeUnShielding8, soundTrack, makeUnShielding7, makeImmune2, makeUnShielding2, makeUnShielding3, makeUnShielding4, makeUnShielding5, makeUnShielding6, suitTrack2, makeDamageUp, smokeTrack)
...
def doGeneration3(attack):
    ...
    def getCloudTrack(lightning, suit, battle = battle):
        ...
        tracks.append(Sequence(partTrack, Func(particleNode.removeNode)))
        return tracks
    suitTrack = Sequence(Parallel(getSuitAnimTrack(attack), ActorInterval(suit, 'mob-mentality', endTime=1)), Parallel(Func(suit.setSuitStatusEffect, 'powerhouseGeneration', modifier=10, mode='refreshModifier')), 
...
def doAbsorb(attack):
    suit = attack['suit']
    makeShielding = Func(suit.setSuitStatusEffect, 'absorbing', modifier=1, turns=1)
    suitTrack = Sequence(getSuitAnimTrack(attack))
...
def doThrowImmune(attack):
    suit = attack['suit']
    makeShielding = Func(suit.setSuitStatusEffect, 'throwImmune', modifier=1)
    suitTrack = Sequence(getSuitAnimTrack(attack), Func(suit.removePowerhouseRotation))
    texture2 = loader.loadTexture('phase_14/maps/cc_t_ene_circuitbreaker_throw.png')
    headTrack = Sequence()
    suitTrack.append(Wait(3.0))
    for headPart in suit.headParts:
        if not suit.isSkeleton:
            headTrack.append(Func(headPart.setTexture, texture2, 1))
    return Parallel(suitTrack, headTrack, makeShielding)

def doSoundImmune(attack):
    suit = attack['suit']
    makeShielding = Func(suit.setSuitStatusEffect, 'soundImmune', modifier=1)
    suitTrack = Sequence(getSuitAnimTrack(attack), Func(suit.removePowerhouseRotation))
...
def doSoakImmune(attack):
    suit = attack['suit']
    makeShielding = Func(suit.setSuitStatusEffect, 'soakResist', modifier=1)
    suitTrack = Sequence(getSuitAnimTrack(attack), Func(suit.removePowerhouseRotation))
...
def doDropImmune(attack):
    suit = attack['suit']
    makeShielding = Func(suit.setSuitStatusEffect, 'dropImmune', modifier=1)
    suitTrack = Sequence(getSuitAnimTrack(attack), Func(suit.removePowerhouseRotation))
    headTrack = Sequence()
...
def doZapImmune(attack):
    suit = attack['suit']
    makeShielding = Func(suit.setSuitStatusEffect, 'zapImmune', modifier=1)
    suitTrack = Sequence(getSuitAnimTrack(attack), Func(suit.removePowerhouseRotation))
    suitTrack.append(Wait(3.0))
    headTrack = Sequence(Wait(1))
...
def doSyphon(attack):
    ...
    liftTracks = Parallel()
    headTrack = Sequence()
    makeShielding = Func(suit.setSuitStatusEffect, 'syphon', modifier=1)
    ...
    liftTracks.append(getPartTrack(liftEffect, 0.0, 5.1, (liftEffect, suit, 0), softStop=-1.0))
    ...
    for headPart in suit.headParts:
        if not suit.isSkeleton:
            headTrack.append(Func(headPart.setTexture, texture2, 1))
    return Parallel(suitTrack, makeShielding, liftTracks, headTrack)
...
def doSyphonDesperation(attack):
    ...
    for s in battle.activeSuits:
        ...
        liftEffect.setZ(liftEffect.getZ() - 1.3)
        liftTracks.append(getPartTrack(liftEffect, 0.0, 5.1, (liftEffect, s, 0), softStop=-1.0))
...
def doAftershockDamage(attack):
    ...
    for t in targets:
        ...
        def getCloudTrack(lightning, suit, battle = battle):
            ...
            particleNode.setColorScaleOff(1)
            particleEffect.getParticlesNamed('particles-1').emitter.setOffsetForce(Vec3(0.0000, 0.0000, 15.0000 + suit.height))
            soundTrack = getSoundTrack('AA_cog_shock.ogg', delay=tContact - 0.61 + .5, node=toon)
            ...
            tracks.append(track)
            tracks.append(Sequence(Parallel(soundTrack, partTrack), Func(particleNode.removeNode)))
            return tracks
        x = toon.getX()
        y = toon.getY()
        z = toon.getZ()
...
def doAftershock(attack):
    ...
    for t in targets:
        ...
        notifyTrack.append(Parallel(Func(toon.setToonStatusEffect, 'zapped', turns=3)))
...
def doLureImmune(attack):
    suit = attack['suit']
    makeShielding = Func(suit.setSuitStatusEffect, 'lureImmune', modifier=1)
    headTrack = Sequence()
    suitTrack = Sequence(getSuitAnimTrack(attack), Func(suit.removePowerhouseRotation))
    suitTrack.append(Wait(3.0))
...
def doOverseer(attack):
    ...
    # =========================================================
    # TARGET REACTIONS / OVERSEER STATUS
    # =========================================================
    for targetData in targets:
        if 'suit' not in targetData:
            continue
        ...
        targetTrack.append(
            Func(
                targetSuit.setSuitStatusEffect,
                'overseer',
                modifier=1,
                turns=3
            )
...
def doGroundbreaker(attack):
    ...
    for t in targets:
        ...
        puddleTracks.append(Parallel(Func(toon.setToonStatusEffect, 'groundbroken', turns=2)))
