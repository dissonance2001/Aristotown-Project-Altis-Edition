...
def __createSuitDamageTrack(battle, suit, hp, lure, trapProp, revived=0, died=0):
    ...
    result = Sequence()
    suitGone = 0
    specialBossDeath = bool(died) and suit.dna.name in ('psetter', 'chainsaw')
    ...
    if trapName == 'banana':
        ...
        if died and not specialBossDeath and not suit.isVirtual and not suit.hasSuitStatusEffect('overpressured') and suit.dna.name not in ('erfit', 'videog', 'erclaim'):
            suitGone = 1
        ...
        if base.localAvatar in battle.activeToons:
            if not died or specialBossDeath:
        ...
        if died and not specialBossDeath:
            suitGone = 1
            #result.append(createIncomingTrainInterval(battle, suit, hp, lure, trapProp))
    ...
    if suitGone and trapName == 'tnt':
        ...
        elif died != 0 and not suit.isVirtual and not suitGone and not suit.hasSuitStatusEffect('overpressured') and not suit.dna.name == 'videog':
