...
@cutsceneSequence(name='Erfit: Summon Suit', enum=EDE.summonSuitErfit)
def seq_erfitRise(suitIndex:    SEAT.dropdown_suits = 0,
                  destPos:      SEAT.slider_xyz = (0, 0, 0),
                  cutsceneDict: dict = None) -> Sequence:
    puddle = globalPropPool.getProp('quicksand')
    puddle.setColor(LVecBase4f(0.1, 0.1, 1.0, 1))
    puddle.setHpr(Point3(300, 0, 0))
    ...
    startPos = destPos + Point3(0, 0, (-SuitTimings.fromSky * ToontownGlobals.SuitWalkSpeed) / 2)
    suit.setPos(startPos)
    moveIval = createSuitMoveIvalErfit(suit, destPos, puddle.copyTo(render))
    suitTrack = Sequence(
        Func(suit.setPos, startPos),
        seq_showSuit(suitIndex, cutsceneDict),
        moveIval,
...
def seq_applyStagelight(targetIndex:        SEAT.dropdown_actors = None,
                        ...
                        stageLightScaleX:   SEAT.slider_min_zero = 1,
                        stageLightScaleY:   SEAT.slider_min_zero = 1,
                        stageLightScaleZ:   SEAT.slider_min_zero = 2,
    ...
    stagelight.setScale(stageLightScaleX, stageLightScaleY, stageLightScaleZ)
...
@cutsceneSequence(name='Duck Shuffler: Gamble', enum=EDE.duckShufflerRoll, hidden=False)
def seq_duckShufflerRoll(suitIndex:         SEAT.dropdown_suits = 0,
                         rollWait:          SEAT.slider_min_zero = 1.0,
                         rollADuration:     SEAT.slider_min_zero = 3.0,
                         rollBDuration:     SEAT.slider_min_zero = 3.2,
                         rollCDuration:     SEAT.slider_min_zero = 3.4,
                         endDuration:       SEAT.slider_min_zero = 0.1,
                         stopDistance:      SEAT.slider_min_zero = 0.04,
                         rollCount:         SEAT.slider_min_zero = 11,
                         asInterval:        SEAT.boolean = False,
                         cutsceneDict:      dict = None) -> Parallel:
    suit = cutsceneDict['suits'][suitIndex]
    if not suit:
        return Sequence()
    if suit.style.name != 'duckshfl':
        return Sequence()
    eyePosIndex = [cutsceneDict['arguments'][0]] * 3
    if eyePosIndex[0] == 4:
        # BUST ! pick three random ones.
        potentialResults = list(range(4)) * 2
        random.shuffle(potentialResults)
        for i in range(3):
            eyePosIndex[i] = potentialResults.pop()
    rollCount = round(rollCount)
    retParallel = Parallel(
        ActorInterval(suit.specialHead, 'gamble'),
        ActorInterval(suit, 'pull-slots'),
        Sequence(
            Wait(rollWait),
            Func(suit.specialHead.rollEyeSlot,
                 eyeIndex=0, eyePosIndex=eyePosIndex[0],
                 rollDuration=rollADuration,
                 endDuration=endDuration, stopDistance=stopDistance, rollCount=rollCount),
            Func(suit.specialHead.rollEyeSlot,
                 eyeIndex=1, eyePosIndex=eyePosIndex[1],
                 rollDuration=rollBDuration,
                 endDuration=endDuration, stopDistance=stopDistance, rollCount=rollCount),
            Func(suit.specialHead.rollEyeSlot,
                 eyeIndex=2, eyePosIndex=eyePosIndex[2],
                 rollDuration=rollCDuration,
                 endDuration=endDuration, stopDistance=stopDistance, rollCount=rollCount),
        ) if not asInterval else Sequence(
            Wait(rollWait),
            Parallel(
                suit.specialHead.rollEyeSlot(
                     eyeIndex=0, eyePosIndex=eyePosIndex[0],
                     rollDuration=rollADuration,
                     endDuration=endDuration, stopDistance=stopDistance, rollCount=rollCount, asInterval=True),
                suit.specialHead.rollEyeSlot(
                     eyeIndex=1, eyePosIndex=eyePosIndex[1],
                     rollDuration=rollBDuration,
                     endDuration=endDuration, stopDistance=stopDistance, rollCount=rollCount, asInterval=True),
...
@cutsceneSequence(name='Duck Shuffler:  the voices', enum=EDE.duckShufflerEyePos, hidden=False)
def seq_duckShufflerEyePos(suitIndex:       SEAT.dropdown_suits = 0,
                           eyePos:          SEAT.slider_min_zero = 0.0,
                           cutsceneDict:      dict = None) -> Parallel:
    suit = cutsceneDict['suits'][suitIndex]
    if not suit:
        return Sequence()
    if suit.style.name != 'duckshfl':
        return Sequence()
    def death():
        suit.specialHead.setEyePos(eyePos)
    return Sequence(
        Func(death)
    )
