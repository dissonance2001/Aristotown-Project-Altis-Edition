...
def createSuitMoveIvalErfit(suit, destPos, hole):
    ...
    moveIval = Sequence(Func(suit.pose, 'landing', 0),
                        ProjectileInterval(suit, duration=flyingDur, endPos=destPos,
                                           gravityMult=0.125), ActorInterval(suit, 'landing'))
    if suit.prop is None:
        suit.prop = BattleProps.globalPropPool.getProp('propeller')
    propDur = suit.prop.getDuration('propeller')
    lastSpinFrame = 8
    ...
    bigScale = 2.25
    biggerScale = 2.5
    holeTrack = Sequence(Wait(0.25),
                         Func(hole.show),
                         LerpScaleInterval(hole, 0.65, biggerScale, blendType='easeIn'),
                         LerpScaleInterval(hole, 0.15, bigScale, blendType='easeOut'),
                         Wait(1.5),
                         LerpScaleInterval(hole, 0.15, biggerScale, blendType='easeIn'),
                         LerpScaleInterval(hole, 0.65, smallScale, blendType='easeOut'),
                         Func(hole.removeNode))
    hole.hide()
    hole.setScale(0.01)
    result = Parallel(
        Func(suit.attachPropeller),
...
def seq_suitApplyVisualEffect(suitIndex:     SEAT.dropdown_suits = 0,
                              vfxIndex:      SEAT.dropdown_visual_effects = 0,
    ...
    return Sequence(
        Wait(delay),
        Func(MovieUtil.applyVisualEffect, suit, effect)
    )


@cutsceneSequence(name='Suit: Unapply Visual Effect', enum=EDE.suitUnapplyVisualEffect)
def seq_suitUnapplyVisualEffect(suitIndex:     SEAT.dropdown_suits = 0,
                                vfxIndex:      SEAT.dropdown_visual_effects = 0,
                                delay:         SEAT.slider_min_zero = 0.0,
                                cutsceneDict:  dict = None) -> Sequence:
    ...
    return Sequence(
        Wait(delay),
        Func(MovieUtil.unapplyVisualEffect, suit, effect)
    )
...
def seq_setChainsawHeadGlitchState(suitIndex: SEAT.dropdown_suits = 0,
                                   delay:  SEAT.slider_min_zero = 0,
                                   glitch:  SEAT.boolean = 0,
                                   cutsceneDict: dict = None) -> Sequence:
    ...
    glitchFunc = suit.specialHead.enterGlitch if glitch else suit.specialHead.exitGlitch
...
def seq_setPrethinkerBrainBlastState(suitIndex: SEAT.dropdown_suits = 0,
                                   delay:  SEAT.slider_min_zero = 0,
                                   cutsceneDict: dict = None) -> Sequence:
    suit = cutsceneDict['suits'][suitIndex]
    if not suit:
    ...
    return Sequence(
        Wait(delay),
        Func(suit.specialHead.doBrainBlast)
    )
...
def seq_suitFireFromCannon(suitIndex:  SEAT.dropdown_suits = 0,
    ...
    reactIval = Parallel(
        ...
        Sequence(
            LerpPosInterval(cannonHolder, 2.0, Point3(0, 0, 7), blendType='easeInOut'),
            Parallel(
                rotateToPointTrack,
            ...
            Parallel(
                LerpHprInterval(barrel, 0.6, Point3(0, 90, 0), blendType='easeIn'),
                playSoundCannonAdjust
            ),
            LerpPosInterval(cannonHolder, 1.0, Point3(0, 0, 0), blendType='easeInOut')
        ),
        Sequence(
            Wait(0.0),
            Parallel(
                ...
                Sequence(
                    ...
                    Parallel(
                        playSoundCogPanic,
                        LerpPosInterval(suit, 1.5, Point3(0, -deep, 0.0), blendType='easeIn')
                    )
                )
            ),
            Wait(fireDelay),
...
def seq_laaTrapdoor(cutsceneDict:    dict = None) -> Parallel:
    retParallel = Parallel()
    for suit in cutsceneDict['suits']:
        if not suit:
            continue
        ...
        retParallel.append(Sequence(
            Sequence(ActorInterval(suit, 'flail', startTime=0, endTime=0.65), Wait(1)), Sequence(ActorInterval(suit, 'flail', startTime=0.65, endTime=0), Func(suit.loop, 'neutral'))
            )
        )
    return retParallel
