...
class TownBattleCogPanel(DirectFrame):
    ...
    def setLevelText(self):
        ...
        if ((self.cog.getManager() or self.cog.getGovernaught()) and
            ...
            if (self.cog.hasSuitStatusEffect('lureResist') or
                    self.cog.hasSuitStatusEffect('lureImmune') or
                    self.cog.hasSuitStatusEffect('highRollerImmune') or
                    self.cog.hasSuitStatusEffect('immune')):
                ...
                try:
                    lureRounds = int(self.cog.getSuitStatusModifier('lureResist'))
                except:
                    lureRounds = 0
                if lureRounds <= 0:
                    description = "This Cog is entirely immune to being %s." % gagText
                    iconPulse = True
                else:
                    description = "This Cog will stay %s for %d round%s." % (
                        gagText, lureRounds, '' if lureRounds == 1 else 's')
                    iconPulse = False
        ...
        if self.cog.hasSuitStatusEffect('yellowLight'):
            status = loader.loadModel('phase_3.5/models/gui/status_effects')
            self.statusIcon = loader.loadModel('phase_5/models/effects/cc_m_txc_fx_bat_target_indicators')
            ...
            iconRoot = NodePath('immuneIcon')
            self.statusIcon.reparentTo(iconRoot)
            self.extraText = DirectLabel(parent=iconRoot, relief=None, text="%s" % self.cog.getSuitStatusTurns('yellowLight'),
                                         text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1),
                                         text_font=ToontownGlobals.getInterfaceFont(), text_bg=Vec4(0, 0, 0, 0),
                                         pos=(0.25, 0, -0.45),
                                         text_scale=.6)
