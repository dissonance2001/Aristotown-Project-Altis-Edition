...
class ToontownMusic:
    ...
    def __init__(self):
        ...
        if __debug__:
            musicPath = Filename("/phase_3/audio/music.json")
        else:
            musicPath = Filename("/phase_3/audio/music.json")

        # self.contentLutsMgr = base.contentLutsMgr

        self.prevMusicList = []   # Stores last played music list, to allow for music to properly sync between subzones
        self.prevMusicChoice = ''  # Stores the choice of the above list.
        self.musicData = json.loads(vfs.readFile(musicPath, True))
        self.overrideContentPackMusic()
        self.playingMusic = {}  # Dict that stores music codes to their active music files
        ...
        if isinstance(music, str):
            ...
            if musicFile:
                ...
                if (musicFile.length() == 0) and music != NULL_MUSIC:
                    ...
                    self.playingMusic[music] = musicFile  # Store this music file in self.playingMusic
                    base.playMusic(musicFile, looping, 1, volume, time)
                    self.notify.warning(f'Music path {music} is either misnnamed or incorrectly pathed in music.json. Check to make sure you are using relative paths (/audio/bgm/file.ogg)')
    ...
    def getMusicFilepath(self, musicCode):
        ...
        if musicPath is None:
            try:
                ...
                self.notify.warning(f"Music with code {musicCode} did not load properly. Check music.json and see if it was properly implemented.")
                return 'error'
    ...
    def getMusicTime(self, music):
        ...
        if music:
            ...
            self.notify.warning(f'Tried to get music time for song {music} that was not active. Returning 0')
            return 0
    ...
    def setPlayRate(self, music, playRate):
        ...
        if isinstance(music, str):
            ...
            if sound:
                ...
                self.notify.warning(f'Tried to set the playrate of music {music} when it was not active!')
        else:
            # Assume it's an AudioSound
    ...
    def setVolume(self, music, volume):
        ...
        if isinstance(music, str):
            ...
            if sound:
                ...
                self.notify.warning(f'Tried to set the volume of music {music} when it was not active!')
        else:
            # Assume it's an AudioSound
    ...
    def fadeOutMusic(self, time: float = 3.0, music=None):
        """
