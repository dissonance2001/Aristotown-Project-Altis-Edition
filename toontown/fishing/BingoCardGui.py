...
class BingoCardGui(DirectFrame):
    """
    BingoCardGui(DirectFrame)

    Provide a base layout of the bingo card GUI which can be used in a variety of games.
    """

    def __init__(self, parent = aspect2d, **kw):
        """
        This method provides initial construction of the Card.
        It determines if the card is of a valid size, and impossible for diagonal cases.
        In addition, it initializes the DirectFrame and BingoCardBase classes from which it is derived.

        :param parent: The parent that the card. (aspect2d)
        :param kw: OptionDefs for the DirectFrame
        """
    ...
    def loadGameTimer(self):
        """
        This method instantiates a game timer, positions it and scales it according to the CardImageScale.
        """
    ...
    def resetGameTimer(self):
        """
        This method reset the Game Timer to Zero.
        """
        self.timer.reset()

    def startGameCountdown(self, time):
        """
        Starts the Countdown of the game timer.

        :param time: the amount of time to countdown from in secs
        """
    ...
    def startNextGameCountdown(self, time):
        """
        Starts the Countdown of the intermission and timeout timer.
    ...
    def hideNextGameTimer(self):
        """
        This method hides the next game timer. It is called when the timer is no longer in use.
        """
        self.nextGame['text'] = ''
        self.nextGame.hide()
    ...
    def showNextGameTimer(self, text):
        """
        This method shows the next game timer. It is called when the timer is needed once more.

        :param text: text to display with the timer.
        """
    ...
    def resetNextGameTimer(self):
        """
        This method resets the next game timer to zero.
        """
        self.nextGameTimer.reset()

    def stopNextGameTimer(self):
        """
        This method stops the next game timer when it is no longer needed.
        """
        self.nextGameTimer.stop()

    def setNextGameText(self, text):
        """
        This method sets the text to display for the intermission or timeout period.
    ...
    def setJackpotText(self, text):
        """
        This method sets the text to display for the jackpot.
    ...
    def resetGameTypeText(self):
        """
        This method resets the Game Type text to ''.
        """
        self.gameType['text'] = ''
    ...
    def loadNextGameTimer(self):
        """
        This method instantiates a game timer, hides the image
        of the timer, and positions it relative to the card
        so that when a timeout has occurred, the countdown will
    ...
    def setGameOver(self, text):
        """
        This method sets the gameOver text to the appropriate string.

        :param text: string of text that should be updated.
        """
        self.gameOver['text'] = text

    def load(self):
        """
        Loads the basic GUI of the card outside of the base image.
        For now, Labels for 'B'I'N'G'O' are generated
        and are scaled/positioned according to the image scale.
        """
        ...
        self.gameOver = DirectLabel(
            ...
            relief = None, state = DGG.NORMAL,
            text = '', text_scale = textScale,
            text_fg = (1, 1, 1, 1),
            text_font = ToontownGlobals.getSignFont()
        )
    ...
    def destroy(self):
        """
        This method cleans up the Card so that there are no persisting memory leaks.
        Each cell within the card is destroyed as well as each letter label.
        """
        self.cleanTutorial()
    ...
    def loadCard(self):
        """
        This method generates the necessary logos for each cell within the card.
        It then attempts to show those logos based-on the current game state.

        If a cell is not occupied by a "Bingo Marker", ie a 0 in the gameState bit-string, then its logo will be shown.
    ...
    def disableCard(self):
        """
        This method disables each cell within the card so that the button can no longer trigger an invalid play attempt.
        """
    ...
    def enableCard(self, callback = None):
        """
        This method enables each cell within the card so that when a button is pressed,
        it triggers the appropriate callback routine.

        The Free Cell is never enabled.

        :param callback: A callback routine to handle a button press.
        """
    ...
    def generateCard(self, tileSeed, zoneId):
        """
        This method generates the actual card, and is based-on the tileSeed
        that is provided by the AI and the zoneId which is obtained from the pond.
        ...
        for i in range(emptyCells):
            fishVitals = FishGlobals.getRandomFishVitals(zoneId, rodSubtype, rng)
        ...
        for i in range(rowSize):
            for j in range(self.game.getColSize()):
                ...
                if i * rowSize + j == int(self.game.getCardSize() / 2):
                    tmpFish = 'Free'
                else:
    ...
    def cellUpdateCheck(self, id, genus, species):
        ...
        If so, that cell should now become disabled and the checkForWin method is called in
    ...
    def cellUpdate(self, cellId):
        """
        This method updates a particular cell.

        Essentially, it disables this cell so that a player can no
        longer click the button and generate further checks against it.
    ...
    def getGame(self):
        """
        Accessor method that retrieves the current game which is associated with the GUI instance.

        :return: the current bingo game
        """
        return self.game

    def removeGame(self):
        """
        This method removes the current game from the bingo gui.

        This occurs after a game has been completed and a new game will be instantiated in the next round.

        It will also occur the CardGui is destroyed.
        """
        self.stopCellBlinking()
        if self.game:
            self.game.destroy()
    ...
    def getUnmarkedMatches(self, fish):
        """
        :return: a list of cells that match a particular fish
        """
    ...
    def getCellColor(self, id):
        """
        This method determines what color a cell on the bingo card should be.
        This is useful for representing patterns such as Diagonals or Four Corner, etc.

        :param id: the Cell Id to return this color
        :return: specific color or base color of a cell
        """
    ...
    def fishCaught(self, fish):
        """
        This method is called when a fish has been caught.
    ...
    def setBingo(self, state = DGG.DISABLED, callback = None):
        """
        This method updates the current state of the bingo button that is used to "call" out bingo.

        :param state: DGG.NORMAL or DGG.DISABLED. (Default: DGG.DISABLED)
        :param callback: a callback function to check for bingo when a user clicks the button.
        :return: 0 [BG.NO_UPDATE] | 2 [BG.WIN]
        """
    ...
    def checkForBingo(self):
        """
        This method checks for bingo of the current game and returns if there was a win.
        ...
        if self.game:
            ...
            # reset bingo button if in between games but we still think we can get a bingo
            self.bingo['state'] = DGG.DISABLED
            taskMgr.remove('bingoFlash')
    ...
    def showJackpot(self):
        return  # hiding jackpot for now, since it is a bit broken
        self.jpSign.show()
    ...
    def flashJackpotLights(self, flashMode, nTimeIndex = 0):
        """
        - 0: "cool chasing lights"
        - 1: "simple blink"
        - 2: "every third light... rotating"

        :param int flashMode: 0 | 1 | 2
        :type nTimeIndex: int
        :return: Task
        """
        if flashMode == 2:
    ...
    def makeTutorial(self):
        self.tutorial = TTDialog.TTDialog(
            ...
            sidePad=0,
            borderScale=0.03,
            # use this one to set the size of the dialog since it's the longest string
    ...
    def showTutorial(self, messageType):
        ...
        self.tutorial.show()
        self.tutorial.doPopInAnimation()
    ...
    def onMouseEnter(self, event):
        """
        the mouse has just entered this entity, show tutorial
        """
    ...
    def onMouseLeave(self, event):
        """
        the mouse has just left this entity, hide tutorial
        """
        self.hideTutorial()
