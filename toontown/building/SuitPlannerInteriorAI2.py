...
class SuitPlannerInteriorAI:
    ...
    def __setupSuitInfo(self, suit, bldgTrack, suitLevel, suitType):
        ...
        if suitName and self.respectInvasions:
            ...
            suitLevel = min(max(suitLevel, suitType), suitType + 12)
        dna = SuitDNA.SuitDNA()
        dna.newSuitRandom(suitType, bldgTrack)
        suit.dna = dna
        self.notify.debug('Creating suit type ' + suit.dna.name + ' of level ' + str(suitLevel) + ' from type ' + str(
            suitType) + ' and track ' + str(bldgTrack))
        ...
        if random.randint(0, 100) <= ToontownBattleGlobals.EXECUTIVE_BASE_CHANCE and not suit.getManager():
            suit.setExecutive(1)
        if random.randint(0,
                          100) <= ToontownBattleGlobals.GOVERNAUGHT_BASE_CHANCE and not suit.getManager() and not suit.getExecutive():
