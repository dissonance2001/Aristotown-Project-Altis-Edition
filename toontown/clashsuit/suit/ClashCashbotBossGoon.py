...
class ClashCashbotBossGoon(ClashGoon.ClashGoon, DistributedCashbotBossObject.DistributedCashbotBossObject):
    ...
    def __init__(self, cr):
        ...
        ClashGoon.ClashGoon.__init__(self, cr)
        self.target = None
        ...
        self.toMagnetSoundInterval = Sequence(SoundInterval(self.flyToMagnetSfx, duration=BossCogGlobals.CashbotBossToMagnetTime, node=self), SoundInterval(self.hitMagnetSfx, node=self))
    ...
    def generate(self):
        ...
        ClashGoon.ClashGoon.generate(self)
    ...
    def announceGenerate(self):
        ...
        ClashGoon.ClashGoon.announceGenerate(self)
    ...
    def getMinImpact(self):
        return BossCogGlobals.CashbotBossGoonImpact
    ...
    def handleToonDetect(self, collEntry = None):
        if self.boss.localToonIsSafe:
            return
        if not getattr(self, 'dclass', None):
            self.notify.warning(f'CashbotBoss State: {self.boss.state}')
            self.notify.error(f'You likely have gotten here due to a walk-state exploit. This is not recommended.')
        ClashGoon.ClashGoon.handleToonDetect(self, collEntry)
    ...
    def enterOff(self):
        ClashGoon.ClashGoon.enterOff(self)
    ...
    def exitOff(self):
        ...
        ClashGoon.ClashGoon.exitOff(self)
    ...
    def enterBattle(self, avId = None, ts = 0):
        ClashGoon.ClashGoon.enterBattle(self, avId, ts)
    ...
    def enterStunned(self, ts = 0):
        ClashGoon.ClashGoon.enterStunned(self, ts)
        self.unstashCollisions()

    def enterRecovery(self, ts = 0, pauseTime = 0):
        ClashGoon.ClashGoon.enterRecovery(self, ts, pauseTime)
        self.unstashCollisions()
