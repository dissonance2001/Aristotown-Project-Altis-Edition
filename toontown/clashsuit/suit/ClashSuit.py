...
# whether or not to enable battle detection when a suit
# enters bellicose mode
ALLOW_BATTLE_DETECT = 1


@DirectNotifyCategory()
class ClashSuit(ClashSuitBase, FSM, DelayDeletable):
    """
    ...
    defaultTransitions = {
        "Off": ["FromSky", "FromSuitBuilding", "Walk", "Battle", "Neutral", "ToToonBuilding",
                "ToSuitBuilding", "ToCogHQ", "FromCogHQ", "ToSky", "FlyAway", "DanceThenFlyAway",
                "WalkToStreet", "WalkFromStreet"],
        "FromSky": ["Walk", "Battle", "Neutral", "ToSky", "WalkFromStreet"],
        "FromSuitBuilding": ["WalkToStreet", "Walk", "Battle", "Neutral", "ToSky"],
        "WalkToStreet": ["Walk", "Battle", "Neutral", "ToSky", "ToToonBuilding", "ToSuitBuilding",
                         "ToCogHQ", "WalkFromStreet"],
        "WalkFromStreet": ["ToToonBuilding", "ToSuitBuilding", "ToCogHQ", "Battle", "Neutral", "ToSky"],
        "Walk": ["WaitForBattle", "Battle", "Neutral", "WalkFromStreet", "ToSky", "ToCogHQ", "Walk", "FromCogHQ",
                 "ToSuitBuilding"],
        "Battle": ["Walk", "ToToonBuilding", "ToCogHQ", "ToSuitBuilding", "ToSky"],
        "Neutral": [],
        "WaitForBattle": ["Battle", "Neutral", "Walk", "WalkToStreet", "WalkFromStreet", "ToToonBuilding",
                          "ToCogHQ", "ToSuitBuilding", "ToSky"],
        "ToToonBuilding": ["Neutral", "Battle"],
        "ToSuitBuilding": ["Neutral", "Battle"],
        "ToCogHQ": ["Neutral", "Battle", "Walk", "FromCogHQ", "ToCogHQ"],
        "FromCogHQ": ["Neutral", "Battle", "Walk", "FromCogHQ", "ToCogHQ"],
        "ToSky": ["Battle"],
        "FlyAway": ["Walk", "WalkToStreet", "WalkFromStreet", "ToToonBuilding", "FromSuitBuilding", "ToSuitBuilding",
                    "FromCogHQ", "ToCogHQ", "FromSky", "ToSky"],
        "DanceThenFlyAway": ["Walk", "FromSky", "ToSuitBuilding", "WalkFromStreet"],
    }
    ...
    def __init__(self, cr):
        ...
        ClashSuitBase.__init__(self, cr)
        # Set up the ClashSuit state machine
        ...
        # Used to initiate battle with suits by throwing pies at them -- disabled due to state bugs and whatnot.
    ...
    def announceGenerate(self):
        ...
        """
        ClashSuitBase.announceGenerate(self)
        self.resumePath(1)
    ...
    def setPathPosition(self, index, timestamp):
        ...
        # To ensure we don't receive some surprising overflow on the AI,
        # we'll go ahead and modulo the index so that the suit is
        # indeed arriving at an expected point.
        self.pathPositionIndex = index % self.legList.getNumLegs()
    ...
    def doPathLeg(self, leg, time):
        ...
        already elapsed.
        """
        self.request(leg.getTypeName(), leg, time)
        return 0
    ...
    def __localPieSplat(self, pieCode, entry):
        if pieCode == ToontownGlobals.PieCodeNotBossCog and entry.getIntoNodePath().getNetTag('pieBattleDetect') == str(self.doId):
            self.d_requestBattle(self.getPos(), self.getHpr())
        return
    ...
    def enterWalkFromStreet(self, leg, time):
        ...
        h = self.calculateHeading(a, b)
        self.mtrack = Parallel(
            LerpPosInterval(self, leg.getLegTime(), b1, startPos=a),
            LerpHprScaleInterval(self, .3, Vec3(h, 0, 0), Vec3(1, 1, 1)),
    ...
    def enterWalk(self, leg, time):
        ...
        if leg.getType() == SuitLeg.TWalkHidden:
        ...
        self.mtrack = Parallel(
            ...
            LerpQuatScaleInterval(self, .3, Vec3(h, 0, 0), Vec3(1, 1, 1)),
    ...
    def enterBattle(self):
        ClashSuitBase.enterBattle(self)
        self.resumePath(0)
    ...
    def enterNeutral(self):
        ...
        self.notify.debug('ClashSuit: Neutral (entering a Door)')
        self.resumePath(0)
        self.loop('neutral', 0)
    ...
    def enterWaitForBattle(self):
        ClashSuitBase.enterWaitForBattle(self)
        self.resumePath(0)
    ...
    def playDialogue(self, type, length, delay = 0.0):
        ...
        if sfxIndex is not None and sfxIndex < len(dialogueArray) and dialogueArray[sfxIndex] is not None:
            ...
            self.updateSoundSpeed(sfx)
            soundSequence = Sequence(Wait(delay), SoundInterval(sfx, node=None, listenerNode=base.localAvatar, loop=0, volume=1.0))
