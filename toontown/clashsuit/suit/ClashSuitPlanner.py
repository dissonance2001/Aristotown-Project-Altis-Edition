...
class ClashSuitPlanner(DistributedObject.DistributedObject, SuitPlannerBase.SuitPlannerBase):
    """
    The client side version of the suit planner.
    This version has less functionality than the server side, but some functionality is common between the two
    """
    ...
    def generate(self):
        ...
        """
        self.notify.debug('ClashSuitPlanner %d: generating' % self.getDoId())
        DistributedObject.DistributedObject.generate(self)
        base.cr.currSuitPlanner = self

    def disable(self):
        """
        Called when the DistributedObject is removed from the world.
        """
        self.notify.debug('ClashSuitPlanner %d: disabling' % self.getDoId())
        self.hidePaths()
    ...
    def hidePaths(self):
        """
        Hides the visualization created by a previous call to showPaths().
        """
        if self.pathViz:
    ...
    def showPaths(self):
        """
        Draw a visualization of the suit paths at runtime for the user's convenience.
        """
        self.hidePaths()
        ...
        # Also create an unused collision sphere to show each battle cell.
    ...
    def __doShowPoints(self, vizNode, lines, p, points):
        if p is None:
            # Choose a new point at random.
            # We arbitrarily take the last one on the list.
            pi = len(points) - 1
            if pi < 0:
                return
            p = points[pi]
            ...
            if p not in points:
                # We've already visited this point, and presumably already drawn the edge.
                return
