...
class ClashCashbotBossGoonAI(ClashGoonAI.ClashGoonAI, DistributedCashbotBossObjectAI.DistributedCashbotBossObjectAI):
    legLength = 10
    directionTable = [(0, 15),
     (10, 10),
     (-10, 10),
    ...
    def __init__(self, air, boss):
        ClashGoonAI.ClashGoonAI.__init__(self, air, 0)
    ...
    def requestBattle(self, pauseTime):
        ...
        ClashGoonAI.ClashGoonAI.requestBattle(self, pauseTime)
    ...
    def requestStunned(self, pauseTime):
        ...
        if avId in self.numStompsDict:
            self.numStompsDict[avId] += 1
        else:
            self.numStompsDict[avId] = 1
        self.boss.d_updateGoonsStomped(avId)
        self.isStunned = True
        if avId in self.boss.toonGoonStompsDict:
            exp = math.ceil(25 * self.scale)
            if avId in self.numStompsDict:
                diminished_exp = exp * (15/(self.numStompsDict[avId]+4))
                self.boss.toonGoonStompsDict[avId] += 1
                toon = self.cr.doId2do.get(avId)
                if toon:
                    mult = toon.getDeptExpMult(ToontownGlobals.DEPARTMENT_CASHBOT)
                    toon.addDepartmentExp(math.ceil(diminished_exp * mult), ToontownGlobals.DEPARTMENT_CASHBOT)
            ...
            if avId in self.numStompsDict:
                ...
                if toon:
                    mult = toon.getDeptExpMult(ToontownGlobals.DEPARTMENT_CASHBOT)
                    toon.addDepartmentExp(math.ceil(diminished_exp * mult), ToontownGlobals.DEPARTMENT_CASHBOT)
        ClashGoonAI.ClashGoonAI.requestStunned(self, pauseTime)
    ...
    def hitBoss(self, impact, craneId):
        ...
        if self.state == 'Dropped' or self.state == 'Grabbed':
            if not self.boss.heldObject:
                damage = int(impact * 20 * self.scale)
                self.boss.recordHit(max(damage, 2), impact, craneId, self)
        self.b_destroyGoon()
    ...
    def enterGrabbed(self, avId, craneId):
        self.stunGrabbed = bool(self.isStunned)
        self.isStunned = False

        crane = simbase.air.doId2do.get(craneId)
        if crane:
            if crane.getIndex() > 3: # Red magnets & check if goon is stomped
