...
class Goon(Avatar.Avatar):
    ...
    def createHead(self):
        ...
        if self.type == 'pg':
            ...
            self.hat.find('**/security_hat').hide()
            self.hat.find('**/hard_hat').hide()
            hat = loader.loadModel('cosmetics/hat/models/cc_m_acc_hat_golf_classic')
            hat.setScale(0.5)
        ...
        if base.cr.newsManager and base.cr.newsManager.isHolidayRunning(ToontownGlobals.CHRISTMAS):
            hat = loader.loadModel('phase_13/models/events/toonseltown/goon_antler_hat')
            hat.reparentTo(self.hat)
            eyeColor = random.choice([Vec4(1, 0, 0, 1), Vec4(0, 1, 0, 1)])
            self.radarColor = (eyeColor[0], eyeColor[1], eyeColor[2], 0.2)
        else:
            eyeColor = Vec4(1, 1, 0, 1)

        self.eye = self.find('**/eye')
        self.eye.setColorScale(1, 1, 1, 1)
        self.eye.setColor(eyeColor)
        self.radar = None
    ...
    def scaleRadar(self):
        ...
        if self.radarColor:
            self.radar.setColor(self.radarColor)
        else:
            self.radar.setColor(1, 1, 1, 0.2)

    def colorHat(self):
        if base.cr.newsManager and base.cr.newsManager.isHolidayRunning(ToontownGlobals.CHRISTMAS):
            colorList = GoonGlobals.WINTER_COLORS
        elif self.type == 'pg':
